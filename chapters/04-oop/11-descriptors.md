...
**描述器**是一個實作了下列任一方法的類別，當它被當成**另一個類別的類別屬性**時，會攔截對該屬性的存取：
...
- **data descriptor（資料描述器）**：有 `__set__` 或 `__delete__`。**優先於** instance `__dict__`。
- **non-data descriptor（非資料描述器）**：只有 `__get__`。**低於** instance `__dict__`。
