...
- **不打算獨立實例化**，也不代表一個「is-a」的完整概念。
- 透過**多重繼承**「混入」主類別（見 [MRO](04-mro.md)）。

**組合（composition）** 則是「一個物件擁有其他物件當屬性」，透過**委派（delegation）** 呼叫它們——表達 **has-a** 關係。

兩者都是「不重複程式碼」的手段，但繼承是「我**是**一種父類別」，組合是「我**擁有**一個協作者」。
