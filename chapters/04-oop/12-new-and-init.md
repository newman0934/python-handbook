...
> `__init__` 你天天用，但它其實不負責「造出」物件——那是 `__new__` 的事。這章講物件誕生的兩個步驟，以及為什麼「單例」「子類化不可變型別」這些問題，答案都藏在 `__init__` 之前那一步。
...
- 你要做**單例**（整個程式只該有一個設定物件、一個連線管理器）——攔在 `__new__` 控制「到底造不造新的」。
- 你要**子類化不可變型別**（繼承 `str` / `int` / `tuple` 並改造它的值）——值在 `__new__` 就定死了，`__init__` 動不了。
- 你用了某個框架，發現「`__init__` 沒被呼叫」或「回傳的不是我以為的型別」——八成有人動了 `__new__`。
- 你在讀 `namedtuple`、`enum`、或某些 ORM 的原始碼，看到 `__new__` 想搞懂它在幹嘛。
- 面試被問:「**`__new__` 和 `__init__` 差在哪?什麼時候要用 `__new__`?**」
