...
- **原生居民（primitive）**：int、bool 這類基本型別——輕巧，但沒有戶籍、不算「物件」，很多場合不能直接用（Java 要 boxing 裝箱才行）。
- **物件公民（Object）**：有完整身分的類別實例。

**Python 的世界只有一種居民：全員都是物件公民。**

整數 `5`、字串、函式、類別、模組、甚至「型別」本身——**通通有戶口**：

- 有**身分證字號**（`id`）
- 有**類別欄**（`type`）
- 有**內容**（value）
- 都能被賦值、傳遞、塞進容器——享有完整公民權

這不是抽象口號,可以當場查戶口：

```pycon
>>> id(5), type(5)          # 整數有戶口
>>> id(len), type(len)      # 函式有戶口
>>> id(int), type(int)      # 連「int 這個型別」也有戶口——它是 type 的實例!
```

最後一行揭示這個制度是**遞迴到底**的：型別也是物件、型別的型別還是物件（`type(type) is type`）——[Part 4 的模具工廠](../04-oop/13-metaclass.md)的世界觀根基就在這。

為什麼這是「第一原理」？因為後面每一章——引用計數、記憶體、GIL——講的都是「**物件**怎麼被管理」;而 Python 裡**沒有任何值能豁免**。
...
在很多語言（如 Java），有「基本型別（primitive）」和「物件（Object）」的二分——`int` 不是物件，要 boxing 才能當物件用。

**Python 沒有這種二分：所有值都是物件**——全員有戶口。

「是物件」意味著每個值都：

- **存在於堆積（heap）記憶體**，透過參照（名稱綁定——[便利貼](../02-fundamentals/01-dynamic-typing.md)）存取。
- **有身分（identity）**：唯一的 `id`。
- **有型別（type）**：它屬於哪個類別。
- **有值（value）**：它的內容。
- **可被當作值操作**：賦值、傳遞、當參數/回傳、存進容器。

而且這是**遞迴的**——連「型別」本身也是物件（`int` 是一個 `type` 物件），「型別的型別」還是物件（`type` 是 `type` 自己，見 [metaclass](../04-oop/13-metaclass.md)——模具工廠自己造自己）。
