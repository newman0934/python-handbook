...
呼叫函式傳東西，像安排客人入座，有兩種方式：

- **照座位順序入座**（位置引數）：`f(1, 2)`——第一個給第一個參數，依序對號。
- **對名牌入座**（關鍵字引數）：`f(b=2, a=1)`——指名道姓，順序隨意。

再來是兩個明星語法，名字嚇人、原理簡單：

- **`*args`**＝「**沒名牌的、統統裝進這個袋子**」——多出來的位置引數收成一個 tuple。
- **`**kwargs`**＝「**有名牌的、統統裝進這個袋子**」——多出來的關鍵字引數收成一個 dict。

所以 `def f(*args, **kwargs)` 是「什麼都收」的萬用簽章（裝飾器最愛）。
反方向也通：呼叫時 `f(*my_list)` 是「把袋子倒出來」逐一入座。

定義端還有兩個「座位區隔線」：`/` 之前只能對號入座、`*` 之後必須對名牌——用來設計嚴謹的 API,看得懂即可。
...
先記結論：**預設值只在函式定義時建立一次**,所有呼叫共用同一個 list——第二次呼叫會看到第一次的殘留。正解是 `items=None` 再在函式內建新的。為什麼會這樣,章內拆解。

## 🔗 前端對照

`*args` / `**kwargs` 和前端的 **rest / spread（`...`）** 是同一類東西——收集或展開多個引數:

| 目的 | Python | JavaScript |
|------|--------|-----------|
| 收集多個位置引數 | `def f(*args):` | `function f(...args) {}` |
| 展開陣列當引數 | `f(*mylist)` | `f(...myArray)` |
| 具名選項 | `def f(**kwargs):` | 慣例上傳一個 options 物件 `f({ ... })` |
| 展開 dict / 物件 | `f(**mydict)` | `f({ ...myObj })`（合併物件） |

一句話:`*` ≈ 陣列的 `...`、`**` ≈ 物件的 `...`。差別:Python 把「具名引數」做成語言層的
**keyword arguments**（`**kwargs`）;JS 沒有具名引數,慣例是**傳一個 options 物件**。
...
呼叫函式時，引數（argument）有兩種傳法——兩種入座方式：

- **位置引數（positional）**：依順序對應，`f(1, 2)`——照座位順序入座。
- **關鍵字引數（keyword）**：依名稱對應，`f(a=1, b=2)`，順序無所謂——對名牌入座。

定義端則能規定每個參數「能用哪種方式傳」：Python 3 提供 `/` 和 `*` 兩個分隔符（座位區隔線），把參數列切成三段——`/` 之前只准位置、`*` 之後只准關鍵字——精確控制介面。
