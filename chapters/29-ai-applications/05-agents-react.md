...
[RAG](01-rag-pipeline.md) 讓 LLM 會查資料,但還是「**問一次、答一次**」的單步反射。
真實任務常常需要**多步驟、隨機應變**:
「查一下這個客戶最近的訂單,如果有退貨就發道歉信,並更新 CRM」——
這要查資料、做判斷、再行動,環環相扣。**Agent(智能代理)** 就是讓 LLM 能這樣工作。

核心引擎叫 **ReAct = Reasoning(推理)+ Acting(行動)** 的循環,
像一個邊想邊做的助理,每一輪吐出三樣東西:

```text
Thought(想):「要回答這問題,我得先查訂單資料庫」
Action(做):  呼叫工具 query_orders(客戶ID)      ← 就是 Part 28 的 tool use
Observation(看):工具回傳的結果
   → 接著再想下一步…直到有足夠資訊,給出最終答案
```

**想 → 做 → 看結果 → 再想**,不斷循環,直到任務完成。
關鍵在那個 **Thought**:讓模型**把推理攤開來寫**([chain-of-thought](../28-llm-genai/03-prompt-engineering.md)),
它才會想清楚再動手,你也才能 debug「它為什麼這樣做」。

這個看似簡單的循環,是所有 AI agent 的心臟。但它也有典型的坑,這章會講:
**兜圈子**(卡在同樣的思考出不來)、**幻覺工具**(呼叫不存在的工具)、
**無限迴圈**(要設步數上限)。你會**親手從零寫一個 ReAct agent**——
不靠框架,才能真正看懂 agent 在做什麼,之後用 [框架](09-frameworks.md)時心裡有數。
