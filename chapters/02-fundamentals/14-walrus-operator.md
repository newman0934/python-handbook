...
背景知識：Python 刻意規定 `=`（賦值敘述）**沒有值**、不能放進 if 裡——就是防 C 語言經典的 `if (x = 5)` 手滑慘案（把 == 打成 =）。
`:=` 是後來開的一扇「明確標示的窗」：想在運算式裡賦值？可以，但你得用這個一看就知道意圖的特殊符號。

三個它發光的場景：while 條件（`while (chunk := read()):`）、推導式裡避免重算、if 判斷順便留值。

警告跟推導式一樣：**它是消除重複的工具，不是壓行數的工具**——塞得到處都是，可讀性反而崩。

## 🎯 什麼時候會用到

`:=`(賦值運算式)的價值是**「算一次、順手綁定、就地使用」**,只在少數情境真正加分:

- **`while` 讀取迴圈**:`while (line := f.readline()):`——邊讀邊判斷,
  免去「迴圈前讀一次、迴圈裡再讀一次」的重複。
- **comprehension 裡避免重算**:`[y for x in data if (y := f(x)) > 0]`——
  `f(x)` 只算一次,又能同時當條件與結果。
- **免去「只為了下一行 if 而寫的暫存變數」**:`if (n := len(data)) > 10: print(n)`。

⚠️ **什麼時候「別」用**:純粹為了少寫一行而塞進複雜運算式——會傷可讀性。
walrus 是「剛好能讓程式更清楚」時的小工具,不是越用越潮——**別人看不懂,就是用錯了。**

一句話:**同一個值「要算、要判斷、又要留用」擠在一處時 → walrus;其餘情況普通賦值更清楚。**
...
- **賦值敘述（statement）**：`x = 5`。是「敘述」，本身**沒有值**，不能放進運算式——`if (x = f()):` 是語法錯誤。
  這是刻意設計：防止 C 語言經典的 `=`/`==` 手滑慘案。
- **賦值運算式（expression）**：`x := 5`。**既賦值、又回傳被賦的值**，所以可以嵌在運算式中——那扇「明確標示的窗」。
...
name := expression
```
（上面是語法示意，非可執行片段；`:=` 不能單獨當敘述，實際用法見下方範例。）
