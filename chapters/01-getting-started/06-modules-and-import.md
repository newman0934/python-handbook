...
**模組（module）就是一個 `.py` 檔。** 檔名 `math_utils.py` 對應模組名 `math_utils`。

模組是 Python 組織程式碼的基本單位，它提供**命名空間（namespace）**：`math_utils` 裡的 `add` 和 `string_utils` 裡的 `add` 是兩個不同的東西（`math_utils.add` vs `string_utils.add`），不會撞名。這讓大型程式可以把功能分門別類，各自獨立。

當你 `import` 一個模組，Python 會執行那個檔案，把裡面定義的名稱（函式、類別、變數）收進一個**模組物件**，你再透過這個物件去取用。是的——**模組本身也是物件**（呼應「一切皆物件」）。
