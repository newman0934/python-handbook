...
- **拋出（raise）**：出錯時，程式「拋出」例外，**立即中斷當前的正常執行流程**。
- **傳播（propagate）**：拋出的例外沿著**呼叫堆疊往上冒**——當前函式沒處理，就傳給呼叫它的函式，一路往上。
- **處理或終止**：若某層用 `try/except` 接住它，就在那裡處理；若一路冒到最頂層都沒人接，程式**終止**並印出 traceback。

這和「回傳錯誤碼」的根本差異：例外**不會被默默忽略**——不處理就崩潰，強迫你正視錯誤。
