...
> 日期：2026-07-14
> 狀態：**已實作完成**（A/B/C/D 全數落地，CI 全綠）
> 對象：**已完成**的 python-handbook（實作前 31 Part、368 章；實作後 385 章）
...
- [x] `chapters/00-backend-foundations/` 建立，含 `README.md` 索引與 10 章（ch01–09 + 統整章 ch10）。
- [x] Part 14 新增背景任務進階（B：ch21–23）與 API 契約深化（D：ch19–20）章群，統整章（→24）同步更新。
- [x] Part 21 新增「作為可靠 HTTP 客戶端」章群（C：ch09–10），統整章（→11）同步更新。
- [x] `chapters/README.md` 總覽新增 Part 0 列；CLAUDE.md 章數更新（→385）。
- [x] `ruff` / `mypy` / `pytest`（examples）/ `check_chapters.py` 全綠（part00 的 UDP 測試在 Windows 偶發 WinError 10013，Linux CI 不受影響）。
- [x] 每個新增可執行範例都實測過、附預期輸出（不得只是死程式碼）。
