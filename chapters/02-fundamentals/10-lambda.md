...
`lambda` 就是**便條紙函式**：一句話寫得完的小函式，寫在便條上隨手遞出去。

```python
sorted(words, key=lambda w: len(w))   # 遞一張便條：「照長度排」
```

和正式函式（`def`）的關係，一句話講完：**兩者做出來的都是函式物件，沒有本質差別**——差在便條紙有兩個限制：

1. **只能寫一個運算式**（不能換行、不能寫多個敘述），運算式的值就是回傳值（不用寫 return）。
2. **沒有名字**（所以叫匿名函式）——本來就是用完即丟的。

什麼時候用？就一種場景：**「臨時傳個小行為」給別的函式**（`sorted` 的 key、`max` 的 key、callback）。

什麼時候不用？也很好記：

- **需要名字**（`square = lambda x: ...`）→ 直接用 `def`，還能有 docstring 和好看的錯誤訊息。
- **一行寫不下** → 用 `def`。
- 便條紙開始像一篇作文 → 用 `def`。

一句話：**lambda 是便條，def 是正式文件——便條夠用時很方便，但別把合約寫在便條上。**

## 🔗 前端對照

`lambda` 對應前端天天在用的**箭頭函式（arrow function）**——都是「一行的匿名函式」。但 Python 刻意做得很克制:

| | Python `lambda` | JavaScript 箭頭函式 |
|---|-----------------|--------------------|
| 語法 | `lambda x: x + 1` | `x => x + 1` |
| 函式體 | **只能一個運算式** | 可單運算式,也可 `{ ... }` 多行 |
| 常見用途 | 當 `sorted` / `map` 的 `key` / 回呼 | 到處都是（回呼、方法、簡短函式） |
| `this` / 作用域 | 一般閉包規則 | 箭頭函式**綁定外層 `this`**（Python 無此問題） |

一句話:概念一樣,但 **Python 的 `lambda` 只能放一個運算式**——要多行就得用具名 `def`。
JS 箭頭函式沒這限制,所以用得比 `lambda` 廣得多。
...
`lambda` 和 `def` 建立的都是**函式物件**，沒有本質區別——差別只在語法與限制（便條紙 vs 正式文件）：

- `def` 建立**具名**函式，主體可以是**多行敘述**。
- `lambda` 建立**匿名**函式，主體只能是**單一運算式**——該運算式的值就是回傳值（不寫 `return`）。
...
「匿名」是指它不需要名字就能存在（可直接當引數遞出去）；上面綁到 `square` 只是示範——實務上真要命名就該用 `def`（見最佳實踐）。
