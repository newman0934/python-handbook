...
> class 能建立物件；那**誰建立 class**？既然「class 本身也是物件」，它必然由某個東西造出來——那個東西叫 metaclass（預設是 `type`）。這章解開這個「工廠的工廠」，也告訴你為什麼 99% 的時候你不該碰它。

## 💡 白話導讀（建議先讀）

前面說過：class 是模具，實例是模具做出來的雞蛋糕。

現在問一個奇怪但合理的問題：**模具是誰做的？**

答案：模具工廠。在 Python 裡，這個工廠叫 **metaclass（元類別）**，而預設的那家工廠，名字叫 `type`。

證據可以自己驗：

```pycon
>>> type(d)      # 雞蛋糕的型別 → Dog（模具）
>>> type(Dog)    # 模具的型別 → type（工廠！）
```

所以整條生產鏈是：**實例，由 class 做出來；class，由 metaclass 做出來**。

為什麼會有這條鏈？因為 Python「一切皆物件」——class 也是物件，是物件就得有人把它做出來。
你寫 `class Dog:` 這段語法時，其實是在**向 type 工廠下訂單**：「照這張規格，做一個叫 Dog 的模具給我」。

自訂 metaclass，等於開一家自己的工廠——在「模具被製造的瞬間」動手腳：自動註冊、檢查規格、改造內容。
...
回到[一切皆物件](../10-cpython-internals/01-everything-is-object.md)：class 也是物件，是物件就必然「被某個東西建立出來」。
那個東西就是 **metaclass**——導讀說的「模具工廠」。
...
<class 'type'>
>>> type(type)       # type 是它自己的 metaclass（工廠自己造自己）
<class 'type'>
```

**生產鏈**：`實例` ← 由 `class` 建立；`class` ← 由 `metaclass`（預設 `type`）建立。所以：

- `class` 是「產生實例的模板」（模具）。
- `metaclass` 是「產生 class 的模板」（模具工廠）。

`type` 是 Python 內建的預設 metaclass——不特別指定時，所有 class 都由這家工廠出品。
