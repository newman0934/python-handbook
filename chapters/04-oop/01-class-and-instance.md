...
> `self` 到底是什麼、`__init__` 什麼時候被誰呼叫、為什麼同一個 class 蓋出來的兩個物件會**不小心共用同一個清單**？這章從「藍圖 vs 蓋出來的房子」講起，把物件導向的第一塊地基打穩。
...
Python 的 class 和 JavaScript 的 `class` 概念一致,但幾個「一定會遇到」的差異值得先知道:

| | Python | JavaScript |
|---|--------|-----------|
| 建構子 | `def __init__(self):` | `constructor() {}` |
| 實例本身 | **明寫 `self`** 當第一參數 | 隱含的 `this` |
| 建立實例 | `Foo()`（**沒有 `new`**） | `new Foo()` |
| 存取屬性 | `self.x` | `this.x` |
| 私有 | 慣例前底線 `_x`（非強制） | `#x`（真正私有） |

一句話:模型相同,但 Python **要你明寫 `self`**、建立實例**不用 `new`**。
JS 的 `this` 會因呼叫方式而變（常見坑）;Python 的 `self` 是明確傳入的參數,不會有那種困惑。
