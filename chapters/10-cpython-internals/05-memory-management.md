...
Python 程式每秒生成海量小物件（每個 int、每個暫時的 tuple⋯⋯）。
如果**每做一個小物件都去跟作業系統要記憶體**——像每用一顆螺絲就跑一趟五金行——光路程就拖垮效能。

CPython 的解法是開**自家倉庫（pymalloc）**，三層批發制度：

```text
五金行(作業系統)
   ↑ 一次批一大箱(arena,256 KB)
自家倉庫
   ├── 分格抽屜(pool,4 KB)——每格「只放同一種尺寸」的零件
   │     └── 零件格(block)——物件實際住的小格子
```

規則兩條：

1. **小物件（≤ 512 bytes）找自家倉庫**——同尺寸的格子隨拿隨用,不驚動作業系統;用完歸位,下一個同尺寸物件直接續住。**Python 絕大多數物件都走這條快速通道。**
2. **大物件（> 512 bytes）才專程跑五金行**（直接 malloc）。

「同尺寸放同格」是關鍵巧思:回收再分配時**不會產生記憶體碎片**——這格永遠放這個尺寸,拿走一個、補進一個,嚴絲合縫。

這章也回答一個常見疑問:「del 了大 list,為什麼工作管理員裡記憶體沒降?」——因為記憶體還給的是**自家倉庫**（供之後重用）,不一定立刻退還給作業系統。這是正常的,不是洩漏。
...
CPython 的記憶體管理分層——自家倉庫制度：大物件直接找作業系統，小物件（≤ 512 bytes）由 CPython 自己的配置器 **pymalloc** 管理，避免頻繁的系統呼叫：

```text
作業系統記憶體（五金行）
    ↑（大物件 > 512 bytes 直接 malloc）
CPython pymalloc（自家倉庫，管小物件 ≤ 512 bytes）
    ├── arena（256 KB 大塊，向 OS 批貨）
    │   └── pool（4 KB 抽屜，同一 pool 只分配同尺寸的 block）
    │       └── block（物件實際住的小格）
```

- **arena**：pymalloc 向作業系統要的大塊記憶體（256 KB）。
- **pool**：arena 切成的 4 KB 頁——每個 pool 只分配**同一尺寸**的 block（不產生碎片的關鍵）。
- **block**：物件實際佔用的小塊。
