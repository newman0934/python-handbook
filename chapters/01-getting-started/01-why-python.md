...
用幾個維度定位 Python，每一個都會影響你之後怎麼寫程式：
...
- **「動態型別」不等於「弱型別」。** 動態指的是「型別何時決定」（執行期），弱/強指的是「會不會自動轉型」。Python 是「動態 + 強型別」——型別執行期才知道，但一旦知道就嚴格，不會亂轉。
- **「直譯」不代表「沒有編譯」。** CPython 其實會先把原始碼編成 bytecode（`.pyc`），只是這步是自動、隱形的。詳見 [Python 如何執行](12-how-python-runs.md)。
