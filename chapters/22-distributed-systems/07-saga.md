...
在單體裡,「下單＋扣庫存＋扣款」包成一個資料庫交易,
全成功或全回滾([ACID](../15-database/README.md)),睡得安穩。
拆成三個微服務、三個資料庫後,**跨服務的 ACID 交易不存在了**——怎麼辦?

想想**旅行社訂行程**:機票、飯店、租車分別跟三家公司訂,
不存在「三家同時鎖定、一鍵全訂」的魔法。實際做法是**依序訂**:
機票訂到→飯店訂到→**租車沒車了**→怎麼辦?
打電話**取消**已訂的飯店、再取消機票——按**相反順序撤銷**,
回到「彷彿沒訂過」的狀態。

這就是 **Saga**:把分散式交易拆成一串**本地交易** T1→T2→T3(各自 ACID),
第 k 步失敗,就執行前面各步的**補償交易** Ck-1…C1。

注意「補償」不是資料庫回滾,是**語意上的撤銷**——
取消機票可能被收手續費、已寄出的信收不回來(只能再寄一封更正信)。
所以設計 Saga 的真功夫在:**每一步都要先想好「怎麼撤」**,
撤不了的步驟(如對外付款)盡量排最後。

協調方式兩派:

- **編舞(choreography)**:沒有指揮,各服務**訂閱彼此的事件**自主反應
  (訂單建立→庫存服務聽到就扣庫存→…)。去中心,但流程散在各處,難追蹤。
- **編排(orchestration)**:一個**指揮家**(orchestrator)集中下令、收結果、
  決定下一步或補償。流程一目瞭然,主流選擇,但指揮家要高可用。

這章用 Python 實作一個編排式 Saga(含補償與重試),
並講與[冪等](06-idempotency.md)的搭配——補償也可能被重複執行,同樣要冪等。
