...
沒有入場管制的遊樂園,一團爆量遊客(失控的重試迴圈、惡意刷量)就能癱瘓全園。
**限流器**就是入場管制,四種演算法是四種管制策略:

- **固定視窗＝整點換場**:每小時放 100 人,整點歸零重計。簡單,
  但有**邊界漏洞**:59 分進 100 人、00 分再進 100 人——兩分鐘內灌進兩倍。
- **滑動視窗日誌＝記下每個人的入場時間**:任何時刻往回看 60 分鐘,精確,
  但要存每筆時間戳,人多時記憶體爆炸。
- **滑動視窗計數＝用前後兩場的加權估算**:精度與成本的折衷,實務常用。
- **令牌桶（token bucket）＝定速發放的入場券**:每秒往桶裡丟 10 張券,
  桶最多存 100 張;來一個請求拿一張,沒券就拒絕。
  精髓是**允許短暫尖峰**(桶裡存的券可以一口氣用掉)但**長期平均被鎖死**——
  最常被問、也最常被用(nginx、雲端 API 多是它或近親 leaky bucket)。

分散式場景的追問也要準備:計數器放哪?——放單機記憶體,
每台各自為政(客戶端換台機器就重新計);放 Redis 集中計數,
就要處理原子性(`INCR`+`EXPIRE` 或 Lua script)。
被限流的回應也有規矩:**429 Too Many Requests**＋`Retry-After` 標頭。
這章用 Python 把四種演算法各實作一遍,邊界行為用測試釘死。
