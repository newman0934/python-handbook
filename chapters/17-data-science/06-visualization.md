...
`df.describe()` 給你平均數和標準差,但兩組資料可以平均數相同、長相天差地遠——
一張直方圖讓你**看見**分布的形狀、離群值、雙峰。視覺化不是美化,是探索工具。

matplotlib 是 Python 畫圖的地基,先分清它的兩層物件（新手最大坑）：

- **`Figure`＝相框**：整張圖的容器。
- **`Axes`＝框裡的一格子圖**——**不是「軸線」！**（軸線叫 axis,英文只差一個字母,超級誤導。）
  一個相框可以裝好幾格（2×2 儀表板就是一個 Figure、四個 Axes）。

用法也只推薦一種——**物件導向風格**,指名道姓對哪格畫：

```python
fig, ax = plt.subplots()      # 拿到相框與格子
ax.plot(x, y)                 # 對這一格畫線
ax.set_title("標題")          # 對這一格設標題
```

（另一種 `plt.plot(...)` 全域風格像「對著空氣喊畫」,單圖方便,多圖必亂,正式程式別用。）

圖的種類按**你想回答的問題**選,不是按好看選：看趨勢→折線圖;比大小→長條圖;
看分布→直方圖;找兩變數關係→散佈圖。pandas 的 `df.plot()` 與 seaborn
都是 matplotlib 的皮,懂了 Figure/Axes,它們畫出來的圖你都能再加工。
