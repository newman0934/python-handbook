...
上一章的分層有個隱憂：業務層直接依賴資料層——**廚房綁死某一家供應商**。
哪天供應商換了（換資料庫、換 ORM），廚房（業務邏輯）也得跟著改。這合理嗎？

Clean Architecture 的回答是：**把最重要的東西放在圓心，讓所有依賴指向圓心**。

想像一顆蛋：**蛋黃是業務規則**（「訂單金額不能為負」——不管用什麼框架，這條規則永遠成立），
**蛋白是流程編排**（use case），**蛋殼才是框架、資料庫、UI**。
蛋黃不知道蛋殼的存在；換一個殼，蛋黃一行都不用改。

「讓依賴指向核心」聽起來抽象，做法其實只有一招（下一章會細講）：
**核心定義介面（interface），外圍來實作**。
業務層說「我需要一個能 `save(order)` 的東西」（定義介面），
至於背後是 PostgreSQL 還是記憶體 dict，是外圈的事——**細節依賴抽象，而不是抽象依賴細節**。

好處立刻兌現在測試：測業務邏輯時塞一個假的記憶體版實作進去，
不用起資料庫、不用起 Web 伺服器，毫秒級跑完。

這章講同心圓的分法、依賴規則、以及 Python 裡怎麼落地（不必教條式照搬四層）。
