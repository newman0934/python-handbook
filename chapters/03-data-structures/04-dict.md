...
dict 為什麼查東西那麼快？用置物櫃講：

**笨方法（list）**：東西隨便放，要找就從第一格逐格翻——十萬格平均翻五萬次（O(n)）。

**dict 的方法**：放東西時，先把「名字」丟進一台**號碼機**（hash 函式），吐出一個櫃號，放進那一格。
要找的時候？同一個名字再丟進號碼機——**同樣的名字永遠吐同一個號碼**——直接走到那格開櫃。
**不用翻找，直接定位**（平均 O(1)）。這台號碼機＋櫃子的組合，術語叫**雜湊表（hash table）**。

這個機制立刻解釋兩件事：

1. **為什麼 key 必須「不可變」**（str、int、tuple 可以，list 不行）：
   號碼機是照「內容」算號的。如果 key 內容會變——放的時候算出 3 號櫃，內容變了之後算出 7 號櫃——**東西永遠找不回來**。所以會變的東西不准當 key。
2. **為什麼 dict 查找快過 list 百倍**：直接定位 vs 逐格翻。

再補一個現代事實：**Python 3.7 起，dict 保證記住插入順序**——先放的遍歷時先出來。
...
dict 底層是**雜湊表（hash table）**——那套「號碼機＋置物櫃」。存 `d[key] = value` 時：

1. 對 key 呼叫 `hash(key)` 算出雜湊值（號碼機吐號）。
2. 用雜湊值決定放進底層陣列的哪個槽（bucket）（哪一格櫃子）。
3. 查找時同樣算 hash 直接定位——**平均 O(1)**，不用逐一比對。

這解釋兩件事：

- **key 必須 hashable**（可算 hash 且不可變，見 [hashable](07-hashable.md)）：`int`/`str`/`tuple` 可以，`list`/`dict`/`set` 不行——內容會變，號碼就變，東西在櫃子裡迷路。
- **查找/插入/刪除平均 O(1)**，遠快於 list 的 O(n) 線性搜尋。

現代事實：**Python 3.7 起 dict 保證保留插入順序**（3.6 是實作細節，3.7 寫進語言規範）——遍歷順序 = 插入順序。
