...
先講一個工程界的共識：**程式碼被「讀」的時間，遠多於被「寫」的時間**。
所以「寫得別人看得懂」不是禮貌，是生產力。

問題是，每個人的習慣不同——變數名用駝峰還是底線？等號兩邊要不要空格？
如果一個團隊十個人十種風格,讀程式碼就像讀十種筆跡的手寫信。

**PEP 8** 就是 Python 官方的「標準筆跡」：一份風格指南，規定命名、縮排、空格、換行怎麼寫。
全世界的 Python 專案幾乎都遵守它——於是**所有 Python 程式碼看起來像同一個人寫的**，你打開任何開源專案都不用重新適應。

兩件事先知道：

1. **違反 PEP 8 不會報錯**——它是慣例不是語法。但不遵守，code review 會被念、協作會痛苦。
2. **你不用背它**——現代工具（ruff、black）一鍵自動排版，[第 11 章](11-editor-and-tooling-setup.md)會配好。這章重點是看懂「為什麼這樣規定」。

比 PEP 8 更深一層的是「**Pythonic**」這個詞——意思是「順著 Python 的設計思考」，而不是把 Java/C 的寫法硬翻成 Python 語法。
例如：遍歷 list 直接 `for item in items`，而不是 `for i in range(len(items))`——後者一看就是「別的語言的口音」。
這章會給你一組「口音矯正」對照表。
...
**PEP = Python Enhancement Proposal（Python 增強提案）**——Python 官方的設計文件流程。
任何語言層級的改動、慣例、決策，都以編號的 PEP 記錄。你會反覆遇到幾個：

- **PEP 8**：程式碼風格指南（本章主角）——那份「標準筆跡」。
...
PEP 8 由 Guido 等人撰寫，是**官方推薦的風格慣例**——不是語法強制（違反不報錯），但整個社群、幾乎所有專案都遵循。
遵守它的回報：任何人打開你的程式碼都「零適應成本」。
