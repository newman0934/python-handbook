...
一棟辦公大樓 20 家公司,訪客難道要自己記每家的樓層、
每家各驗一次證件?當然不是——**一樓設總櫃檯**:
驗證件、換訪客證、指路、擋掉推銷的,一次搞定。

**API gateway 就是微服務的總櫃檯**——所有外部請求的**單一入口**:

- **指路（路由）**:`/orders/*` 轉給訂單服務、`/users/*` 轉給使用者服務——
  客戶端只需要記一個網址。
- **驗證件（認證）**:token 在櫃檯驗一次,後面的服務**信任櫃檯**,
  不必 20 個服務各實作一次 JWT 驗證——
  [橫切關注](../16-architecture/README.md)集中做,是 gateway 的最大價值。
- **擋黃牛（限流）**:[令牌桶](../20-security-system-design/11-system-design-rate-limiter.md)
  放在入口,惡意流量進不了大樓。
- 順手做的:TLS 終止、回應快取、請求記錄、跨服務聚合（BFF 模式）。

代價也要知道:櫃檯是**單點**（要做高可用）、多一跳延遲、
以及最危險的——櫃檯太聰明:業務邏輯偷偷長進 gateway,
它就變成了新的單體（「智慧管道、笨端點」是反了,應該**笨管道、智慧端點**）。

實務選型:雲端託管(AWS API Gateway)、自架(Kong、Traefik、nginx)、
或 K8s 的 Ingress/Gateway API。這章講職責邊界、選型、
並用 FastAPI 示範一個教學版 gateway(路由+驗證+限流)看清內部機制。
