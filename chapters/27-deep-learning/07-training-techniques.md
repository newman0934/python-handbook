...
[手刻網路](03-nn-from-scratch.md)在 XOR 玩具問題上很聽話,
但訓練**真實的深度網路**會冒出一堆麻煩:嚴重過擬合、訓練到一半「學不動」了、
或者損失突然變成 NaN 爆掉。這章是深度學習的**實戰急救包**——
一套讓「能訓練」變成「訓練得好」的必備技巧。

分三類麻煩、三套藥:

**1. 過擬合(背熟訓練資料、換新資料就崩):**

- **Dropout**:訓練時**隨機關掉一部分神經元**——逼網路別過度依賴某幾個,
  像強迫團隊每天有人請假,大家都得學會補位,結果更穩健。
- **Early stopping**:盯著驗證損失,一旦不再進步就**及時收手**,停在泛化最好的點。
- **資料增強**:把圖片翻轉、裁切、變色,一張變多張——免費擴充資料。

**2. 訓練不動(梯度消失/爆炸):**

- 深層網路裡,梯度一路往回傳會**越乘越小(消失)或越乘越大(爆炸)**。
- 解方:用 **ReLU**(不像 sigmoid 兩端會把梯度壓扁)、
  **好的權重初始化**(He/Xavier,讓訊號不會一開始就衰減)、
  **Batch Normalization**(每層把數值拉回穩定範圍)。

**3. 學得太慢/不穩:**

- 用更聰明的**優化器**:**Adam** 會自動替每個參數調整步伐,
  比純梯度下降快又穩,是現代預設。
- **學習率排程**:訓練後期把步伐調小,穩穩收斂。

這些技巧不是可有可無的裝飾——**沒有它們,深層網路根本訓練不起來**。
這章逐一講原理與 PyTorch 用法,是你從「跑得動 demo」到「訓得出真模型」的橋。
