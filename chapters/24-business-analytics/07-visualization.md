...
同一份資料,一張對的圖讓決策者**三秒看懂**你算三天的洞察;
一張錯的圖,能讓**正確的資料傳達錯誤的訊息**——甚至誤導、欺騙。
這章講的不是「怎麼畫得漂亮」,是**怎麼畫得對、畫得誠實**。

選圖表只問一個問題:**「我想表達哪種關係?」**——目的決定圖種:

- **比大小**→長條圖(別用圓餅圖硬比接近的數值,人眼判斷弧形面積很差)。
- **看趨勢**→折線圖(時間序列的預設)。
- **看分布**→直方圖、箱型圖。
- **看兩變數關係**→散佈圖。

重頭戲是**圖表的誠實原則**——這些是新聞和簡報裡最常見的操弄手法,
你要會識破、更要拒絕使用:

- **長條圖 Y 軸不從 0 開始**:把「成長 2%」畫成「翻了三倍」的視覺假象——最經典的騙術。
- **雙 Y 軸硬湊相關**:兩條線放一起看似同漲同跌,其實只是刻度對齊的錯覺。
- **3D 圖、花俏特效**:扭曲比例、掩蓋真相,幾乎沒有例外地該避免。
- **櫻桃摘取(cherry-picking)**:只選對你有利的時間區間。

一句話立場:**圖表是論證的工具,不是化妝品**。
資料墨水比(data-ink ratio)、去除多餘裝飾、標清單位與來源——
這章教你畫出「經得起質疑」的圖。
