...
教科書的資料乾乾淨淨，真實資料像剛收來的二手貨：**有缺頁、有重複、標籤貼錯、
字串帶著多餘空白、同一個城市寫成「台北」「臺北」「Taipei」三種**。
資料分析界的名言——80% 的時間花在清理——這章教你把這 80% 花得又快又對。

第一課是認識「缺」怎麼表示。pandas 的缺失值 `NaN` 有兩個反直覺的脾氣：

- **有傳染性**：`NaN + 1` 還是 `NaN`——像一滴墨水染黑整杯水，運算鏈上有一個缺值，結果就缺。
- **連自己都不等於自己**：`NaN == NaN` 是 `False`！所以偵測缺失**不能用 `==`**，
  要用 `df.isna()`。（還有個歷史坑：整數欄一出現缺失，整欄會被升級成 float——
  新式的 `Int64` 可空型別能救。）

面對缺失只有三條路，各有適用場合：**刪**（`dropna`，缺太多且列數夠時）、
**補**（`fillna`，用 0／平均／前值,看語義）、**留**（有些模型能直接吃缺值）。

接著是例行的清理套餐,這章逐項示範：去重複（`drop_duplicates`）、修型別
（`astype`、`pd.to_datetime`——「數字被讀成字串」是最常見病）、字串整容
（`.str.strip()`／`.str.lower()`）、統一類別值（`replace`／`map`）。
清完的標準只有一條：**每欄型別正確、值域合理、缺失有被刻意處理過**。
