...
前面兩章都指到這裡：「能不能當 dict key、能不能進 set」——判準只有一個詞：**hashable**。

回到[置物櫃與號碼機](04-dict.md)的畫面。一個物件要能被號碼機服務（hashable），要滿足：

1. **報得出一個號碼**（有 `__hash__`），而且**這輩子報的號碼都一樣**。
2. **能回答「我跟它是不是同一個」**（有 `__eq__`）。

為什麼「號碼不能變」是死規定？想像這一幕：

> 你拿著一顆 list 去寄物。號碼機照它的內容算出 3 號櫃，放進去。
> 之後有人 append 了一個元素——內容變了。
> 你回來取件，號碼機照「新內容」算出 7 號櫃。你走到 7 號櫃——**空的**。
> 東西還在 3 號櫃，但你永遠算不回那個號碼了。**它在櫃子裡迷路了。**

這就是為什麼 **Python 直接規定：會變的內建型別（list、dict、set）不准寄物**（不可 hash），
而不可變的（str、int、tuple、frozenset）都可以——內容不會變，號碼永遠算得回來。

這章的後半講自訂類別怎麼辦（預設可 hash，但按「身分」不按「內容」），
以及一條重要契約：**`__eq__` 說相等的兩個物件，hash 必須相同**——自己實作時破壞這條，dict 就會出現找不到東西的靈異現象。
...
一個物件是 **hashable** 的，若它滿足兩個條件（能被號碼機服務的資格）：

1. 有 `__hash__()` 方法，能回傳一個整數 hash 值，且這個值在物件**生命週期內不變**（報的號碼一輩子一樣）。
2. 有 `__eq__()` 方法，能和其他物件比較相等。

dict 和 set 用雜湊表儲存（見 [dict](04-dict.md)），要對 key/元素算 hash 定位——所以 **key/元素必須 hashable**。

**可 hash 與不可變高度相關**：內容會變 → hash 會變 → 在雜湊表裡「迷路」（放進去時算一個槽、內容變了算另一個槽，再也找不到）。
所以 Python **預設讓可變的內建型別不可 hash**：
