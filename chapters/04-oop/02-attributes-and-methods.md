...
大多數 Python 物件的屬性存在一個叫 **`__dict__` 的字典**裡（呼應「dict 是 Python 核心」，見 [dict](../03-data-structures/04-dict.md)）：
...
`self.x = 1` 本質是 `self.__dict__['x'] = 1`。因為屬性是動態的 dict，你能**在執行期任意增刪屬性**——這是 Python 的靈活性來源，也是它比靜態語言「鬆」的地方。
