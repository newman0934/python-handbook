...
上雲會踩的第一個坑,幾乎都跟 **IAM(身分與存取管理)** 有關——
不是「服務跑不起來」,就是「權限開太大,一個外洩全盤皆輸」。
IAM 是雲端安全的**地基**,也是最容易被新手忽略、被駭客最愛的破口。

再複雜的 IAM,都是一句話的排列組合:

```text
誰(Principal)  可以做什麼(Permission)  對哪個資源(Resource)
使用者/服務      讀/寫/刪除/呼叫          這個 bucket / 那個 DB
```

「這個服務**能不能**對**那個資源**做**這件事**」——所有授權都在回答這個問題。

貫穿全章的一條鐵律,是資安的核心原則——**最小權限(least privilege)**:
**只給剛好夠用的權限,一絲多的都不給**。
你的 task-api 只需要讀某個 bucket、連某個 DB?那就只給這兩個權限——
別圖省事掛一個「管理員」角色上去。因為萬一這個服務被[注入攻擊](../30-production-ai/05-prompt-injection-security.md)
攻破,攻擊者能造成的破壞,**上限就是這個服務的權限**——權限給越小,災難越小。

這章還有一個**極其重要、能省下無數事故**的觀念——**用角色(role),別用金鑰(key)**:
在雲上,讓「服務本身」擁有一個角色(雲自動給它臨時、會過期的憑證),
遠比「產生一把永久 API 金鑰塞進程式」安全——
因為金鑰會外洩、會忘記輪替([Part 20 secrets 章](../20-security-system-design/05-secrets-management.md)的老問題),
而角色沒有金鑰可洩。這個觀念會在 [ch09 的 OIDC](09-cicd-to-cloud.md) 大放異彩。
這章把 AWS 與 GCP 的 IAM 模型、role/policy 怎麼設講清楚——上雲的安全第一課。
