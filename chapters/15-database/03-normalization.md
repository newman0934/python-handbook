...
想像你的通訊錄這樣記:每則聊天記錄都抄一遍朋友的電話。

朋友換號碼了——你要改 500 處;漏改一處,通訊錄開始**自相矛盾**(同一人兩個號碼,哪個對?)。

這就是資料庫設計的萬惡之源:**冗餘**——同一個事實存了很多份。它衍生三種事故:

- **更新異常**:改不完、改漏了就矛盾。
- **插入異常**:想記「還沒聊過天的新朋友」——沒有聊天記錄就無處可放。
- **刪除異常**:刪掉唯一一則聊天,朋友的電話也跟著消失。

**正規化**就是解藥,核心只有一句:

> **每個事實只存一次,存在它該在的地方。**(電話存朋友表,聊天記錄只存「跟誰聊」的參照)

那些嚇人的術語(1NF、2NF、3NF)只是「拆到多乾淨」的等級,判斷口訣一句:非鍵欄位必須相依於「**鍵、整個鍵、且只有鍵**」。

最後的反轉:有時我們**故意**留冗餘(反正規化)——讀取太頻繁、JOIN 太貴時,用冗餘換速度。原則:**先正規化,有實測的效能理由再反著來**。
