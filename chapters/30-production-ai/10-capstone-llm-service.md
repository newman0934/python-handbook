...
這是 Part 30、也是整條 **AI 工程師學習線(Part 28–30)** 的**畢業專題**。
我們把這個 Part 的每一道防線,組裝成**一個生產級 LLM 服務的完整請求管線**——
一個真實請求進來,依序穿過每一道你學過的關卡:

```text
請求(使用者的問題)
  │
  ├─ 0. A/B 分流(ch08):這個使用者走哪個版本/模型?
  ├─ 1. 限流(ch03):token bucket 擋暴衝 → 超過就 429
  ├─ 2. 輸入護欄(ch05/06):偵測注入、遮蔽 PII → 有問題就攔
  ├─ 3. 預算檢查(ch03):今日花費超上限 → 拒絕或降級
  ├─ 4. 呼叫 LLM(ch02):串流生成,配重試/降級(ch03)
  ├─ 5. 輸出護欄(ch06):驗證格式、內容安全、忠實度 → 不合格就攔/改寫
  ├─ 6. 記錄(ch04):結構化 log + metrics + trace(token/成本/延遲)
  └─ 回應(附版本標記,供事後追溯與 A/B 分析)
       ↓
     線上收集回饋 → 資料飛輪(ch09)
```

這章的價值不是再學新東西,而是**看清這些防線如何協同成一個整體**——
每一道關卡各司其職,少一道,系統就多一個上線後會爆的破口。
你會**親手把它們串成一個乾淨的 FastAPI 服務**,
有完整的可觀測性、可靠性、安全性,以及支撐持續改進的資料迴路。

走完這章,你就掌握了**把 LLM 從 demo 帶到生產**的完整工程能力——
這正是當前市場最稀缺、最值錢的 AI 工程技能。
最後的 [Part 31](../31-cloud-platform-deployment/README.md)會把整個服務**真正部署上雲**,
為全書的實戰旅程畫下句點。
