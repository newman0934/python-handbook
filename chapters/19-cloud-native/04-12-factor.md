...
為什麼麥當勞能在任何城市快速開分店？因為有一套 **SOP**:
任何門市都用同一份標準流程、口味不靠某位老師傅的手感、供應商可以換、
店員不用記熟客的臉（會員資料在系統裡,不在店員腦中）。

**12-factor app** 就是讓你的應用能在任何雲端「開分店」的 SOP。
十二條裡最核心的幾條,翻成白話:

- **同一份藍圖**（Codebase）:dev/staging/prod 部署的是同一份程式碼,只差設定。
- **口味不寫死**（Config）:會隨環境變的值放**環境變數**——
  [設定管理章](../16-architecture/11-config-management.md)「衣服別紋在身上」的正式出處就是這裡。
- **供應商可換**（Backing services）:資料庫、快取、佇列都用連線字串指定,
  換一家只改 URL,程式不動。
- **店員不記熟客**（Stateless processes）:行程**無狀態**——session、上傳檔都放外部
  （Redis、物件儲存）,任何一個行程倒了、加了,服務照常。這是**水平擴展的前提**:
  無狀態,才能無腦加分店。
- **日誌往外倒**（Logs）:應用只管往 stdout 印,收集是平台的事——別自己寫檔案輪替。

這十二條不是教條,是「被雲端環境毒打過的人」總結的傷疤。
這章逐條講:每條防的是什麼事故、Python/FastAPI 專案怎麼落實。
