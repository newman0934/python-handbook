...
```pycon
>>> a = 100; b = 100
>>> a is b          # True ?!  —— 兩個變數,同一張身分證?
>>> a = 1000; b = 1000
>>> a is b          # False —— 換個數字又不是了??
```

謎底是一個優化——**interning（駐留）**,像口袋裡的**常用零錢**：

1 元、5 元、10 元硬幣天天用——與其每次交易都鑄一枚新幣,不如**預先鑄好一批,全民共用**。

CPython 對兩類「常用的不可變值」這麼做：

- **小整數 −5 到 256**:直譯器啟動時就鑄好,終身共用——所以 `100 is 100` 為 True（同一枚硬幣）,`1000` 不在範圍、每次新鑄——False。
- **部分字串**（長得像識別字的、編譯期字面值）:相同內容共用一份。

省記憶體、比較加速（比身分證字號比比內容快）——因為**不可變**,共用絕對安全（[Part 3](../03-data-structures/06-mutability.md):沒人能改硬幣的面額）。

但這章真正要你帶走的是一個**紀律**：

> 這些是**實作細節**——範圍會變、規則不保證、其他直譯器不一樣。
> **所以比較「值」永遠用 `==`,`is` 只留給 None**——[Part 2 的鐵則](../02-fundamentals/05-operators.md),現在你知道它防的就是這裡的陷阱:`is` 有時碰巧 True,讓錯誤寫法看起來能動,換個數字就炸。
...
**interning（駐留）**：「快取並重用某些不可變物件」的優化——常用零錢預先鑄好、全民共用。
與其每次都建新物件，不如讓所有「相同值」的引用指向**同一個**快取物件——省記憶體、加速比較。

CPython 做兩種 interning：

- **小整數快取**：**−5 到 256** 的整數在直譯器啟動時預先建好、永久存在。任何產生這範圍整數的操作都回傳**同一個**快取物件。
- **字串 interning**：某些字串（看起來像識別字的、字面值）被駐留，相同內容共用一個物件。

因為是快取重用，這些物件 `id` 相同 → `is` 為 True。

**但這是實作細節**——範圍外的值不保證、規則隨版本變。因此鐵則：比較值用 `==`，`is` 只用於 None。
