...
服務上線後,兩個問題會**立刻**找上門:
「**它現在好不好?**」(可觀測性)和「**這個月要繳多少?**」(成本)。
[Part 19](../19-cloud-native/08-observability.md) 講過可觀測性原理,
這章講它在**雲平台上**怎麼落地,並補上一個純雲端的新課題——**帳單控管**。

**可觀測性**:三支柱([logs/metrics/traces](../22-distributed-systems/08-distributed-tracing.md))
在雲上有現成工具(AWS **CloudWatch**、GCP **Cloud Monitoring/Logging**)——
你的容器日誌自動收集、指標自動繪圖、可設**告警**。
這章還帶入一個進階但關鍵的概念——**SLO(服務等級目標)**:
明確定義「可用性 99.9%」「p95 延遲 < 300ms」這種**可量化的健康承諾**,
並用**錯誤預算**管理(允許多少不達標),讓「服務好不好」有客觀標準,而不是憑感覺。

**成本**:這是上雲後最容易失控、也最容易被忽略的一塊——
雲帳單的恐怖故事(忘了關的資源、失控的自動擴縮、跨區流量費)每天都在上演。
這章給你一套**成本工程**:看懂帳單的成本模型(運算/儲存/流量/請求各怎麼算)、
設**預算告警**(花超過就通知,別等月底帳單嚇到)、
以及省錢招式(縮到零、選對實例、[快取](../18-performance/04-caching.md)降呼叫、
生命週期規則自動清理舊資料)。

一句話總結:**上線不是終點,而是「持續盯著它好不好、花多少」的開始**。
這章讓你的 task-api 部署,配上看得見的儀表板與管得住的帳單——
一個對得起「生產環境」四個字的收尾。
