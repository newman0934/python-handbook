...
照顧一群服務,要問的其實是**兩個不同的問題**,搞混會出大事:

- **「人還活著嗎?」＝ liveness（存活探針）**。
  答「否」的後果是**重啟容器**——這是急救電擊,對「卡死且自己好不了」的狀態
  (死鎖、event loop 卡住)才用。
- **「現在能接客嗎?」＝ readiness（就緒探針）**。
  答「否」的後果溫和得多:**暫時不排流量給它**(從 Service 摘掉),不重啟。
  適合「暫時不行」:還在啟動暖機、依賴的資料庫暫時斷線、正在優雅關閉排空。

最經典的事故就是把兩者搞混:**把「資料庫連不上」寫進 liveness**——
資料庫抖一下,K8s 判定全部 Pod「死了」,**整批重啟**,
一個小故障被放大成全面停機。記住鐵律:

> **liveness 要淺**(只看行程本身能不能回應,絕不檢查外部依賴);
> **readiness 可以深**(依賴掛了就先別接客,等它回來)。

再加一個 **startup probe**(啟動探針)照顧慢啟動的應用:
啟動期間先別用 liveness 電它,給它時間熱身。

這章用 FastAPI 實作 `/healthz`(淺)與 `/readyz`(深,檢查 DB/Redis)兩個端點、
對應的 K8s YAML、以及與[優雅關閉](../19-cloud-native/07-graceful-shutdown.md)的配合
(收到 SIGTERM 先把 readiness 轉紅,流量排空再退場)。
