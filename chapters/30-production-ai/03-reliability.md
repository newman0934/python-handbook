...
你的服務依賴一個**外部 LLM API**,而它**一定會出狀況**:
逾時、回 429(你打太快被限流)、偶發 500、某個區域整個抖動。
如果你天真地「呼叫、拿結果、繼續」,那上游一打噴嚏,你的服務就跟著感冒——
使用者看到錯誤、請求卡死。這章講怎麼讓服務在**不可靠的依賴之上**依然穩健。

其實這整套「呼叫護具」你在 [Part 21 微服務](../21-microservices/07-rate-limit-circuit-breaker.md)
和 [Part 22 分散式](../22-distributed-systems/README.md)都學過了——
這章是把它們**套用到 LLM 場景**。四道防線,層層疊上:

1. **重試 + 指數退避**:暫時性失敗(429/逾時)常常「再試一次就好」。
   但**立刻重試會火上加油**(大家同時重打,壅塞更嚴重),所以等待時間要**倍增**
   (1s→2s→4s),再加**隨機抖動**打散重試洪峰。**且只重試該重試的錯**(5xx/429),
   別重試 400/401(那是你自己的錯,重試一萬次也一樣)。
2. **超時 + [熔斷](../21-microservices/07-rate-limit-circuit-breaker.md)**:
   上游明顯掛了就**跳閘快速失敗**,別讓請求全卡在等待上拖垮自己。
3. **降級(fallback)**:主模型不行,退而求其次——
   換備用模型、回快取答案、或給一個誠實的「暫時無法服務」,而不是直接 500。
4. **限流 + 預算控制**:用 [token bucket](../20-security-system-design/11-system-design-rate-limiter.md)
   擋暴衝,並盯 **token 花費上限**——LLM 特有的,一個失控迴圈能燒出天價帳單。

這章用 Python 把四道防線實作成一個可靠的 LLM 呼叫封裝——
**能動的 demo 誰都會寫,扛得住上游抽風的服務才是工程**。
