...
切片就是「**從序列剪一段下來**」：`a[start:stop:step]`——從哪開始、到哪為止、每幾個取一個。

最重要的規則只有一條，先用尺的刻度想：

> **含頭不含尾**：`a[2:5]` 取索引 2、3、4——**不含 5**。

為什麼這樣設計？三個漂亮的結果：

- 長度一目瞭然：`a[2:5]` 就是 `5 - 2 = 3` 個元素。
- 無縫拼接：`a[:i] + a[i:]` 剛好等於原序列——在 i 處切開，不重複不遺漏。
- 「取前 n 個」就是 `a[:n]`，乾淨。
...
```python
a[:3]      # 前三個（start 省略 = 從頭）
a[3:]      # 第三個之後全部（stop 省略 = 到尾）
a[-2:]     # 最後兩個（負索引 = 從後面數）
a[::2]     # 每隔一個取一個
a[::-1]    # 整個反轉（負步長 = 倒著走）
a[2:999]   # 不會報錯！超出的部分自動截斷（跟單一索引 a[999] 會爆不同）
```

還有一個進階但好用的：**切片出現在等號左邊**（`a[1:3] = [9, 9, 9]`）是「把那一段換掉」——原地手術。
...
切片取序列的一段，語法 `seq[start:stop:step]`。核心規則：

- **半開區間 `[start, stop)`**——含頭不含尾。`a[2:5]` 取索引 2、3、4，共 `5 - 2 = 3` 個。
- **三個參數都可省略**：預設 start=0、stop=長度、step=1。
- **不會越界報錯**：`a[2:999]` 自動截到結尾——這和單一索引 `a[999]`（IndexError）不同。

「半開區間」的甜頭（導讀提過，值得再看一次）：

- `a[:i] + a[i:] == a`——在 i 處剛好切開，不重不漏。
- 長度直接是 `stop - start`。
