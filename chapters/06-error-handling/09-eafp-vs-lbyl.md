...
- **LBYL（Look Before You Leap，三思而後行）**：**先檢查前置條件**，確認 OK 才執行操作。用一堆 `if` 檢查。
- **EAFP（Easier to Ask Forgiveness than Permission，先斬後奏）**：**直接執行操作**，假設會成功，用 `try/except` 處理失敗。

Python 偏好 EAFP，源於它的哲學：例外是常態、廉價；而且 EAFP 避免了「檢查」與「操作」之間的**競態條件（race condition）**——LBYL 在檢查通過後、實際操作前，狀態可能已改變。
