...
1. **重現（reproduce）**：先找到「穩定重現」bug 的最小步驟/輸入。無法重現就無法可靠地修（也無法驗證修好了）。[測試](01-why-testing.md)是重現 bug 的最佳工具——寫一個會失敗的測試。
2. **定位（localize）**：縮小 bug 的範圍。用**二分法（bisection）**——在中間點檢查狀態，確認 bug 在前半還是後半，反覆折半，快速逼近出錯處。
3. **假設（hypothesize）**：對「為什麼錯」提出具體假設。
4. **驗證（verify）**：用除錯器/log 檢查，**驗證或推翻**假設——不是「改改看」，是「證明」。
5. **修正 + 驗證**：修根因（非症狀），並確認修好（讓失敗的測試通過、且沒引入迴歸）。

**兩種除錯手段的取捨**：

- **互動除錯器（pdb/IDE debugger）**：暫停程式、逐行走、即時檢視/修改變數。**適合複雜的邏輯 bug**——你需要「看到程式當下的狀態」。強大但需要能在本機重現。
- **log / print 除錯**：在關鍵處印出狀態。**適合難以互動的場景**——正式環境、時序相關、多執行緒/非同步、偶發問題。侵入性、要事後看，但能捕捉「當下發生了什麼」（見 [可觀測性](../19-cloud-native/08-observability.md)）。

實務兩者並用：本機複雜 bug 用除錯器、正式環境/偶發問題用結構化 log。
