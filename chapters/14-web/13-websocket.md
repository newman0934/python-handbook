...
**WebSocket** 與 HTTP 的根本差異：

| | HTTP | WebSocket |
|--|------|-----------|
| 模式 | 請求-回應（單向） | 全雙工（雙向） |
| 連線 | 短暫（一問一答就斷） | 持久（建立後保持） |
| 推送 | ❌（客戶端要輪詢） | ✅（伺服器主動推） |
| 適合 | 一般 API | 即時（聊天/通知/串流） |

WebSocket 連線流程：**HTTP 握手升級成 WebSocket → 持久連線建立 → 雙方隨時收發訊息 → 直到一方關閉**。因為連線持久且雙向，伺服器能主動推送——這是「即時」的關鍵。WebSocket 需要 ASGI 的非同步能力（管理大量長連線，見 [asyncio](../09-concurrency/07-asyncio-basics.md)）。
