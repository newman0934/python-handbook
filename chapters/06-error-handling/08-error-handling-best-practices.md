...
前面學了工具，這章是**職業守則**——五條鐵律，每條背後都是無數次生產事故換來的：

**1. 只接你「知道怎麼處理」的例外。**
`except ValueError:` 而不是 `except:`。裸 except 像「不管什麼警報一律按掉」——連火災和 Ctrl-C 都被你按掉了。

**2. 接到就要「做點什麼」——絕不吞錯。**
`except: pass` 是全 Python 最危險的兩行：錯誤發生了、然後**被滅口**。之後系統行為詭異，你連線索都沒有。至少要記 log。

**3. 在「能做決策」的層級處理。**
底層函式接到錯往往不知道該怎麼辦（重試？告訴使用者？）——那就**別接，讓它往上傳**（警報的傳播機制就是為此設計的），傳到知道怎麼辦的那層再接。

**4. 讓 bug 崩潰。**
「不該發生」的程式邏輯錯誤（例如狀態不可能為負卻負了）——**別接住掩蓋**。讓它崩、印 traceback,才修得掉。接住 bug 等於把病人臉上的痛苦表情蓋住。

**5. 用 logging,別用 print。**
print 沒時間、沒等級、沒 traceback、上線後看不到;[logging](../11-stdlib/08-logging.md) 全都有。

一句總綱：**錯誤處理的目標不是「讓錯誤消失」,是「讓錯誤被誠實地面對」。**
...
1. **精確捕捉**：接你**知道怎麼處理**的特定例外，別接一切（裸 `except:` 連 Ctrl-C 都吞）。
2. **不吞錯**：捕捉到就要**做點什麼**（處理、記錄、重拋）——`except: pass` 是把錯誤滅口。
3. **在對的層級處理**：在「能做出有意義決策」的地方處理；下層不知道怎麼辦就**讓它傳播**（見[例外概論](01-exceptions.md)）。
