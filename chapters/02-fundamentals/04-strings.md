...
理解 Python 字串只需要一個畫面：**字串是刻在石板上的字**。

刻好就不能改——想「修改」？只能**刻一塊新石板**。

這一個事實解釋了字串的所有行為：

- `s.upper()` 為什麼不會改變 `s`？——它刻了一塊全大寫的**新石板**回傳給你，原石板動都沒動。（忘了接回傳值 = 新石板直接丟了，最常見的錯。）
- 為什麼迴圈裡 `s += x` 是效能陷阱？——每次 `+=` 都是「把舊石板整塊重刻＋加新字」。一萬次迴圈就是重刻一萬塊石板。正解：把碎片收進 list，最後 `"".join()` 一次刻。

另外兩個本質，一句話帶過：

- 石板上刻的是 **Unicode 文字**（`str` 管文字；位元組是另一型別 `bytes`，[第 16 章](16-encoding-bytes.md)分曉）。
- 石板是**有序序列**——可以索引、切片、用 `for` 逐字走。

格式化就記一個：**f-string**（`f"你好，{name}"`）——現代唯一推薦寫法，其餘（`%`、`.format()`）是歷史。

## 🔗 前端對照

Python 的 f-string 對應 JavaScript 的**樣板字面值（template literal）**——都是把變數直接嵌進字串:

| 目的 | Python | JavaScript |
|------|--------|-----------|
| 內插變數 | `f"Hi {name}"` | `` `Hi ${name}` `` |
| 內嵌運算式 | `f"{a + b}"` | `` `${a + b}` `` |
| 多行字串 | `"""..."""` | 反引號 `` `...` `` |
| 格式化 | `f"{x:.2f}"`（內建格式規格） | 得用 `x.toFixed(2)` |

一句話:內插概念一樣（`{}` ≈ `${}`）。但 Python 的 f-string 還內建**格式規格**（`:.2f`、`:>10`、`:,`）,
比樣板字面值多了一整套排版能力,不用另外呼叫方法。
...
1. **不可變（immutable）**——刻好的石板。字串一旦建立就不能改內容；所有「修改」方法（`upper`、`replace`⋯⋯）都**回傳新字串**，原字串不動。
2. **Unicode 文字**：`str` 存的是文字（字元），不是位元組。位元組是獨立的 `bytes` 型別（見 [Python 2 vs 3](../01-getting-started/10-python2-vs-3.md)、[編碼與 bytes](16-encoding-bytes.md)）。
3. **序列（sequence）**：字元的有序序列——支援索引、切片、迭代、`len`（見[切片](../03-data-structures/03-slicing.md)）。

「不可變」是理解一切的鑰匙：

- 因為不可變，`s.upper()` 不會改 `s`——回傳新石板。
- 因為不可變，迴圈裡 `s += x` 每次都造新字串——重刻整塊石板的效能陷阱。
