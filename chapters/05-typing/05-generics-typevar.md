...
**`TypeVar`** 是一個「型別的變數」——代表「某個尚未確定、但前後一致的型別」。它讓你在函式或類別的簽章裡表達型別之間的**關聯**：
...
當你呼叫 `first([1, 2, 3])`，mypy **推斷** T = int，於是回傳型別是 int；`first(["a"])` 則 T = str。**同一個 T 在簽章中出現多次，代表它們必須是同一型別**——這就是泛型保留的關聯。

對比 `Any`：`list[Any] -> Any` 完全丟失關聯（回傳當 Any）；`list[T] -> T` 保留了「回傳型別 = 元素型別」。
