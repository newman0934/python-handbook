...
- 專案 A 是三年前的老系統，要用 requests **2.x** 版。
- 專案 B 是新專案，要用 requests **3.x** 版。

如果全電腦只有一個「收貨地址」（site-packages），這兩個需求**直接打架**——裝了新版，舊專案壞；裝回舊版，新專案壞。

**虛擬環境（venv）** 的解法很直白：**每個專案配一個自己的獨立房間**。

一個虛擬環境其實就是一個資料夾，裡面有：自己的 `python` 捷徑 + **自己專屬的 site-packages**（自己的收貨地址）。
專案 A 的房間裝 2.x、專案 B 的房間裝 3.x——互不往來，天下太平。

「啟用（activate）」聽起來玄，原理只是一句話：**把這個房間的路徑插到系統搜尋路徑的最前面**。
於是你打 `python`、`pip` 時，最先被找到的就是房間裡的那份——`pip install` 自然也裝進房間裡。

三個常用動作先給你：

```bash
python -m venv .venv          # 蓋房間（專案目錄下慣例叫 .venv）
.venv\Scripts\activate        # 進房間（Windows；mac/Linux 用 source .venv/bin/activate）
deactivate                    # 出房間
```

不想要了？**整個資料夾刪掉就沒了**，乾乾淨淨。

最後一句重話：venv 不是「進階技巧」——**每個專案開頭先建 venv**，是專業 Python 開發的最低門檻。

## 🔗 前端對照

venv 解決的問題,前端用 `node_modules` 解決——都是**把每個專案的相依隔離開**,不讓 A 專案的套件版本干擾 B:

| | Python venv | 前端 |
|---|-------------|------|
| 隔離單位 | 一個虛擬環境（`.venv/`） | 每個專案的 `node_modules/` |
| 是否預設隔離 | **否**,要自己建 venv 並啟用 | **是**,`npm install` 天生裝在本地 |
| 全域安裝 | `pip install`（不建議） | `npm install -g` |

一句話:前端「開新專案就有 `node_modules`」是自動的;**Python 要你手動建 venv 並啟用**——
少了這一步,套件會裝到系統或別的專案,這是新手最常見的環境地雷。
...
一個虛擬環境本質上就是一個**資料夾**（那個「獨立房間」），裡面有：

1. 一個指向某個 Python 直譯器的連結（或副本）。
2. **它自己專屬的 site-packages**（一開始幾乎是空的）——自己的收貨地址。
3. 啟用/停用用的腳本。

「隔離」的原理很單純：

> **啟用（activate）= 把這個環境的路徑塞到 PATH 最前面。**

於是你打 `python` 和 `pip`，用到的就是這個環境裡的版本；`pip install` 也就裝進這個環境專屬的 site-packages。
不同專案各有各的環境資料夾，套件互不干擾。刪掉資料夾，環境就乾乾淨淨地消失。

`venv` 是 Python **標準庫內建**的虛擬環境工具（3.3+），不需額外安裝。
