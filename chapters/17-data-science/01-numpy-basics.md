...
先回想 [物件模型](../10-cpython-internals/README.md) 講過的事：Python 的每個數字都是一個「盒子」
（完整的 PyObject，帶型別資訊、引用計數），`list` 存的只是指向這些盒子的**紙條（指標）**，
盒子本身散落在記憶體各處。存一百萬個數字＝一百萬個盒子＋一百萬張紙條。

numpy 的 `ndarray` 做了一件狠事：**把盒子全拆了，只留裸數字，排進一條連續的蛋盒**。

- **同質（homogeneous）**：每格同尺寸（同一個 `dtype`，如每格 8 bytes 的 `int64`）——
  所以第 i 格的位置用乘法就能算出來，不用逐一翻找。
- **連續記憶體**：整排數字肩並肩躺在一起——CPU 快取一次載入一整段，掃起來飛快。

這兩個特性就是 numpy 快的全部根源，也是它整套行為的解釋：

- 為什麼**不能混型別**？蛋盒每格同尺寸，塞不下不同大小的東西。
- 為什麼 **`a + 1` 能一次加到全部**？C 層對連續記憶體跑迴圈，不經過 Python 直譯器。
- 為什麼**改大小要建新陣列**？連續區塊沒有「中間插一格」這種事。

pandas、scikit-learn、PyTorch 底下全是這套蛋盒。這章把 `ndarray` 的三要素
（dtype、shape、stride）與基本操作講清楚——地基打好，上層全部順理成章。
