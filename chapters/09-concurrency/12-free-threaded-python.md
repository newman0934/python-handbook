...
**free-threaded Python（無 GIL 建置）** 移除了全域直譯器鎖，於是：

- **執行緒能真正並行執行 Python bytecode**——多執行緒跑 CPU 密集任務終於能利用多核心，不必再用 multiprocessing。
- 保留 threading 的優勢：**共享記憶體**（不必像行程那樣序列化資料傳遞）。

這解決了 Python 並發最大的痛點：以前「要真並行只能 multiprocessing（獨立記憶體、pickle 開銷）」，未來「threading 就能真並行 + 共享記憶體」。

但移除 GIL 不是免費的（見下）——GIL 當初存在是有原因的（簡化記憶體管理、加快單執行緒、方便 C 擴充），移除它需要重新設計這些，且有取捨。
