...
[Part 9](../09-concurrency/02-gil.md) 用「全廚房一把刀」講了 GIL 的**影響**;這章回到刀的鑄造現場,講**它為什麼非存在不可**。

線索在[第 3 章的計數牌](03-reference-counting.md)。每個物件掛著「幾張便利貼貼著我」的計數,而 **+1 這個動作其實是三小步**：

```text
讀出目前的數(比如 7)→ 加一(8)→ 寫回去
```

現在兩條執行緒**同時**對同一個物件 +1：

```text
執行緒 A:讀到 7          執行緒 B:也讀到 7
A:算出 8                 B:算出 8
A:寫回 8                 B:寫回 8    ← 兩次 +1,結果只加了 1!
```

計數被記壞的後果是災難級的:數字偏小 → 物件**被提早回收**,別人還貼著它 → 記憶體損毀、程式神祕崩潰;數字偏大 → 永不回收 → 洩漏。

怎麼防?兩個選項:

- **每個物件的計數牌配一把小鎖**——安全,但 Python 物件多如牛毛,鎖的開銷讓單執行緒都變慢。
- **整個直譯器一把大鎖**:同一時刻只有一條執行緒能跑 bytecode——計數天下太平,單執行緒零額外開銷,C 擴充也好寫。

CPython 選了後者。**這把大鎖就是 GIL**——它不是針對「你的程式碼」,是在保護「直譯器自己的帳本」。

理解這個因果,你就懂為什麼[拆掉它（free-threaded）](../09-concurrency/12-free-threaded-python.md)這麼難:拆刀,就得回頭給每本帳配鎖——正是當年不選的那條路,如今用更聰明的方式重走。
...
它存在的**根本原因是保護引用計數**（見[引用計數](03-reference-counting.md)的計數牌）：

- 每個物件有引用計數 `ob_refcnt`，賦值/傳遞時 +1、消失時 −1。
- 這個 +1/−1 是**非原子操作**（讀-改-寫三步）。兩個執行緒同時對同一物件增減，會產生**競態**——計數算錯 → 物件被過早回收（記憶體損毀）或永不回收（洩漏）。
- **最簡單的解法**：一把大鎖（GIL）保證「一次只有一個執行緒動任何物件」——引用計數就不必為每個物件加細粒度鎖。

所以 GIL 是**設計取捨**：

> 用「犧牲多執行緒並行」換「引用計數簡單安全 + 單執行緒高效 + C 擴充好寫」。
