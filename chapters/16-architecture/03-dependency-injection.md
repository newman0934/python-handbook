...
Python 是**鴨子型別**——注入的依賴不一定要繼承抽象基底類別，只要有對的方法就行（見 [Protocol](../05-typing/09-protocol.md)）。用 `Protocol` 定義結構型介面，比 ABC 更 Pythonic：
...
- **依賴抽象（Protocol/ABC）而非具體**（見 [SOLID](05-solid.md) DIP、[Protocol](../05-typing/09-protocol.md)）：可替換、可 mock。
