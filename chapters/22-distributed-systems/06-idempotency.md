...
**電梯按鈕**:按一次和按十次,電梯都只來一趟——這就是**冪等(idempotent)**。
**「再存 100 元」**:按十次就存了一千——**不冪等**,而且危險。

為什麼分散式系統必須關心這件事?因為**重複執行無法避免**:
網路逾時了,客戶端不知道「請求到底成功沒」,只能重試;
訊息佇列的[至少一次投遞](04-message-queue.md)本來就會重複。
你擋不住「同一操作被執行多次」,只能讓「多次執行的效果等於一次」。

先學會分辨天生體質:

- **天生冪等**:讀取(GET)、設絕對值(`set balance = 100`)、刪除(刪已刪的東西結果一樣)、
  PUT 全量替換。
- **天生不冪等(危險名單)**:相對變化(`balance -= 100`)、
  追加(INSERT/POST)、發送(寄信、推播)。

不冪等的操作怎麼救?萬用解法是**唯一單號(idempotency key)**:
客戶端為每筆操作生成唯一鍵(如 UUID),隨請求送出;
服務端**記住處理過的單號**——同單號再來,不重做,
**直接回上次的結果**(注意:是回存好的結果,不是回錯誤)。
Stripe 等支付 API 的 `Idempotency-Key` header 就是業界範本。

實作細節這章逐一講:單號存哪(DB 唯一索引最穩、Redis 要想 TTL)、
「檢查+執行+記錄」要在**同一個交易**裡(否則競態)、
以及與[重試](../21-microservices/07-rate-limit-circuit-breaker.md)的關係——
**冪等是重試的通行證:沒有冪等,重試就是賭博**。
