...
線性回歸是所有 ML 的**第一課**,不是因為它最強,而是因為它**最好懂**——
懂了它,後面所有模型的核心套路(用資料調參數、定義誤差、最小化誤差)你就都懂了。

它在做的事,國中就學過——**找一條最貼近點的線**:

```text
房價 ≈ w₁×坪數 + w₂×屋齡 + w₃×距捷運 + b
```

每個特徵配一個**權重 w**(這特徵多重要、正向還負向),
加起來再加個底(截距 b)。**學習**就是找出最好的那組 w 和 b。

「最好」怎麼定義?看**誤差**:預測房價和真實房價差多少。
把每筆的誤差平方(讓正負不抵消、大錯罰更重)、取平均——這叫 **MSE(均方誤差)**。
於是學習變成一道數學題:**找 w、b 讓 MSE 最小**。

怎麼找到最小?兩條路這章都講:
**正規方程**(直接用公式一步解出,小資料適用)、
**梯度下降**(像蒙眼下山,每步往最陡的方向走一點,大資料與深度學習的通用解法——
這是理解神經網路訓練的地基,[Part 27](../27-deep-learning/README.md) 會重逢)。

最後一個讓它歷久不衰的優點:**可解釋**。
權重直接告訴你「坪數每增一坪,房價漲多少」——
在需要向老闆/主管解釋「為什麼這樣預測」的商業場景,這種透明度無可取代。
這章也講它的前提假設與 R² 等評估指標。
