...
**弱引用** 是一種特殊引用：它讓你能存取一個物件，但**不增加該物件的引用計數**（見 [引用計數](03-reference-counting.md)）。後果：

- 若一個物件**只剩弱引用**指向它（沒有任何強引用），它會被**正常回收**。
- 回收後，弱引用會**自動失效**（呼叫它回傳 `None`，或觸發回呼）。

對比：
- **強引用（一般引用）**：`x = obj` → 引用計數 +1 → 阻止回收。
- **弱引用**：`ref = weakref.ref(obj)` → 引用計數**不變** → 不阻止回收。

弱引用讓你「觀察但不擁有」——這在「不想因為參照而延長物件壽命」的場景很有用。
