...
上一章說「ML 要的是泛化」。問題來了:你怎麼**知道**模型泛化得好不好?
拿它**做過的練習題**考它——當然滿分,但這證明不了它會解新題。
這是初學者最致命、最常犯的錯:**用訓練資料評估模型**。

所以 ML 工作流的第一鐵律:**資料一進門,先切三份、把測試集鎖進保險箱**:

- **訓練集(train)**＝上課教材:模型看著它學。
- **驗證集(validation)**＝模擬考:調參數、選模型時用它評分。
- **測試集(test)**＝正式大考:**全程不准碰**,只在最後開封一次,
  給出「模型真實實力」的誠實估計。偷看測試集調參,等於考前洩題,分數全是假的。

為什麼驗證集要獨立於測試集?因為你反覆用某份資料調參,就會慢慢「調到它身上」——
所以拿來調參的(validation)和最終評分的(test)必須分開。
資料不夠切三份時,用**交叉驗證(cross-validation)**:把訓練集輪流當模擬考,
用滿每一筆資料又不污染測試集。

另一個新手陷阱這章會反覆強調——**資料洩漏(data leakage)**:
前處理(如標準化的平均值)**只能在訓練集上學**,再套用到驗證/測試集;
若先算全體平均再切分,測試集的資訊就「洩」進了訓練——分數虛高,上線現形。
記住這句:**任何「從資料學到的東西」,都只能從訓練集學。**
