...
前面幾章你**堅持手刻**了 RAG、agent、記憶、工具循環——
不是因為框架不好,而是**先懂原理,用框架時才不會變成只會抄範例的黑盒使用者**。
現在地基穩了,這章來認識那些幫你省事的框架(**LangChain、LlamaIndex、LangGraph** 等)。

框架的價值,是把你手刻過的東西變成**現成積木**:

- **統一模型介面**:換 Claude/GPT/開源模型,只改一行,業務碼不動。
- **Prompt 模板**:把 [prompt engineering](../28-llm-genai/03-prompt-engineering.md) 的骨架參數化。
- **可組合管線(chain)**:把「prompt → 模型 → 解析 → 下一步」像樂高串起來。
- **現成的 RAG / agent / 記憶元件**:你手刻的那些,框架都有打包好的版本。

但框架是**雙面刃**,這章會誠實地講取捨:

- **好處**:快速起步、生態豐富(現成的載入器、向量庫接口、工具)。
- **代價**:**抽象層太厚**——出 bug 時你得剝開好幾層才找得到問題;
  版本迭代快、有時文件跟不上;過度封裝反而讓簡單的事變複雜。

**選用原則**:簡單需求(單純呼叫 + 一點 RAG),**手刻或薄框架**反而更清爽、
更好維護;複雜的多步 agent、要視覺化流程,重量級框架(如 LangGraph)才划算。
關鍵是——**因為你手刻過,你才有能力判斷框架幫你做了什麼、值不值得**。
這章帶你用框架重寫前面的 RAG/agent,對照手刻版,親眼看清「省了什麼、藏了什麼」。
