...
LLM 預設回**一段自然語言**,但你的程式要的是**能用的資料**或**能執行的動作**。
這章講兩個把「聊天機器人」變成「應用元件」的關鍵能力。

**1. 結構化輸出——逼模型回乾淨的 JSON。**
你想從一段履歷抽出「姓名、年資、技能」,
總不能對著模型的一段散文自己用正則慢慢挖。
給它一個 **JSON schema**(「我要這些欄位、這些型別」),
模型就回**保證可解析**的 JSON,程式直接拿去用。
資訊抽取、分類、把非結構化文字轉成結構化資料,全靠它。

**2. Tool use(工具呼叫)——讓模型會「動手」。**
模型本身只會生成文字,不會查資料庫、不會算數學、不知道今天天氣。
Tool use 給它一雙手:你**描述**幾個工具(名字、功能、參數),
模型**遇到需要時,回一個「請幫我呼叫 `get_weather(台北)`」的請求**——
注意:**模型不自己執行,是請你的程式執行**,再把結果餵回去,它接續作答。
這是一個**多步循環**:模型要求呼叫 → 你執行 → 回傳結果 → 模型繼續。

這個「模型會判斷何時該用工具」的能力,是所有 **AI agent** 的基石——
[Part 29](../29-ai-applications/README.md) 的 agent 就是把這個循環自動化、串起多個工具。
這章用 Anthropic SDK 實作結構化輸出與一個完整的 tool use 循環,
把 LLM 從「會說」升級成「會做」。
