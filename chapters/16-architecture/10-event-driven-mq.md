...
下單成功後要：寄確認信、扣庫存、通知物流。兩種做法：

**打電話一一通知**（同步呼叫）：下單流程裡依序呼叫三個服務,全部做完才回應使用者。
慢（使用者等所有事做完）、脆（寄信服務掛了,連下單都失敗）、緊耦合（多一個需求就改下單程式碼）。

**在公告欄貼一張紙**（事件驅動）：下單只做一件事——貼出「訂單 #123 成立」，立刻回應使用者。
寄信組、庫存組、物流組**自己訂閱公告欄**,看到新公告各自處理。
下單方不知道、也不需要知道有誰在看公告——**新增一個訂閱者，下單程式碼一行都不用改**。

那張「公告欄」就是**訊息佇列（message queue / broker）**，它有兩種經營模式：

- **工作佇列**（task queue）：一張任務單**只給一個** worker 做（像取號叫號，分工不重工）。
  典型：Celery + RabbitMQ 跑背景任務。
- **發布訂閱**（pub/sub）：一則事件**廣播給所有**訂閱者各自處理。典型：Kafka。

代價也要先講明：事件是**最終一致**（貼出公告到有人處理有時間差）、
訊息可能重送（消費者要**冪等**——同一則處理兩次結果不變）、除錯要跨服務追蹤。
這章把模式、取捨、與 Python 實作（含一個可執行的 in-process 事件匯流排）講清楚。
