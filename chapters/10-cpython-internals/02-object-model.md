...
上一章說每個物件都有「戶口」。這章看戶口上的**三個欄位**：

| 欄位 | 白話 | 怎麼查 | 誰在比較它 |
|------|------|--------|-----------|
| **id（身分）** | 身分證字號——「你是哪一個」 | `id(obj)` | **`is`** |
| **type（型別）** | 類別欄——「你是哪一種」 | `type(obj)` | `isinstance` |
| **value（值）** | 內容——「你現在長怎樣」 | 物件本身 | **`==`** |

[Part 2 埋的伏筆](../02-fundamentals/05-operators.md)在這裡收割——**`is` vs `==` 的真相**：

- `==` 比**內容**：「你們長得一樣嗎？」——雙胞胎回答「一樣」。
- `is` 比**身分證字號**：「你們是同一個人嗎？」——雙胞胎回答「不是」。

```pycon
>>> a = [1, 2]; b = [1, 2]
>>> a == b       # True —— 內容相同(雙胞胎)
>>> a is b       # False —— 兩個不同的物件(兩張身分證)
```

再記兩條戶籍法：

1. **id 和 type 終身不變**——一個物件不會中途換身分、換型別;**會變的只有 value**（而且僅限[可變物件](../03-data-structures/06-mutability.md)）。
2. CPython 裡 `id` 其實就是**記憶體位址**——這是實作細節,知道即可,別依賴。

這三要素是後面所有底層機制的座標系——之後講回收、interning,全用它們描述。
...
CPython 的每個物件都有三個核心屬性——戶口上的三個欄位：

| 屬性 | 是什麼 | 如何取得 | 相關操作 |
|------|--------|----------|----------|
| **id（身分）** | 物件的唯一識別（CPython 是記憶體位址）——身分證字號 | `id(obj)` | `is`（比較身分） |
| **type（型別）** | 物件屬於哪個類別——類別欄 | `type(obj)` | `isinstance` / `type() ==` |
| **value（值）** | 物件的內容 | 物件本身 | `==`（比較值）、可變性 |

戶籍法兩條：

- **id 與 type 在物件生命週期內不變**——不會中途換身分或型別。
- **value 可能變**——僅限可變物件（見[可變性](../03-data-structures/06-mutability.md)）。

`is` 比身分證（同一個人嗎）、`==` 比內容（長得一樣嗎）——雙胞胎：`==` True、`is` False。
