...
服務之間每秒對話上萬次,用 JSON 溝通就像**每封信都把稱謂寫好寫滿**:
`{"user_id": 42, "user_name": "alice", ...}`——欄位名每次都重複傳,
又是文字格式,解析又慢。內部高頻通訊,值得換一套更精的辦法。

**Protocol Buffers ＝ 事先講好格式的電報**。
雙方先共用一份**表格範本**（`.proto` 檔）:「第 1 欄是 id（整數）、第 2 欄是 name（字串）」。
之後傳輸**只送欄位編號＋值**,二進位編碼——不送欄位名、沒有引號逗號,
體積常只有 JSON 的 1/3~1/10,解析快一個量級。

範本還有一個更重要的身分:**跨語言的合約**。
`protoc` 編譯器吃 `.proto`,吐出 Python/Go/Java 的強型別類別——
訂單服務（Python）和庫存服務（Go）共用同一份合約,欄位型別對不上**編譯期就知道**,
而不是上線後炸出 KeyError。

**gRPC** 則是搭配的通話方式:跑在 HTTP/2 上（一條連線多路併發、可雙向串流）,
`.proto` 裡直接定義服務方法（`rpc GetUser(...)`）,呼叫遠端服務像呼叫本地函式。

什麼時候用它、什麼時候仍用 REST/JSON?口訣:**對外用 REST（通用、好除錯、瀏覽器直接吃）,
內部高頻用 gRPC（快、強型別、有合約）**。這章從 `.proto` 寫起,
生成 Python 程式碼、跑起一對 client/server,並講欄位編號的演進規則（為什麼不能改號)。
