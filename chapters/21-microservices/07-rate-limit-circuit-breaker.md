...
家裡電線走火,保險絲**跳閘**——犧牲一個迴路,保住整棟房子。
微服務裡沒有保險絲的樣子是:庫存服務掛了,訂單服務的每個請求
都**傻等 30 秒逾時**,執行緒全卡在等;上游的 API gateway 又等訂單服務……
一個服務的故障沿呼叫鏈**雪崩**,全系統癱瘓——大家不是被故障打死的,
是被「等待故障」拖死的。

**熔斷器（circuit breaker）** 就是裝在呼叫方的保險絲,一個三態狀態機:

- **Closed（正常通電）**:請求照常放行,同時默默統計失敗率。
- **Open（跳閘）**:失敗率超標,**之後的請求連送都不送、立刻失敗**——
  快速失敗換來兩件事:自己的執行緒不再陪葬;下游得到喘息空間恢復。
- **Half-Open（試探）**:跳閘一段時間後,放**少量**請求試水溫——
  成功→復電（Closed）;失敗→繼續跳閘（Open）,重新計時。

熔斷之外,這章把整套「呼叫護具」講齊,它們是疊著穿的:
**timeout**（永遠要設,不設就是無限等）→ **retry**（配指數退避＋抖動,
且只重試冪等操作——否則重試出兩筆訂單）→ **circuit breaker**（重試也救不了時跳閘）→
**fallback**（跳閘期間給降級回應:推薦服務掛了就回熱門榜,別回 500）。

用 Python 實作一個可測試的熔斷器狀態機,是本章的主菜——
面試同樣高頻:「講講熔斷器三態與為什麼需要 Half-Open」。
