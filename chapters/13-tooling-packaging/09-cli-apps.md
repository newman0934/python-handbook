...
- **位置參數（positional）**：必要的、依順序的參數（`greet Alice` 的 `Alice`）。
- **選項 / 旗標（option / flag）**：`--count 3`（帶值）、`--verbose`（布林旗標）。通常選填、有預設值。
- **子命令（subcommand）**：把一個工具分成多個動作（`git commit`、`git push`）——每個子命令有自己的參數。
- **`--help` / 使用說明**：自動產生，讓使用者知道怎麼用。
- **退出碼（exit code）**：成功回 0、失敗回非 0（讓 shell/CI 能判斷，見 [os/sys](../11-stdlib/01-os-sys.md)）。

**三個工具的定位**：

- **`argparse`（標準庫）**：無需安裝、功能完整、穩定。用**命令式**風格建 parser（`add_argument`）。稍嫌囉嗦，但零依賴、任何環境都能用——**簡單工具、不想加依賴時的首選**。
- **`click`（第三方）**：用**裝飾器**定義命令與選項，更宣告式、更少樣板、錯誤處理與說明更漂亮，生態豐富（進度條、顏色、prompt）。**複雜 CLI 的主流選擇**。
- **`typer`（第三方，基於 click）**：用 **Python 型別註解** 定義參數（型別即驗證），最少樣板、最現代，自動從型別產生說明與驗證。**新專案的推薦**（尤其你熟悉 FastAPI 的話）。

**選型**：零依賴/簡單 → `argparse`；功能豐富/複雜 → `click`；型別優先/現代 → `typer`。
