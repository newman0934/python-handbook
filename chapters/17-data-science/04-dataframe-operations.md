...
拿到一疊發票，老闆問「每個城市各花了多少？」你會怎麼做？

1. 把發票**按城市分成幾堆**（split）
2. **每堆各自加總**（apply）
3. 把各堆結果**抄回一張總表**（combine）

這就是 `df.groupby("city")["amount"].sum()` 做的事——
**split-apply-combine（拆分—套用—合併）**，pandas 分組聚合的唯一心智模型。
記住這三步，所有變體（多欄分組、一次算 sum+mean、自訂函式）都是同一招換參數。

第二件日常：**把兩張表接起來（merge / join）**。訂單表有 `user_id`，用戶表有名字——
按共同的鍵把兩份名單對起來，像按學號合併兩份成績單。唯一要想清楚的是**對不上的人怎麼辦**：

- `inner`：兩邊都有的才留（交集）
- `left`：左表全留，右表對不上的補 NaN
- `outer`:兩邊都全留（聯集）

第三件：**重塑（reshape）**。長表變寬表（`pivot_table`：城市當列、月份當欄，做報表）、
寬表變長表（`melt`：把欄名壓回值）。加上**排序**（`sort_values`）與**串接**（`concat`），
這章就是 pandas 的四大日常操作——資料分析八成的工夫都在這裡面。
