...
服務上線了。半夜有人回報「答案怪怪的」、帳單比預期高一倍、某些請求慢得要命——
**你怎麼查?** 你連不進使用者的瀏覽器、重現不了那個對話。
你唯一能靠的,是服務**當時往外送的訊號**。這就是**可觀測性**,
而 LLM 服務比傳統服務更需要它,因為它多了一堆獨有的、會出事的維度。

底層還是你在 [Part 19](../19-cloud-native/08-observability.md) 學的**三支柱**,
但每一柱都要加上 **LLM 專屬的內容**:

- **Logs(日誌)**:不只記「這個請求 200 OK」,要**結構化記下**每次 LLM 呼叫的
  model、prompt(或其雜湊/摘要——顧及 [PII](06-guardrails.md))、
  input/output token 數、花了多少錢、延遲、走了哪些工具。
- **Metrics(指標)**:除了傳統的 QPS/延遲,要多盯 LLM 的:
  **每請求 token 數、每請求成本、TTFT(首字時間)、快取命中率、工具呼叫失敗率**。
- **Traces(追蹤)**:一個 [agent](../29-ai-applications/05-agents-react.md) 請求會跨好幾步
  (檢索→模型→工具→再模型),用[分散式追蹤](../22-distributed-systems/08-distributed-tracing.md)
  把整條鏈串起來,才看得出「慢在哪一步、哪一步幻覺了」。

還有一個 LLM 獨有的關卡:**品質也要被觀測**——
線上抽樣跑[評估](../28-llm-genai/09-evaluation.md)、收集使用者的讚/倒讚,
因為「200 OK」不代表「答得好」。這章教你把這套 LLM 可觀測性建起來,
它同時是 [ch09 資料飛輪](09-data-flywheel.md)的資料來源——**看得見,才改得動**。
