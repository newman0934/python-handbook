...
- **用「函式裝飾器裝飾類別」(`@deco` 加在 `class` 上)**:想對整個類別做統一改造時——
  自動註冊到 registry(外掛/處理器)、批次補方法。`@dataclass`、`@functools.total_ordering` 就是這類。
- **用「類別當裝飾器」(實例可呼叫)**:當裝飾器**自己需要保存狀態**時——
  計數呼叫次數、快取、限流器——用類別的屬性存狀態,比閉包 + `nonlocal` 清楚。
- **和 [metaclass](../04-oop/13-metaclass.md) 的取捨**:**能用類別裝飾器達成的,別動用 metaclass**——
  裝飾器簡單、明確、好組合;「改造一個已建好的類別」優先選它。

一句話:**要改造類別 → 類別裝飾器;裝飾器要帶狀態 → 用類別當裝飾器。**
