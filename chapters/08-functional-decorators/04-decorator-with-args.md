...
回顧：基本裝飾器是「接收函式、回傳包裝函式」。**帶參數的裝飾器**多一層——它是一個**回傳裝飾器的函式（裝飾器工廠）**：

```python
@retry(times=3)          # retry(times=3) 先執行，回傳一個「真正的裝飾器」
def fetch(): ...
```

關鍵理解：`@retry(times=3)` 這行有**括號**——表示 `retry(times=3)` **先被呼叫**，它回傳的東西才是拿來裝飾 `fetch` 的裝飾器。等於：
...
所以帶參數的裝飾器是**三層函式**：外層（收裝飾器參數）→ 中層（收被裝飾函式）→ 內層（包裝器）。
