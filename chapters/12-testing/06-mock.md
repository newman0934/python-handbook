...
**mock（模擬物件）** 是一種**測試替身（test double）**——用「假的、可控的」物件替換真實依賴。`unittest.mock`（標準庫）提供：

- **`Mock`/`MagicMock`**：萬能的假物件——存取任何屬性/方法都自動回傳一個 Mock，可設定回傳值、可記錄呼叫。
- **`patch`**：在測試期間**暫時替換**某個物件（模組層級的函式、類別、方法），測試結束自動還原。

核心用途：**替換外部依賴（讓它回傳你要的、不真的執行）+ 驗證互動（它有沒有被正確呼叫）**。
