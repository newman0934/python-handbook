...
為什麼所有資料庫都長成「表格」?不是巧合,是 1970 年一個數學決定。

在那之前,資料庫像「用指標串起來的迷宮」——查資料要寫「怎麼走」的程序。E.F. Codd 的革命性提議:**用數學的「集合」來存資料**——表格只是集合的視覺化。

這帶來三個看似奇怪、其實是數學必然的性質:

- **列沒有順序**(集合無序)——所以不寫 ORDER BY 就不保證順序。
- **每格只放一個值**——一格塞「多個電話」違反模型,之後一定痛。
- **每列要能被唯一指認**——這就是**主鍵**(身分證字號);表和表之間用**外鍵**牽線。

而 SQL 的查詢,底層是一套叫**關聯代數**的運算:σ 選列(WHERE)、π 選欄(SELECT)、⋈ 配對(JOIN)。
最有用的一個祛魅:**join 不是魔法,就是「兩兩配對再過濾」**——所以忘寫 join 條件會爆出笛卡兒積(每列配每列)。

最後一個大紅利叫**資料獨立性**:因為你只說「要什麼」不說「怎麼拿」,DBA 加索引、優化器改執行方式,你的查詢一行都不用改——這就是宣告式的力量,也是關聯式統治半世紀的原因。
