...
自己動手寫「可以被 for 的物件」時，有一個設計鐵則。用「書與書籤」講：

- **容器（iterable）是書**：它只負責**持有內容**。
- **iterator 是書籤**：它負責記「**讀到第幾頁**」。

鐵則就是：**書和書籤必須分開——書本身絕不記頁數**。

為什麼？想像把書籤直接黏死在書上（把「遍歷進度」存在容器身上）：

- 兩個人**不能同時讀**這本書——共用一個進度，你翻一頁我也被翻頁（巢狀 for 直接壞掉）。
- 讀完一遍，書籤停在最後——**這本書再也不能從頭讀**（無法重複遍歷）。

正確設計：**每次有人要讀（呼叫 `__iter__`），就發一張全新的書籤**（回傳一個新的、進度歸零的 iterator）。
書可以同時被十個人讀、讀幾遍都行——因為每個人拿的是自己的書籤。

落到程式碼就是兩個類別的分工：

- 容器類別：實作 `__iter__` → **回傳新的 iterator 物件**。
- iterator 類別：實作 `__next__`（吐下一個 / 沒了就 raise StopIteration）＋ `__iter__`（回傳自己）。

這章帶你各寫一次,並看清楚「把進度放錯地方」會出什麼鬼。
...
實作迭代協定有兩種角色，鐵則是**把它們分開**（書與書籤）：

- **容器（iterable）**：持有資料（書），實作 `__iter__`——每次呼叫都**回傳一個新的 iterator**（發一張新書籤）。
- **iterator**：持有「遍歷到哪」的狀態（書籤），實作 `__next__`（回下一個 / raise `StopIteration`）和 `__iter__`（回自己）。

**為什麼要分開？** 因為「遍歷進度」是**一次遍歷**的狀態，不該屬於容器本身。

把進度放在容器上（書籤黏死在書上）的後果：

- 容器只能被遍歷一次（讀完書籤停在最後）。
- 無法巢狀遍歷——兩個 `for` 同時跑會共用進度、互相干擾（兩人搶一張書籤）。
