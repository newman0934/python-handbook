...
LLM 生成一段長回應要好幾秒。如果**等它全寫完才顯示**,
使用者對著空白畫面乾等——體驗差,還可能觸發逾時。
你在 ChatGPT 看到字**一個一個蹦出來**,就是**串流(streaming)** 的效果。

原理很直覺:模型本來就是[一個 token 一個 token 生成](01-llm-fundamentals.md)的,
串流只是**「產一個、送一個」**,而不是「全部產完一次送」。
技術上用 **SSE(Server-Sent Events)**,你的程式**迭代一串事件**,
每收到一小塊文字(delta)就立刻顯示——**首字時間(TTFB)** 從幾秒降到零點幾秒,
感受天差地遠。

這章的另一半是 **async(非同步)**,而它正好是你**老朋友的重逢**:
還記得 [Part 9 的單人服務生](../09-concurrency/README.md)、
[Part 18 的 async 效能](../18-performance/07-async-performance.md)嗎?
LLM 呼叫是**極致的 I/O-bound**——你的程式 99% 時間在**等模型回話**,CPU 全閒著。
這正是 async 的主場:用 `async`/`await`,
一個程式可以**同時等一百個 LLM 請求**(如批次處理一萬筆客服訊息),
吞吐量翻幾十倍——而不是傻傻一個等完再發下一個。

這章實作串流輸出(含 tool use 的串流)、
用 `asyncio.gather` 併發多個請求、以及配 `Semaphore` 限流
(別把自己的 rate limit 打爆)——把前面並發章的觀念,落到 LLM 實戰。
