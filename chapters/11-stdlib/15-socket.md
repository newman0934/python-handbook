...
**socket（套接字）** 是「網路通訊的端點」——兩個程式透過各自的 socket 建立連線、收發資料。它是作業系統提供的網路介面，Python 的 `socket` 模組是它的封裝。

兩種主要協定：

- **TCP（傳輸控制協定）**：**可靠、有連線、有序**——保證資料完整到達、順序正確。HTTP、資料庫、多數應用用 TCP。
- **UDP（使用者資料報協定）**：**快、無連線、不保證**——不保證到達或順序，但延遲低。影音串流、遊戲、DNS 用 UDP。

**伺服器/客戶端模型**：伺服器**監聽**某個位址+埠、等待連線；客戶端**連線**到伺服器；建立連線後雙向收發資料。
