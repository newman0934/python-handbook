...
weakref 的存在只為一件事:**「我想引用這個物件,但不想因此害它無法被回收。」**
這在這些時候正是你要的:

- **快取 / 暫存**:快取指向物件,但當程式其他地方都不再用它時,**讓它能被回收**——
  別讓快取本身變成記憶體洩漏。`WeakValueDictionary` / `WeakKeyDictionary` 就為此而生。
- **觀察者模式**:subject 持有一堆 observer,但不該因此讓「早已無用的 observer」永遠活著(這是常見的洩漏源)。
- **打破循環引用**:父子互相引用時,讓其中一邊(子 → 父)用弱引用,避免循環(見 [垃圾回收](04-garbage-collection.md))。
- **替第三方物件附掛資料**:想給「你改不了的物件」關聯額外資訊,又不想延長它的壽命。

一句話:**當「持有一個引用」不該「延長物件壽命」時,就用 weakref**——它幾乎總出現在快取、觀察者、與避免洩漏的場景。
