...
> `re` 模組用正規表達式做模式比對——`search`/`match`/`findall`/`sub`。它強大但也容易寫出難懂、低效、甚至有安全漏洞的 pattern。掌握核心用法與「何時不該用 regex」同樣重要。
...
**正規表達式（regular expression, regex）** 是描述「文字模式」的語言——用特殊符號表達「一個數字」「一個以上的字母」「開頭是…」等。`re` 模組編譯這些 pattern 並在文字上比對。
