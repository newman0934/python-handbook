...
> 一個「只是用來裝資料」的 class，為什麼要手寫十幾行的 `__init__`、`__repr__`、`__eq__`？`@dataclass` 一行就全幫你生出來。這章講它替你做了什麼，以及那個幾乎每個人都踩過的「可變預設值」坑。

## 💡 白話導讀（建議先讀）

很多類別其實「只是用來裝資料的」——一個 Point 裝 x 和 y、一個 User 裝名字和 email。

但手寫這種類別意外地囉嗦。你得寫：

- `__init__`：把每個參數一一賦值給屬性（`self.x = x`、`self.y = y`⋯⋯）
- `__repr__`：讓 print 出來的東西能看
- `__eq__`：讓兩個內容相同的物件比較起來相等

全是**制式樣板**，跟抄表格一樣無聊，而且欄位一多就容易抄錯。
...
你只宣告「有哪些欄位」，`__init__`、`__repr__`、`__eq__` 它全部自動生成。
十幾行變三行，而且不會抄錯。

這章教你這個現代 Python 裝資料的首選工具，以及它的進階旋鈕（預設值、唯讀 frozen、排序⋯⋯）。

## 🎯 什麼時候會用到

- **任何「只是一包資料」的物件**:DTO(資料傳輸物件)、設定物件、一次回傳多個值的結構、
  樹/圖的節點、事件物件——想要 `__init__`/`__repr__`/`__eq__` 但不想手寫一堆樣板時。
- **當你發現自己在傳「欄位固定的 dict」**(`user["name"]`、`user["age"]`),
  就是該換 dataclass 的訊號:換來型別提示、IDE 自動補全、拼錯欄位當場報錯。

**怎麼在一堆選項裡挑**(由輕到重):

| 選項 | 什麼時候用 |
|------|-----------|
| `tuple` / `dict` | 臨時、一次性、欄位少;不值得定義類別 |
| `NamedTuple` | 要**不可變**、輕量、可解包(`x, y = point`)的小資料 |
| **`@dataclass`** | **預設首選**:可變、可給預設值、可加方法、最通用的資料容器 |
| `pydantic.BaseModel` | 資料來自**外部、不可信**(API 請求、設定檔),需要**執行期驗證與解析** |
...
`@dataclass` 是一個**類別裝飾器**。
它讀取你用型別註記宣告的欄位清單，替你生成那些制式方法：

- `__init__`：依欄位順序接收參數並賦值。
- `__repr__`：印出 `ClassName(field1=..., field2=...)` 的可讀形式。
- `__eq__`：依欄位逐一比較（內容相同就相等）。
- （可選）`__lt__` 等排序方法、`__hash__`、frozen 唯讀等。

分工很清楚：**你專注「有哪些欄位」，樣板交給它**。
