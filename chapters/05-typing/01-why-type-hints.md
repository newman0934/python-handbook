...
想像你在搬家，每個紙箱上貼了標籤：「廚房——易碎」「書房——書」。

有趣的是：**搬家工人根本不看標籤**，照樣把箱子搬上車——貼錯了也不會有人當場阻止你。

那標籤有什麼用？三個用處：

1. **出發前有檢查員**可以對著標籤逐箱核對：「這箱寫『書』，裡面怎麼是碗盤？」——在上路前抓出錯誤。
2. **你自己好找**：看標籤就知道箱子裡是什麼，不用拆開。
3. **交接給別人**時，對方一看就懂。

Python 的型別註記就是這些標籤：

- `def add(a: int, b: int) -> int:` 是在箱子上貼「進來兩個 int、出去一個 int」。
- **Python 執行時不看標籤**（傳錯型別照樣跑）——這點跟 Java 完全不同，一定要先接受。
- 真正看標籤的是**檢查員**（mypy、pyright 這些工具）和你的 **IDE**（自動補全、滑鼠提示都靠它）。

還有一個貼心設計：標籤**想貼多少貼多少**。
可以全不貼（跟以前一樣）、只貼重要的箱子（公開 API）、或全部貼好貼滿（大型專案）——這叫「漸進式型別」。

一句話：**型別註記不改變程式怎麼跑，它改變的是「錯誤在什麼時候被發現」——從半夜上線後，提前到寫程式的當下。**

## 🔗 前端對照

Python 的 type hints 和 TypeScript 解決**同一個問題**——給動態語言加型別,換取 IDE 補全與早期錯誤發現。
但兩者「型別去哪了」完全不同:

| | Python type hints | TypeScript |
|---|-------------------|-----------|
| 是不是另一種語言 | 否,就是 Python 語法 | 是,編譯成 JS |
| 誰檢查 | 外部工具 `mypy`（不擋執行） | `tsc` 編譯期 |
| 執行期還在嗎 | **在**,存在 `__annotations__` | **擦除**,執行期看不到 |
| runtime 能利用嗎 | ✅ pydantic / FastAPI 讀它做驗證 | ❌ 需另用 zod 等在 runtime 重寫一份 |

一句話:**TS 的型別編譯後就消失**;Python 的型別註記**留在執行期**,所以 pydantic 能直接拿它做資料驗證——
這是 Python 型別最迷人的地方（見 [Part 14 pydantic](../14-web/06-pydantic-validation.md)）。
...
Python 採用**漸進式型別（gradual typing）**——標籤想貼多少貼多少。你可以：

- 完全不寫（純動態，和以前一樣）。
- 只在關鍵處寫（公開 API、複雜函式）。
- 全面寫（大型專案、`strict` 模式）。

再強調一次關鍵原則：**型別註記在執行期預設「不被強制」**。
Python 直譯器**不會**因為你把 `int` 傳給標註 `str` 的參數而報錯——註記只是「給人與工具看的中繼資料」（搬家工人不看標籤）。

真正做檢查的是**外部工具**（mypy、pyright）——那位「出發前的檢查員」。
它們不執行你的程式，用**靜態分析**讀碼對標籤，找出矛盾。
