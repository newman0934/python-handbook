...
傳統程式有單元測試把關:改壞了,CI 亮紅燈,擋下合併,壞碼進不了 main。
但 LLM 應用的「行為」藏在 [prompt](../28-llm-genai/03-prompt-engineering.md)、
檢索設定、模型選擇裡——**改一個字的 prompt,可能修好三個場景、悄悄弄壞十個**,
而傳統 `assert` 完全測不出來(輸出每次還不一樣)。這章講怎麼補上這個大洞。

核心概念叫 **eval gate(評估閘門)**:像測試閘門,但把關的是**品質分數**,
而不是非黑即白的通過/失敗。它把 [Part 28 學的評估](../28-llm-genai/09-evaluation.md)
從「偶爾手動跑一下」升級成「**每次改動都自動跑、分數退步就擋下**」的 CI 關卡。

三個要件:

- **評估集(golden set)**:一組代表性的「輸入 + 期望」,像測試案例,
  但評的是品質。**要版本控制、隨產品演進持續擴充**——尤其把線上出過的包收進來。
- **評分器(scorer)**:規則比對、語意相似、或 [LLM-as-judge](../28-llm-genai/09-evaluation.md),
  給每個案例打分。
- **閘門邏輯**:設門檻——「整體分數不得低於基準」「不得有任何案例從對變錯(回歸)」,
  不過就擋下 PR。

還有兩個實務關鍵這章會談:**LLM 評估的不確定性**
(評分本身有波動,門檻要留緩衝、或多次取平均)、
以及**成本**(每次 CI 跑全套 eval 要花錢花時間,得挑代表性子集、或只在關鍵改動時跑全套)。
把 eval 變成 CI 閘門,你才敢**放心地持續迭代 prompt**——
這是 LLM 應用能穩定進步、而不是改東壞西的關鍵基礎設施。
