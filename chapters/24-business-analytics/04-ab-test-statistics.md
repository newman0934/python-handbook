...
[Part 30](../30-production-ai/08-ab-testing-versioning.md) 教過怎麼把流量分流;
這章補上**統計的靈魂**——讓 A/B 測試的結論**站得住腳**,而不是自欺欺人。

一個嚴謹的 A/B 測試,關鍵動作幾乎都在**開跑之前**:

- **先算樣本量**:這是最多人跳過、也最致命的一步。
  根據基準率、你想抓到的最小改善幅度(MDE)、顯著水準,
  **算出「每組要收多少人、要跑多久」**。沒算就開,等於不知道實驗要跑到哪天——
  於是就有了下面這個頭號作弊。
- **禁止偷看喊停(peeking)**:「數據跑到顯著就收工」是 A/B 測試的最大騙局——
  隨機波動總會在某個瞬間湊到「顯著」,你一看到就停,等於專挑運氣好的時刻下結論。
  **講好樣本量,收滿才看結果**。

跑完之後才是統計檢定:比較兩組轉換率用**雙比例 z 檢定**或卡方;
比較兩組數值(客單價、停留時間)用 **t 檢定**;
看的不只是 p-value,更要看**信賴區間**(改善幅度可能落在多少到多少之間——
「提升 0.1%~0.2%」就算顯著也可能不值得做)。

還有一票實戰陷阱這章逐一拆:**辛普森悖論**(整體贏、分群全輸)、
**多重比較**(同時測 20 個指標,總有一個假陽性)、
**新奇效應**(使用者只是好奇新版,一週後打回原形)。
A/B 測試好做,**做對很難**——難的全是統計紀律。
