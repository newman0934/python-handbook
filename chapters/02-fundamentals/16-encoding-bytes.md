...
- **字元集 / Unicode**：一張「字元 ↔ 編號（code point，碼位）」的巨大對照表。`A` 是 U+0041、`中` 是 U+4E2D、`👍` 是 U+1F44D。Unicode 只定義「字元對應哪個編號」，**不規定怎麼存成位元組**。
- **編碼（encoding）**：把 Unicode 碼位**轉成位元組**的規則。UTF-8、UTF-16、Big5、Latin-1 都是編碼。**UTF-8 是網路與檔案的事實標準**（相容 ASCII、變長、省空間）。
- **`str` vs `bytes`**：
  - **`str`**：Python 的**文字**型別——一串 **Unicode 字元**（碼位）。`"café"`、`"中文"`。與編碼無關（它是抽象的字元）。
  - **`bytes`**：一串**位元組**（0–255 的整數）。`b"caf\xc3\xa9"`。是 `str` 經某編碼後的產物，或來自檔案/網路的原始資料。

**核心關係**：

```text
str  --.encode(編碼)-->  bytes      （文字 → 位元組，寫出/傳送時）
bytes --.decode(編碼)-->  str        （位元組 → 文字，讀入/接收時）
```

**關鍵心法**：**`str` 是給人看的文字、`bytes` 是給機器存/傳的位元組**。你的程式內部用 `str` 處理文字，只在**邊界**（讀寫檔案、網路收發）做 encode/decode。這叫 **Unicode sandwich（Unicode 三明治）**：**輸入時 decode 成 str → 內部全用 str → 輸出時 encode 成 bytes**。
...
🎉 **恭喜完成 Part 2！** 你已掌握 Python 的語言地基：動態型別與名稱綁定、數值與字串、流程控制、函式與參數、作用域與閉包、推導式與現代語法，以及字元編碼與 bytes/str。
