...
原理在[原理篇 ch05](05-index-internals.md)講透了(排好序的字典、B+tree)。這章是**實用操作手冊**:

**何時建**——三個高頻訊號:

1. `WHERE` 常用的欄位(`WHERE email = ?`)
2. `JOIN` 的鍵(外鍵欄位)
3. `ORDER BY` 常排的欄位
...
- 佔空間(它是資料的副本)
- **拖慢每一次寫入**(INSERT/UPDATE 都要同步維護索引)——寫多讀少的表,索引要克制
...
看到 `SCAN`(全表掃描)= 沒用到索引;`SEARCH ... USING INDEX` = 用到了。
「加了索引怎麼沒變快」的答案幾乎都在這一步——常見原因:對欄位套了函式、`LIKE '%開頭模糊'`、或優化器判斷全掃更快([ch06 導航系統](06-query-processing.md)的邏輯)。

一句總結:**索引是讀寫之間的槓桿——為真實的查詢模式建索引,用 EXPLAIN 驗證,別憑感覺。**
