...
聊天系統的第一個難題:HTTP 是「客戶端問、伺服器答」——
但聊天需要**伺服器主動敲你**(有人傳訊息給你了)。三種解法演進:

- **輪詢(polling)＝每秒打電話問「有我的信嗎」**:浪費(99% 的電話白打)、延遲高。
- **long polling＝打電話不掛,有信才回**:好一點,但每次收完要重撥。
- **WebSocket＝拉一條專線**:握手一次,之後**雙向隨時互傳**——正解。
  聊天、通知、協作編輯全靠它。

第二個難題:**fan-out(訊息扇出)**——群組訊息要送達 500 個成員,怎麼送?

- **寫時扇出**:訊息一到,立刻推給所有在線成員(即時,適合小群)。
- **讀時扇出**:只寫進群組的訊息流,成員上線時自己來拉(省資源,適合萬人大群)。
  實務常混用:小群推、大群拉。

再來是可靠性三件套,面試官必追問:
**離線怎麼辦**(訊息先落庫,上線後補拉)、
**順序怎麼保證**(同一會話內用序號,別依賴時間戳——
[分散式 ID](13-system-design-distributed-id.md) 下章登場)、
**已讀/送達回執**(狀態機:送出→送達→已讀)。

多台聊天伺服器之間怎麼互通?(你連 A 台、朋友連 B 台)——
答案又是老朋友:[pub/sub 訊息系統](../16-architecture/10-event-driven-mq.md)當跨台廣播骨幹。
這章把整套架構畫出來,並用 FastAPI WebSocket 實作一個可跑的迷你版。
