...
最有價值的分類問題,資料往往**極度偏斜**:詐騙、癌症、故障——
「壞的那類」可能只佔 1%、甚至 0.1%。這帶來一個致命陷阱
(在 [Part 25 評估](../25-machine-learning/06-model-evaluation.md)就警告過):
模型只要**無腦全猜「正常」**,準確率就有 99%——卻**一個詐騙都沒抓到**,
等於沒訓練。這章教你怎麼讓模型**真的學會看見那 1%**。

三類手段,由簡到繁:

- **給少數類的錯更重的罰(class_weight)**:告訴模型「漏抓一個詐騙 = 錯放 99 個正常那麼嚴重」——
  逼它別為了省事忽略少數類。sklearn 一個參數 `class_weight="balanced"` 搞定,
  **最簡單、常最有效,第一手先試它**。
- **動資料的比例(重採樣)**:**過採樣**少數類(複製,或用 **SMOTE** 合成新的少數類樣本)、
  **欠採樣**多數類(丟掉一些)。關鍵坑:**只能對訓練集動手,測試集要維持真實比例**——
  否則評估結果是自欺欺人。
- **調閾值 + 選對指標**:降低判「詐騙」的閾值(寧可誤殺),
  並且**別再看準確率**,改看 [precision/recall/F1/PR-AUC](../25-machine-learning/06-model-evaluation.md)——
  在不平衡問題裡,選對評估指標比選模型還重要。

一句話總結心態:**不平衡問題的敵人是「準確率的假象」**——
先換掉指標、再用加權或重採樣讓模型正視少數類。這章用信用卡詐騙資料實戰全套流程。
