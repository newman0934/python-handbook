...
- **`open(path, mode, encoding)`**：開檔，回傳一個檔案物件。
- **`with`**：context manager，**保證關檔**（即使出錯，見 [context manager](../06-error-handling/06-context-manager.md)）。
- **模式（mode）**：讀/寫/追加、文字/二進位。

**兩種模式維度**：
1. **操作**：`r`（讀）、`w`（寫，覆蓋）、`a`（追加）、`x`（新建，已存在則失敗）。
2. **型別**：文字模式（預設，處理 `str`）vs 二進位模式（`b`，處理 `bytes`）。
