...
1. **瓶頸是 I/O 還是 CPU？**（見 [並發 vs 並行](01-concurrency-vs-parallelism.md)）
   - **I/O 密集**（等網路/磁碟/DB）→ 並發即可，用 threading 或 asyncio。
   - **CPU 密集**（純計算）→ 需真並行，用 multiprocessing（繞過 GIL）。

2. **（若 I/O）並發量多大？**
   - **少量到中量**（幾個到幾十個）→ **threading**（簡單、夠用）。
   - **大量**（數百到數千連線）→ **asyncio**（單執行緒無開銷、可撐海量連線）。
