...
asyncio 的核心是 **event loop（事件迴圈）**——一個在**單一執行緒**內運行的排程器，它的工作循環：

1. 執行一個協程，直到它遇到 `await`（等待某個 I/O 或事件）。
2. 該協程**主動讓出控制權**（掛起），event loop 去執行**別的就緒的**協程。
3. 當被等待的 I/O 完成，event loop 恢復對應的協程。

這叫**協作式多工（cooperative multitasking）**——與 threading 的「搶佔式（作業系統隨時切換）」不同，asyncio 的切換只發生在 `await` 點，由協程**主動**讓出。
...
| 執行緒數 | 多 | 多行程 | **單執行緒** |
| 競態/鎖 | 需要 | 少 | **幾乎不需要**（單執行緒） |
| 適合 | I/O（中量） | CPU | **I/O（大量連線）** |

因為單執行緒 + 只在 await 切換，asyncio **幾乎不需要鎖**（沒有兩個協程「同時」執行的問題）——這消除了 threading 的一大類競態 bug。
