...
「一致性」不是有或沒有,是一條**光譜**——差別在「別人多快能看到你寫入的值」:

- **強一致（線性一致）＝全公司共編同一份雲端文件**:任何人任何時刻看到的
  都是最新版,彷彿世界上只有一份資料。體驗最好、實作最貴——
  每次寫入都要跨節點協調,慢,而且分區時只能停服務（CP）。
- **因果一致＝八卦傳播的底線**:「回覆」絕不會比「原文」先被看到——
  **有因果關係**的事件,所有人都按因果順序看到;無關的事件可以亂序。
  比強一致便宜,又避開最反直覺的怪象。
- **最終一致＝總部公告傳到各分店**:需要時間,期間各店看到的版本可能不同,
  但不再有新寫入後,**終究**大家會一致。最便宜、最能扛（AP 陣營的預設)。

最終一致的「期間」會鬧什麼笑話?兩個經典場景,名字都值得記:

- **讀不到自己的寫**:你發了留言、重新整理卻看不到——因為讀到了還沒同步的副本。
  解法叫 **read-your-writes** 保證(把你的讀取黏在寫入過的節點)。
- **時光倒流**:第一次刷新看到新值、再刷新又變回舊值(兩次讀到不同副本)。
  解法叫**單調讀**(monotonic reads)。

工程上的選法跟 [CAP 章](01-distributed-intro-cap.md)同一句話:
**按業務語義選**——錢包餘額用強一致,按讚數用最終一致;
需要「至少別亂序」的用因果一致。這章把光譜逐級講清,並示範各模型的違例長什麼樣。
