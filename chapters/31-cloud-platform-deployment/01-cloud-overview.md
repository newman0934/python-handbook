...
[Part 19](../19-cloud-native/README.md) 教你把應用**容器化**、寫成
[12-factor](../19-cloud-native/04-12-factor.md)——那是「打包好行李」。
這個 Part 講的是「**行李到底放進哪家旅館、怎麼入住**」:把 task-api 真正部署上 AWS/GCP。

上雲第一個要建立的地圖,是**部署模型光譜**——
一條「你管多少 vs 雲管多少」的滑桿,越往右你越輕鬆、但越不能客製:

```text
自己租房 ← ──────────────────────────── → 住旅館
IaaS         CaaS          託管 K8s         FaaS
(虛擬機)     (容器)        (Kubernetes)     (函式)
自己管       雲幫你跑       雲管叢集         你只丟程式碼
OS+環境      單一容器       你管工作負載      其餘全不管
```

- **IaaS(虛擬機)＝租空房**:給你一台裸機,OS、環境、應用全自己裝——最自由、最累。
- **CaaS(容器,如 Cloud Run/ECS)＝服務式公寓**:你交一個容器映像,
  雲負責跑起來、擴縮、修機器——**多數 web 服務的甜蜜點**,本 Part 主線。
- **託管 K8s(EKS/GKE)＝管理整棟大樓**:要編排很多服務時才值得的複雜度。
- **FaaS(函式,如 Lambda)＝住旅館按次計費**:只丟一個函式,
  不用時**縮到零、零成本**,但有冷啟動延遲。

這章帶你認識 AWS 與 GCP 的對應服務、這條光譜怎麼選,
並確立本 Part 的主線:**用 CaaS(Cloud Run 為主)把 task-api 上雲**——
因為它對「一個容器化的 web 服務」CP 值最高。選對抽象層,是上雲的第一個關鍵決定。
