...
關鍵原則：**領域邏輯不該知道 HTTP**（見 [分層架構](../16-architecture/02-layered-architecture.md)）——它拋領域例外，Web 層的例外處理器負責映射成狀態碼。
...
更好的架構：**領域邏輯拋領域例外（不含 HTTP），Web 層用例外處理器映射成 HTTP**——領域層不依賴 Web（見 [分層架構](../16-architecture/02-layered-architecture.md)、[自訂例外](../06-error-handling/04-custom-exceptions.md)）：
...
好處：**領域邏輯乾淨（純業務，可在非 Web 場景重用、好測試）、HTTP 映射集中在 Web 層**。這是 [分層架構](../16-architecture/02-layered-architecture.md) 在錯誤處理的體現。
...
- **領域邏輯拋領域例外（不含 HTTP）、Web 層用例外處理器映射**（見 [分層架構](../16-architecture/02-layered-architecture.md)）：領域乾淨、可重用、好測。
...
- **能說出「領域邏輯拋領域例外、Web 層用例外處理器映射成 HTTP」的分層好處**（領域乾淨、可重用、好測，連結 [分層架構](../16-architecture/02-layered-architecture.md)）。
