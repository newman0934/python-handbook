...
| 工具 | 表達什麼 | 取代什麼 |
|------|----------|----------|
| `TypedDict` | 有固定 key 與各自型別的 dict | 模糊的 `dict[str, Any]` |
| `Literal` | 值只能是特定字面值之一 | 模糊的 `str` / `int` |
| `Final` | 名稱不可重新賦值（常數） | 靠命名慣例的常數 |
| `Annotated` | 型別 + 額外中繼資料 | 型別與中繼資料分離 |

它們都是**靜態檢查**（執行期不強制），但讓 mypy 能抓到更精細的錯誤。
