...
「Python 是直譯語言」——這句話你聽過一百次，但它是個**過度簡化**。

真相是兩段式的，用一個比喻：

想像一位**同步口譯員**幫外賓演講翻譯。他其實不是「聽一句、直接翻一句」——
專業做法是：先快速把講稿**改寫成一份精簡的速記稿**（去掉冗詞、標好結構），再照著速記稿流暢地念出翻譯。

CPython 執行你的程式，就是這兩步：

1. **編譯**：把你的 `.py` 原始碼，轉成一種精簡的中間指令——**bytecode**（速記稿）。
2. **執行**：由一台**虛擬機（PVM）**逐條讀 bytecode 執行（照速記稿念）。

幾個立刻能解釋的日常現象：

- **`__pycache__` 資料夾是什麼？**——就是速記稿的快取（`.pyc` 檔）。存起來下次不用重新改寫，加速啟動。可以放心刪，會重新生成。
- **為什麼說它「不是純直譯」？**——因為真的有編譯這一步,只是自動、隱形。
- **那和 Java 差在哪？**——架構其實類似(都編成 bytecode 給虛擬機),差在 Java 的 JVM 還會把熱點再編成機器碼(JIT),CPython 傳統上沒有——這是速度差距的來源之一。

這章打開引擎蓋看這條管線——它是之後所有底層章節([bytecode](../10-cpython-internals/README.md)、GIL、效能)的地基。
...
[第 1 章](01-why-python.md)埋的伏筆現在揭曉。CPython 執行程式是**兩段式**的——「先改寫速記稿、再照稿口譯」：

1. **編譯（compile）**：把原始碼 `.py` 編譯成 **bytecode（位元組碼）**——一種平台無關的中間指令（速記稿）。
...
- **不是純編譯**——不像 C 直接編成機器碼。
- **不是純直譯**——不是一行行讀原始碼執行。
- 而是「**先編成 bytecode、再由虛擬機直譯 bytecode**」。

這和 Java（編成 bytecode 給 JVM）在架構上類似——差別在 JVM 有 JIT 會把熱點編成機器碼，CPython 傳統上沒有。
