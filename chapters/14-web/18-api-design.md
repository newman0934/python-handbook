...
**API 是契約**——一旦有客戶端在用，你就不能隨意打破它。好的 API 設計圍繞「**能演進而不破壞**」：

**版本管理（versioning）**——讓 API 能演進：

- **URL 路徑版本**（`/v1/users`、`/v2/users`）：最常見、直觀、好路由。
- **Header 版本**（`Accept: application/vnd.api+json;version=2`）：URL 乾淨，但較隱晦。
- **原則**：**破壞性變更（breaking change）才升版本**——移除欄位、改變意義、改必填。**非破壞性變更**（新增選填欄位、新端點）不必升版，舊客戶端自然忽略新欄位。

**分頁（pagination）**——大集合分批回傳：

- **Offset/Limit（`?page=2&size=20` 或 `?offset=20&limit=20`）**：簡單直觀，能跳頁。缺點：深分頁（offset 很大）慢、資料變動時會漏/重複。
- **Cursor（游標，`?after=<cursor>`）**：用一個指向「上次最後一筆」的游標往後取。**穩定**（資料變動不漏不重）、深分頁也快，但不能跳頁。大型/即時資料首選。

**快取與條件請求（ETag）**——省頻寬：

- 伺服器回應帶 **`ETag`**（內容的指紋，如 hash）。客戶端下次請求帶 `If-None-Match: <etag>`——若內容沒變，伺服器回 **`304 Not Modified`**（**空 body**），客戶端用快取。省下重傳整個資源的頻寬。

**一致性原則**：統一的錯誤格式、命名慣例、查詢參數約定、狀態碼語意（見 [HTTP 基礎](02-http-basics.md)）、[冪等](../22-distributed-systems/06-idempotency.md)——讓客戶端能用同一套邏輯處理整個 API。
