...
同一個模型,換個問法,答案品質天差地遠。
**Prompt engineering** 不是玄學咒語,核心心法只有一句:

> **把模型當成一個絕頂聰明、但對你的情況一無所知的新同事。**

新同事能力很強,但他不知道你要什麼格式、什麼背景、什麼限制——
你把這些講清楚,他就做得好;你含糊其辭,他只能亂猜。
所以好 prompt 的技巧,全是「怎麼把話講清楚」:

- **具體明確**:別說「寫好一點」,說「用三個要點、每點不超過 20 字」。
- **給角色**:「你是資深 Python 工程師,回答簡潔、附程式碼」——一句話定調。
- **給範例(few-shot)**:與其解釋規則,不如**示範**兩三個「輸入→輸出」,
  模型照樣造句——通常比長篇說明更有效。
- **要它一步步想(chain-of-thought)**:複雜推理時加一句「請逐步思考」,
  讓它把思路攤開再作答,正確率明顯提升——
  因為它是[逐字接龍](01-llm-fundamentals.md)的,先寫出推理過程,後面的答案才有依據。

這章還講幾個實戰利器:用 XML 標籤或分隔線劃清「指令 vs 資料」
(避免使用者輸入被誤當指令——這也是 [prompt injection](../20-security-system-design/README.md) 的防線)、
要求指定輸出格式、以及「給模型思考空間」的各種技巧。
Prompt 是 LLM 應用最便宜、最快見效的優化槓桿——先把它練好,再想別的。
