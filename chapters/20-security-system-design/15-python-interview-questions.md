...
這章不教新東西——它是**全書的考前總複習清單**:
把散落在 31 個 Part 裡的面試考點,按主題收成一份索引,
每題附「面試官想聽到什麼」與回去深讀的章節連結。

用法建議:

1. **面試前一週**:從頭掃一遍,答不出「面試官想聽什麼」的題目,點連結回章節重讀。
2. **面試前一晚**:只看你標記過的弱項。
3. **平時**:讀完某個 Part 後來這裡自測該主題,檢驗有沒有真懂。

記住本書一路的原則:面試答題的層次是「**會用 → 知道為什麼 → 講得出底層**」,
每差一層,薪水差一截。這份清單幫你檢查的就是後兩層。
