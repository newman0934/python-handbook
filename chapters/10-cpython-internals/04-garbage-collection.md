...
上一章的計數制度有一個天生盲區，一個畫面看懂：

> **兩個人互相扶著，誰也不會倒——但他們被關在一個沒人進得去的房間裡。**

```python
a = Node(); b = Node()
a.friend = b          # a 貼著 b
b.friend = a          # b 貼著 a —— 互相扶持
del a; del b          # 外界的便利貼全撕掉了!
# 但兩個物件的計數都還是 1(彼此貼著彼此)——永不歸零、永不回收
```

外界再也摸不到他們（是垃圾），計數卻不是零（收不走）——**記憶體洩漏**。

CPython 的第二道防線：**循環垃圾回收器（cyclic GC）**——一位定期巡邏員，專門找「**抱團取暖但與世隔絕**」的物件群：從外界能到達的物件出發做可達性檢查,抓出「只剩彼此支撐」的循環,整團回收。

巡邏員還有一套聰明的排班——**分代（generational）**：

- 觀察事實:「**新生的物件大多早死,活過幾輪的物件傾向長壽**」（暫存變數轉眼就沒,設定物件活到程式結束）。
- 策略:物件分三代——**新生代查勤頻繁,老年代很少打擾**。像嬰兒房每小時巡,老人房一天一次——把力氣花在最可能有垃圾的地方。

還有個省力設計:**int、str 這些「不可能抱團」的原子型別,根本不在巡邏名單上**（它們不持有別的物件,形成不了循環）。

總結 CPython 記憶體回收的全貌一句話:**引用計數即時清大部分,GC 定期抓循環**——兩者是搭檔,不是取代。
...
CPython 的記憶體回收是**兩套機制的組合**——清潔工與巡邏員：

1. **引用計數（主力）**：處理絕大多數情況——歸零立刻回收（見[引用計數](03-reference-counting.md)）。
2. **循環 GC（補充）**：處理引用計數無法回收的**循環引用**——「互相扶著誰也不倒、但與世隔絕」的物件群。

循環 GC 只負責一件事：

> **找出「彼此引用形成循環、但外界已無法觸及」的物件群，回收它們。**

它不取代引用計數——大部分物件仍由引用計數即時回收，GC 只在「有循環垃圾」時介入。

省力設計：只有**可能形成循環的容器型物件**（list、dict、set、自訂物件等能持有其他物件的）才被 GC 追蹤。
**不可變的原子型別（int、str、float）不參與循環**（不持有其他物件的可變引用），所以不在巡邏名單上。
