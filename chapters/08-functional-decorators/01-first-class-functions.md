...
「函式是一等公民（first-class）」這件事,前端工程師再熟悉不過——JavaScript 從頭到尾都是這樣:

| | Python | JavaScript |
|---|--------|-----------|
| 函式存進變數 | `f = greet` | `const f = greet` |
| 當引數傳 | `map(f, xs)` | `xs.map(f)` |
| 當回傳值 | 回傳一個函式（closure） | 同 |
| 匿名函式 | `lambda`（單運算式） | 箭頭函式（無限制） |

一句話:**心智模型完全一樣**——函式就是值,能存、能傳、能回傳。如果你在前端寫慣回呼與高階函式,
這章幾乎是複習;唯一差異是 Python 的匿名函式（`lambda`）比箭頭函式受限（只能一個運算式）。
