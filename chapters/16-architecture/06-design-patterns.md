...
設計模式就是**前人的棋譜**：反覆出現的局面（「怎麼建立物件」「兩個物件怎麼協作」），
前人早就試錯出了穩定下法，並給每一招起了名字——Factory、Strategy、Observer……
名字的價值在溝通：「這裡用 Strategy」一句話，同事就懂整個結構。

但學 Python 的你要先知道一件事：**這些棋譜是為 C++/Java 寫的**，
那些語言沒有一等函式、沒有動態型別。Python 有，所以**很多模式在 Python 裡會縮水甚至消失**：

- **Strategy（策略）**：Java 要寫一堆策略類別；Python **傳個函式就好**（`sorted(key=...)` 就是 Strategy）。
- **Singleton（單例）**：Python 的 **module 天生就是單例**（import 多次只執行一次），多數時候不必寫類別。
- **Decorator（裝飾）**：Python 直接**內建語法** `@`。
- **Iterator**：內建協定（`__iter__`/`__next__`），`for` 迴圈原生支援。

所以這章的重點不是背 23 個模式，而是兩件事：

1. 認得**高頻的幾個**（Factory、Strategy、Observer、Adapter、Facade）解決什麼問題。
2. 知道**Python 的慣用簡化版長什麼樣**——面試答「Python 裡我會用一等函式代替策略類別」，
   比背教科書 UML 加分得多。
