...
> 測試覆蓋率 100%，就代表程式沒 bug 了嗎？——不。這章講怎麼用 `pytest-cov` 找出「根本沒被測到」的死角，以及為什麼「覆蓋率」是找漏洞的工具，一旦被當成追求的**目標**反而會害了你。

## 💡 白話導讀（建議先讀）

「我的測試夠了嗎？」——覆蓋率給你一半的答案。

把程式碼想成**一座城市**,測試是**巡邏路線**。覆蓋率報告=巡邏熱圖：**哪些街區走過、哪些從沒去過**。

```bash
pytest --cov=myapp --cov-report=term-missing
# ↑ 顯示每個檔案的覆蓋率,以及「哪幾行沒被執行過」
```

沒去過的街區（沒被執行的行）**一定沒被測到**——error 處理分支、罕見的 if 路徑,常常就躲在這裡。這是覆蓋率的真本事:**找漏測**。

但要立刻接一句警語——**巡過 ≠ 檢查過**：

```python
def test_process():
    process(data)        # 執行了(覆蓋率 100%!)——但一個 assert 都沒有
```

這個測試覆蓋了所有行,卻**什麼都沒驗證**——覆蓋率滿分,價值零分。

所以記住定位：

> **覆蓋率是「必要非充分」**:沒覆蓋的一定沒測（有用的警報）;覆蓋了的不代表測得好（別當 KPI 崇拜）。

實務建議:行覆蓋率之外開 **branch coverage**（if 的兩邊都要走過,更嚴格）;目標訂個合理值（80% 上下）當警戒線,**別為了 100% 寫無意義的測試**——那是本末倒置。
...
**覆蓋率**量測「測試執行時，程式碼被執行的比例」——巡邏熱圖。兩種：
...
> 覆蓋率告訴你「哪些程式碼**沒被執行**」（一定有漏測）；但**不**告訴你「被執行的有沒有被**正確驗證**」。

所以高覆蓋率是**必要非充分**——沒被覆蓋的一定沒測，被覆蓋的不代表測得好。它是「找漏測」的工具，不是「測試品質」的分數。
