...
服務間溝通只有兩種基本姿勢,對應你每天都在做的事:

- **打電話（同步,request-response）**:撥出去、**掛著等**對方回答。
  REST、[gRPC](02-grpc-protobuf.md) 都是。優點:直覺、馬上拿到答案。
  致命傷:**時間耦合**——對方慢你就慢,對方掛你就失敗。
  呼叫鏈一長（A→B→C→D）,任何一環打噴嚏,整條鏈感冒——
  所以同步呼叫必須配[逾時、重試、熔斷](07-rate-limit-circuit-breaker.md)這套護具。
- **留言（非同步,訊息/事件）**:訊息丟進[公告欄/佇列](../16-architecture/10-event-driven-mq.md)就走人,
  對方有空再處理。優點:**解耦**——對方掛了,訊息在佇列裡等它回來,你完全無感。
  代價:拿不到即時結果、只有最終一致、除錯要跨系統追。

怎麼選?問一個問題:「**呼叫方需要立刻知道結果嗎?**」

- 「下單前查庫存夠不夠」——要,同步。
- 「下單後寄確認信」——不用,非同步(信晚十秒寄到沒人在乎)。

實務系統永遠是**混合**:查詢類同步、副作用類非同步。
這章還講同步呼叫的正確姿勢(必設 timeout、重試要配退避與冪等、
連線池重用)與常見反模式(把非同步硬做成「發了訊息再輪詢等結果」——
那你其實需要的是同步)。
