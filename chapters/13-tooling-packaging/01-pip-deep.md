...
「在我電腦上能跑」的另一大來源：**你和同事裝的套件版本不一樣**。

你三月裝的 `requests` 是 2.31、他今天裝到 2.32,行為差一點點——bug 只在其中一台出現。

解法的核心概念是**食譜 vs 採購清單**：

- **`requirements.txt`（或 pyproject 的 dependencies）＝食譜**：寫「需要 requests」——**意圖**,版本寬鬆。
- **lock 檔＝精確採購清單**：把**當下實際裝了什麼**（含版本、含連 `requests` 自己依賴的 urllib3、certifi⋯⋯）全部釘死。

第二個概念跟著來了——**你裝 1 個套件,實際進來 10 個**：

- **直接相依**:你點名要的（requests）。
- **傳遞相依**:它帶來的親戚（urllib3、certifi⋯⋯）。

「可重現」= **直接＋傳遞全部鎖定精確版本**——別人照採購清單買,環境跟你一模一樣。

日常操作對號:

```bash
pip freeze > requirements.lock    # 產出目前環境的精確清單
pip install -r requirements.lock  # 照清單精確重建
pip install -e ".[dev]"           # 開發模式安裝(本書用法):改原始碼立即生效
```

版本指定的語法（`>=2.0`、`~=2.31`、`==2.31.0`）與 pip 怎麼解版本衝突,章內展開。
...
1. **可重現性（reproducibility）**：如何讓「你的環境」在別人機器、CI、部署伺服器上**精確重建**？——靠鎖定所有套件的精確版本（食譜之外,還要精確採購清單）。
2. **相依解析（dependency resolution）**：pip 如何決定「裝哪些版本」以滿足所有套件的版本需求？——resolver 找出相容的版本組合，找不到就衝突。

關鍵區分：

- **直接相依（direct）**：你明確需要的（`requests`）。
- **傳遞相依（transitive）**：你的相依所需要的（`requests` 帶來的 `urllib3`、`certifi`⋯⋯）。
