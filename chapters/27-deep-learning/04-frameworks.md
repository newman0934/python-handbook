...
[上一章](03-nn-from-scratch.md)你親手算了反向傳播——也親身體會到:
**手動算梯度又繁瑣又易錯**(轉置、形狀、連鎖律,一步錯全盤錯),
而且換個網路架構就得重算一次。這不可能規模化。
**深度學習框架(PyTorch、TensorFlow)** 就是來解決這件事的。

框架的核心魔法叫**自動微分(autograd)**,原理出奇地優雅:

- 你只管寫**前向傳播**(`c = a * b`、`loss = ...`),
  框架在背後**偷偷記錄每一步運算**,連成一張**計算圖**。
- 你呼叫一句 `loss.backward()`,框架就**沿著這張圖倒著走**,
  自動套連鎖律,把每個參數的梯度算好——
  你上一章手刻半天的事,**現在一行搞定,還不會錯**。

除了自動微分,框架還給你三樣大禮:
**GPU 加速**(同樣的程式碼 `.to("cuda")` 就快幾十倍——深度學習能起飛的關鍵)、
**現成的層與優化器**(`nn.Linear`、`Adam` 不用自己刻)、
**龐大的生態**(預訓練模型、資料工具)。

這章用 **PyTorch** 把上一章手刻的網路**重寫一遍**,
你會看到幾十行的手工苦力縮成幾行——
並理解 tensor、autograd、`nn.Module`、優化器這些核心概念。
從這章起,你就有了打造真實深度學習模型的工具。
(本書選 PyTorch 示範,因為它在研究與業界都是主流、語法最直覺。)
