...
先弄清一個容易誤會的事：**「Python」其實是一份規格書，不是一個軟體**。

就像「藍牙」是一套規格，實際上你買的是某牌的藍牙耳機。
你從 python.org 下載安裝的，是這份規格最主流的實作——**CPython**（用 C 寫的 Python）。平常大家說「裝 Python」，講的就是它。

裝好之後，電腦裡多了三樣綁在一起的東西：

1. **直譯器**：那個真正執行 `.py` 檔的程式（就是你打的 `python` 指令）。
2. **標準庫**：附贈的一大箱工具（`os`、`json`⋯⋯），免安裝直接用。
3. **pip**：以後用來裝第三方套件的工具。

「綁在一起」這點很重要——因為電腦上常常**不只一份 Python**（系統內建一份、你自己裝一份、不同版本各一份）。
每一份都有**自己的**標準庫、自己的 pip、自己裝過的套件。

新手最常見的鬼打牆——「明明裝了套件卻 import 不到」——九成的原因就是：**你把套件裝到了 A 份 Python，卻用 B 份在跑程式**。

這章教你搞清楚「我現在用的是哪一份、它裝在哪」，之後遇到多版本共存就不會迷路。
...
回顧上一章：**Python 是規格，你安裝的是某個實作。** 從 python.org 下載的官方安裝檔就是 **CPython**。

安裝一份 CPython，實際上會在系統放進三樣東西（導讀說的「綁在一起的一組」）：

1. **直譯器執行檔**：`python`（或 `python3`、`python3.12`）——會讀你的 `.py` 並執行的程式。
2. **標準庫**：一大包內建模組（`os`、`json`、`collections`⋯⋯），放在安裝目錄的 `lib` 底下。
3. **pip**：套件安裝工具，用來從 PyPI 下載第三方套件（見 [pip 與套件管理](04-pip-and-packages.md)）。

理解「這三者是一整組」，就能理解多版本共存的一切現象：

> **不同 Python 版本 = 不同的標準庫 + 不同的 pip + 不同的已裝套件。**
> 套件裝進 A 份、用 B 份執行，當然 import 不到。
