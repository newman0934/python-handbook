...
餐廳打烊有 SOP:先**掛出「停止接客」**的牌子,讓**已入座的客人吃完**,
再收拾、關瓦斯、鎖門。絕不是時間一到直接斷電趕人。

部署新版時,你的舊容器就是那家要打烊的店。K8s 的打烊通知分兩段:

- **`SIGTERM`＝「準備打烊」**:先禮貌通知,**可以攔截、應該攔截**——
  收到後開始收尾:停止接新請求、把手上的請求做完、關資料庫連線。
- **`SIGKILL`＝直接斷電**:寬限期（預設 30 秒）內沒自己退出,就強制終止,
  **攔不住、救不了**——在途請求硬生生折斷,使用者看到 502。

所以「優雅關閉」的全部工作,就是把 SIGTERM 到 SIGKILL 之間這 30 秒用好:

1. K8s 先把 Pod **從 Service 摘掉**（不再進新客人）——但這有傳播延遲,
   所以最佳實踐是收到 SIGTERM 後**先小睡幾秒**再開始關(還有零星客人在進門)。
2. 讓 event loop 上**在途的請求跑完**（uvicorn/gunicorn 內建支援,但要確認訊號有傳到)。
3. **釋放資源**:資料庫連線池、背景任務,收乾淨。

最經典的坑在 Docker:用 shell 形式的 `CMD python app.py`,訊號送給的是 shell
而不是 Python——你的處理器根本沒收到,30 秒後直接被 SIGKILL。
解法:**exec 形式** `CMD ["python", "app.py"]`。這章逐一示範訊號處理、
FastAPI 的 lifespan 收尾、與 K8s 探針的配合。
