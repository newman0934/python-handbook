...
CPU 密集任務怎麼辦？一間廚房只有一把刀（GIL）——那就**開分店**。

`multiprocessing` 的做法：**每個行程是一家獨立的分店**——

- 自己的廚房、自己的貨架（**獨立記憶體**）
- 最關鍵的：**自己的一把刀（獨立的 GIL！）**

四家分店四把刀，**真正同時切菜**——CPU 密集任務終於吃滿多核心。這是 Python 加速純計算的正解。

但獨立是有代價的，兩條：

1. **開分店很貴**：要啟動一整個新的 Python 直譯器——比雇一位店員（開執行緒）重得多。
2. **分店之間傳東西要「打包宅配」**：記憶體不共享,資料要**序列化（pickle）**打包、送過去、拆包。
   後果 a：傳大資料的開銷可能吃掉並行的收益——**適合「計算重、資料傳輸少」**的工作。
   後果 b：**傳不了不能 pickle 的東西**（開著的檔案、lambda⋯⋯）——經典報錯來源。

還有一個 Windows 使用者必踩的坑,先打預防針：**主程式碼一定要包在 `if __name__ == "__main__":` 裡**——因為子行程啟動時會重新 import 你的模組（[Part 1 那道開關](../01-getting-started/03-repl-and-first-program.md)在這裡救命）,不包會無限開行程。

選型口訣更新：**等待型多店員（threading）、計算型開分店（multiprocessing）。**
...
**行程（process）** 與**執行緒（thread）** 的根本差異——分店 vs 同店店員：

| | 執行緒（threading） | 行程（multiprocessing） |
|--|---------------------|-------------------------|
| 記憶體 | **共享**（同一間店） | **獨立**（各自一家分店） |
| GIL | 共用一個（一把刀，無法並行 CPU） | **各自一個（各有一把刀，能並行）** |
| 建立成本 | 輕（雇店員） | 重（開分店：啟動整個直譯器） |
| 資料交換 | 直接共享（伸手拿） | **要序列化（pickle）打包宅配** |
| 適合 | I/O 密集 | **CPU 密集** |

關鍵：行程**獨立記憶體 + 獨立 GIL** → 能真正並行；但資料無法直接共享，要 pickle 來回傳（有開銷）。

所以 multiprocessing 適合「**計算重、資料傳遞相對少**」的 CPU 密集任務。
