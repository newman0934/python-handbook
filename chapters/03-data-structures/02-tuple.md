...
tuple 乍看是「不能改的 list」——但「不能改」不是缺陷，是它的**賣點**。用一個對比：

- list 像**購物清單**：同類的東西、隨時增刪（「再加一瓶牛奶」）。
- tuple 像**身分證上的欄位**：(姓名, 生日, 號碼)——**每個位置有固定意義**，而且不該被改。

「不可變」直接換來三個好處：

1. **能當 dict 的 key、能放進 set**。因為不可變才能安全算 hash（[第 7 章](07-hashable.md)展開）——`{(緯度, 經度): 地名}` 這種用法 list 做不到。
2. **語意清楚**：看到 tuple 就知道「這是一筆固定結構的紀錄」，位置有意義。
3. **更省、更安全**：不可變的東西可以放心到處共享。

這章還有一個 Python 的招牌甜頭——**解構賦值（unpacking）**：

```python
name, age = ("Alice", 30)      # 一行拆開
x, y = y, x                    # 交換變數的 Pythonic 寫法（背後就是 tuple）
```

最後一個新手必踩的坑先標記：**單元素 tuple 要寫 `(1,)`**——逗號才是重點，`(1)` 只是數字 1 加括號。
...
tuple 和 list 一樣是**有序序列**，差別只有一個核心：**tuple 不可變（immutable）**——建立後不能新增、刪除、替換元素。

「不可變」衍生出三個特性（購物清單 vs 身分證欄位的差異）：

1. **可 hash**（若元素皆可 hash）→ 能當 dict key、放進 set（見 [hashable](07-hashable.md)）。list 不行。
2. **語意上表示「固定結構的紀錄」**：慣例是——list 放「同質、數量可變的集合」；tuple 放「異質、位置有意義的欄位」（如 `(name, age, email)`）。
3. **稍微更省記憶體、可作為常數**：不可變讓它更輕、更能安全共享。
