...
家裡電費暴增，你會怎麼辦？多數人直覺「一定是冷氣」，馬上換變頻——
結果元兇是老冰箱。**優化的第一守則：先量測，別猜**。
工程師對「哪裡慢」的直覺出錯率高得驚人，真正的瓶頸常在你沒想到的地方。

profiler 就是**分電表**：接上去跑一輪，每個函式吃掉多少時間一目瞭然。分兩種量法：

- **確定性 profiler（`cProfile`，內建）**＝**逐筆記帳**：每次函式呼叫都記錄，
  精確（次數、耗時全有），但記帳本身有開銷（程式會變慢）——適合開發時分析。
- **取樣 profiler（`py-spy` 等）**＝**不定時抽查**：每隔幾毫秒看一眼「現在在跑哪個函式」，
  用統計推估。便宜到可以掛在**正式環境**上，但看不到精確呼叫次數。

讀報告時盯兩個欄位：`tottime`（函式**自己**花的時間）與 `cumtime`（含它呼叫的下層）。
找瓶頸看 `tottime` 排行——通常呈現殘酷的 80/20：**八成時間花在兩成程式碼上**，
所以優化那一兩個熱點就夠，其他地方動了也是白動。

流程永遠是：**量測 → 找到熱點 → 只優化熱點 → 再量測驗證**。這章教你把這條流程跑順。
