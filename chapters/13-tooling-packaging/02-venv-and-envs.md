...
1. **套件隔離**：每個專案有獨立的套件空間（venv，見 [Part 1 venv](../01-getting-started/05-venv.md)）——避免專案間套件衝突。
2. **Python 版本管理**：不同專案可能要不同 Python 版本（A 用 3.11、B 用 3.12）——需要工具管理多個 Python 版本。

完整的環境管理 = **對的 Python 版本 + 隔離的套件空間**。傳統要組合多個工具（pyenv + venv + pip），現代工具（uv、poetry）把這些整合了。
