...
讓 LLM 應用**跑起來**很容易,讓它跑得**省錢又快**才是工程。
每次呼叫都按 token 計費、都有延遲、都有 rate limit——
不管好,帳單會嚇死你、使用者會等到走人。這章是 LLM 的**成本與效能管理手冊**。

先看懂**帳單怎麼算的**,才知道省在哪:

```text
成本 = 輸入 token × 輸入單價 + 輸出 token × 輸出單價
```

三個最大的省錢槓桿:

- **輸出比輸入貴得多**(Claude 輸出單價約是輸入的 5 倍)——
  所以**控制輸出長度**(設合理 `max_tokens`、要求簡潔)最直接有效。
- **選對模型**:不是每個任務都要動用最強的 Opus。
  簡單分類/抽取用便宜快速的 Haiku,難的推理才上 Opus——**同任務可省數倍**。
- **別塞太長的 context**:長脈絡每次都要重新計費,精簡輸入。

再看**兩個殺手級優化**:

- **Prompt caching(提示快取)**:如果每次請求都有一大段**固定的前綴**
  (系統指令、知識庫、範例),快取它——重複部分**大幅降價**、又加速。
  這是 LLM 版的[快取](../18-performance/04-caching.md),CP 值極高。
- **語意快取**:相似的問題直接回上次的答案(用 [embedding](06-embeddings-semantic-search.md) 判斷相似)。

加上**批次處理**(非即時任務用 Batch API 半價)、**串流降感知延遲**、
**重試與退避**應對 rate limit——這章把「能動的 demo」打磨成「養得起的產品」。
