...
檔案該怎麼擺？這其實是**家裡收納**的問題，而且只有兩種流派：

- **按物品類型收**：所有衣服進同一個衣櫃、所有書進同一個書櫃。
  對應程式：`models/`、`services/`、`api/`——同類技術放一起（**按層分**）。
- **按房間收**：每個房間自給自足，臥室有自己的衣櫃和書架。
  對應程式：`users/`、`orders/`——每個功能一包，內含自己的 model/service/api（**按功能分**）。

小套房按類型收就夠了；大宅子一定按房間——不然「整理小孩的東西」得跑遍全屋。
程式同理：**小專案按層分、大專案按功能分**（改一個功能的東西全在一個資料夾）。

除了這個大方向，這章還講幾個 Python 專案的實務標配：

- **src layout**：程式碼放 `src/` 下一層，逼你「以安裝後的樣子」測試，
  避開「測試不小心 import 到當前目錄」的經典坑。
- **`pyproject.toml`**：現代 Python 專案的單一設定入口。
- **import 邊界**：資料夾分好了，還得守住「誰能 import 誰」（別讓下層偷 import 上層）。

結構沒有唯一標準答案，但有共識的底線——這章給你一套可以直接抄的起手式。
