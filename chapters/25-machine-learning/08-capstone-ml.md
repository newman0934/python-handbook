...
這是 Part 25 的**畢業專題**:拿一份真實資料集,從頭到尾走完一個**專業級**
ML 流程,把前七章的每一環串成一條不出錯的生產線:

```text
載入+EDA → 切分(先鎖 test) → Pipeline(前處理+模型綁一起)
   → 交叉驗證選超參數 → 全 train 重訓 → test 上唯一一次評估 → 解讀
```

這章的靈魂是 **Pipeline**——它不只是「讓程式碼整潔」,更是**防止資料洩漏的保命裝置**。
把「標準化、編碼、模型」全綁進一個 Pipeline,交叉驗證時,
每一折的前處理參數都**只從該折的訓練部分學**,自動杜絕「測試資訊偷跑進訓練」。
手動前處理極容易在這裡翻車,Pipeline 幫你結構性地擋掉。

搭配 `GridSearchCV`/`RandomizedSearchCV` 做**超參數搜尋**:
系統化地試各種參數組合,用交叉驗證選最佳——而且全程不碰 test。
最後才用最佳參數在**全部訓練資料**上重訓,對 test **開封一次**給出誠實評分。

走完這章,你手上會有一個**可複用的 ML 專案模板**,
把「切分紀律、Pipeline 防洩漏、交叉驗證調參、選對指標」這些散落的原則,
組裝成一套肌肉記憶。往後 [Part 26 進階 ML](../26-advanced-ml/README.md)
接著講整體學習(隨機森林、XGBoost)與更強的模型——但這套流程骨架,終生適用。
