...
**引用計數**：CPython 為每個物件維護一個計數器，記錄「**有多少個引用指向這個物件**」。

- 每當一個新的名稱/容器/引用指向物件，計數 **+1**。
- 每當一個引用消失（名稱重新綁定、離開作用域、從容器移除），計數 **−1**。
- **當計數歸零（沒人指向它），物件立刻被回收**（記憶體釋放、呼叫 `__del__`）。

這是「一切皆物件」（見 [一切皆物件](01-everything-is-object.md)）的直接後果——每個物件的底層結構裡都有一個 `ob_refcnt` 欄位。特點是**即時**（歸零馬上回收，記憶體佔用可預測）且**簡單**。
