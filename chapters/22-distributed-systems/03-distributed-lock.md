...
單機搶資源,`threading.Lock` 就搞定([Part 9](../09-concurrency/README.md));
但**多台機器**的服務實例要搶同一筆訂單呢?行程內的鎖管不到別台機器——
需要一個大家都看得到的**公共預約牌**:掛在公共洗衣機上,
拿到牌的人才能用。實務上這塊牌子通常放 Redis(`SET key val NX EX 30`)。

牌子必須**自動失效**(TTL):否則拿牌的人當機,洗衣機被永久佔住(死鎖)。
但 TTL 恰恰埋下這個領域最著名的地雷:

> 實例 A 拿到牌(30 秒),中途**長時間卡住**(GC 暫停、網路抖動)——
> 牌子過期,實例 B 拿到牌開始用;A 醒來,**以為自己還持有鎖**,
> 也繼續操作——兩人同時在用洗衣機,鎖形同虛設。

關鍵領悟:**光有鎖不夠,資源端也要把關**。解法是 **fencing token**——
牌子帶**遞增流水號**(A 拿到 33 號、B 拿到 34 號),
資源端(資料庫)只接受**比已見過更大的號**:A 醒來拿著 33 號來寫,
被拒絕(已見過 34)。過期的鎖,寫不進去。

這章用 Redis 實作正確的鎖(拿鎖 NX+EX、**釋放要用 Lua 腳本**——
先驗「牌子還是我的」再刪,否則會誤刪別人的牌)、講 Redlock 的爭議、
以及最誠實的建議:**能用冪等或資料庫約束解決的,別用分散式鎖**
([冪等](06-idempotency.md)下下章登場)。
