...
你已經把 [task-api 容器化](../19-cloud-native/01-docker.md)(有了一個 Docker 映像)。
這章就是**臨門一腳**:把這個映像真正變成一個**線上跑、有網址、會自動擴縮的服務**。
主角是**無伺服器容器**平台——**Google Cloud Run** 與 **AWS ECS/Fargate**。

從本機映像到線上服務,通用就三步(記住這個節奏,各家雲只是換指令):

```text
1. Build   docker build -t task-api .     本機或 CI 建映像
2. Push    推到雲的映像倉庫(registry)     AWS: ECR / GCP: Artifact Registry
3. Deploy  平台拉映像、跑容器、給你一個網址
```

「無伺服器容器」的**無伺服器**是關鍵賣點:你**不用管底下的機器**——
不用開 VM、不用修 OS、不用手動擴容。流量來了平台**自動多開幾個容器**,
流量走了**自動縮回**(Cloud Run 甚至能**縮到零**——沒人用時一毛錢不花)。
你只負責交一個容器,其餘交給雲。這對一個 web 服務來說,是省心與省錢的最佳平衡。

這章帶你**實際把 task-api 部署到 Cloud Run**(與 ECS/Fargate 對照):
設定容器的埠、環境變數、記憶體/CPU、最小/最大實例數,
以及一個關鍵細節——**縮到零帶來的[冷啟動](05-serverless.md)**
(閒置後第一個請求要等容器啟動)該怎麼權衡。
從這章起,你的 task-api 就有一個真實的公開網址了。
