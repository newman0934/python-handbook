...
**一等公民（first-class citizen）** 指一個東西能享有「值」的所有待遇：
...
在 Python，**函式就是物件**（呼應 [一切皆物件](../10-cpython-internals/01-everything-is-object.md)、[函式](../02-fundamentals/08-functions.md)），完全享有這些待遇。`def greet(): ...` 建立一個函式物件並綁到名稱 `greet`——`greet` 只是個指向函式物件的名稱，可以像任何變數一樣操作。
