...
同一筆熱門資料，每秒被查一千次——每次都跑資料庫（幾十毫秒）？太浪費。

**快取**的直覺：櫃檯貼一張**便條**抄著答案（Redis，記憶體、亞毫秒），先看便條、沒有才進檔案室（DB）。

但快取的本質是一筆交易：**用「可能過時」換「快」**。於是核心難題永遠是——**資料改了，便條怎麼辦？**（這就是「快取失效」,電腦科學兩大難題之一。）

最常用的套路 **Cache-Aside（旁路快取）**,背下來：

```text
讀:先看便條 → 命中就回;沒中 → 查 DB → 抄一張便條 → 回
寫:更新 DB → 把便條「撕掉」(刪除,別更新——下次讀時自然重抄)
```

（為什麼是刪不是改？併發下「改便條」會有先後錯亂,「撕掉重抄」最穩——章內拆解。）

再配一張安全網:**TTL（存活時間）**——每張便條自帶過期時間,就算忘了撕,過期自動作廢,資料不會無限過時。

Redis 本體是「資料結構伺服器」:不只 string,還有 list、hash、sorted set(排行榜神器)、set——每種配典型場景,章內逐一過。
三個經典坑先掛號:**快取穿透、擊穿、雪崩**——名字嚇人,原理都簡單。
...
TTL 和「淘汰」是**兩件常被混為一談的事**：

- **TTL 過期**：時間到，這筆自己消失（**與記憶體用量無關**）。
- **淘汰（eviction）**：記憶體**滿了**，Redis 被迫踢掉一些 key 挪空間（**與時間無關**）。

記憶體是有限且昂貴的，所以要先告訴 Redis 上限、以及滿了要踢誰：

```text
maxmemory 2gb                  # 上限
maxmemory-policy allkeys-lru   # 滿了要踢誰
```

**這裡有個大坑：Redis 的預設是 `noeviction`——不踢任何人。** 記憶體一滿，
**寫入直接回錯誤**（`OOM command not allowed when used memory > 'maxmemory'`），
快取變成「只能讀不能寫」。**把 Redis 純當快取用時，一定要改成 `allkeys-lru` 或 `allkeys-lfu`**。

主要的策略：

| 策略 | 踢誰 |
|------|------|
| `noeviction` | **不踢，寫入報錯**（預設） |
| `allkeys-lru` | 所有 key 裡，踢**最久沒被用**的 |
| `allkeys-lfu` | 所有 key 裡，踢**最少被用**的（Redis 4.0+） |
| `allkeys-random` | 隨機踢 |
| `volatile-lru` / `volatile-lfu` / `volatile-random` | **只踢有設 TTL 的** key，規則同上 |
| `volatile-ttl` | 只踢有設 TTL 的，優先踢**最快過期**的 |

> ⚠️ `volatile-*` 的坑：它**只動有 TTL 的 key**。若你的 key 都沒設 TTL，記憶體滿了它**沒東西可踢**，
> 行為就退化成 `noeviction`（寫入照樣報錯）。

**LRU vs LFU：兩種賭注**

- **LRU（最久未用）** 賭的是「**最近用過的，近期還會再用**」——就是 [Part 18 `lru_cache`](../18-performance/04-caching.md) 那套「撕掉最久沒人看的便條」。
- **LFU（最少使用）** 賭的是「**常用的會繼續常用**」——它數的是**次數**，不是時間。

差別在哪？想像半夜跑了一支批次工作，把幾萬筆**冷門**資料掃過一遍。
**LRU 會被騙**——這些冷資料「剛剛才用過」，於是它把真正的熱門資料（白天的熱門商品）擠出去，
天亮後命中率崩盤。**LFU 不吃這套**：冷資料只被碰一次，次數低，照樣先被踢。

所以：**存取有明顯時間局部性（近期熱的就是熱的）→ LRU；有穩定的長期熱點、又常被批次/掃描打擾 → LFU。**
拿不定主意就 `allkeys-lru`（最通用）。

> 補充：Redis 的 LRU/LFU 都是**近似的**——它不維護一條完整的排序鏈（太貴），而是**取樣**幾個 key
> 挑最差的踢（`maxmemory-samples`，預設 5）。取樣越多越準、也越耗 CPU。這是「夠好就好」的務實取捨。
...
- **純當快取用就設 `maxmemory` + `allkeys-lru`（或 `allkeys-lfu`）**：預設的 `noeviction` 會在記憶體滿時
  **讓寫入直接報錯**，而不是踢舊資料。常被批次/掃描打亂熱點的場景選 LFU，其餘 LRU 最通用。
...
- **以為記憶體滿了 Redis 會自動踢舊的**：預設是 **`noeviction`——不踢，直接讓寫入報錯**
  （`OOM command not allowed...`），快取變成唯讀。當快取用一定要改 `allkeys-lru` / `allkeys-lfu`。
- **設了 `volatile-*` 卻沒給 key 設 TTL**：`volatile-*` **只踢有 TTL 的 key**；都沒 TTL 就沒東西可踢，
  行為退化成 `noeviction`，照樣寫入報錯。
- **半夜的批次掃描把熱點擠出快取**：LRU 會被「剛用過」騙走（冷資料剛被掃過），天亮命中率崩盤；
  這種場景用 **LFU**（看次數，不看時間）較耐打。
...
- 知道序列化用 JSON（別 pickle 不可信）、Redis 是記憶體儲存（記憶體上限、持久化 RDB/AOF）。
- **「Redis 記憶體滿了會怎樣？LRU 和 LFU 怎麼選？」**
  面試官想聽：「**預設 `noeviction`——不淘汰，寫入直接報 OOM 錯**，所以純當快取一定要設 `maxmemory` +
  `allkeys-lru`／`allkeys-lfu`。**LRU 賭「最近用過的還會再用」；LFU 賭「常用的會繼續常用」**——
  差別在**批次掃描**的場景：LRU 會被剛掃過的冷資料騙走、把熱點擠出去，LFU 看次數所以不吃這套。
  另外 Redis 的 LRU/LFU 都是**取樣近似**的（`maxmemory-samples`），不是精確排序——夠好就好的務實取捨。」
  加分：知道 `volatile-*` 只淘汰有 TTL 的 key，全都沒 TTL 時會退化成 `noeviction`。
