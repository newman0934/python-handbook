...
一般流程是「先寫程式、再補測試」。TDD 反過來——**先立靶,再射箭**：

**🔴 紅——立靶。** 先寫一個「描述你想要的行為」的測試。實作還不存在,跑起來當然失敗（紅燈）——這一步逼你在動手前想清楚:「**我到底要什麼?**」介面長怎樣?輸入輸出是什麼?

**🟢 綠——射中就好。** 寫**剛好讓測試通過**的最少程式。不預先優化、不加想像中的功能——只求綠燈。（甚至可以先 hardcode 回傳值——下一個測試自然會逼你寫真的。）

**🔵 重構——調整姿勢,靶保證沒歪。** 綠燈在手,放心整理程式:去重複、改命名。改壞了?測試立刻變紅——**安全網已經先架好了**。

然後回到紅,寫下一個測試。**每圈幾分鐘**,小步快跑、隨時綠燈。

TDD 買到的三樣東西：

1. **先想清楚再動手**——寫測試=先設計介面,埋頭亂寫的返工少了。
2. **不寫用不到的程式**——每行程式都是被某個測試「逼」出來的,沒有過度設計。
3. **測試覆蓋是天然的**——不是事後補交,是流程的一部分。

平衡話:TDD 是**紀律不是宗教**——探索性程式碼、一次性腳本不必硬套;但寫「行為明確的核心邏輯」時,它是把利器。本書的 [exercises 練習題](../../exercises/)（紅燈 stub → 你實作轉綠）正是 TDD 的節奏。
...
TDD 的核心是一個短循環——**Red-Green-Refactor（紅-綠-重構）**，先立靶再射箭：

1. **🔴 Red（紅）**：先寫一個**失敗的測試**——描述「你想要的行為」（立靶）。此時還沒有實作，測試當然失敗。
2. **🟢 Green（綠）**：寫**剛好讓測試通過**的最少程式——不多寫、不優化，只求綠燈（射中就好）。
3. **🔵 Refactor（重構）**：在測試保護下**改善程式**（去重複、改命名、優化結構）——測試確保重構沒弄壞行為（調姿勢，靶不歪）。

回到第 1 步，寫下一個測試。每圈很短（幾分鐘）——小步前進、隨時綠燈。

**關鍵**：先寫測試逼你先定義「要什麼」；只寫剛好通過的程式避免過度設計；重構在測試保護下安全進行。
