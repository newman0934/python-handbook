...
Python 給你兩種寫程式的模式，像做菜的兩種場景：

- **REPL：試味道的小碟子。** 打開 `python` 不加任何檔名，出現 `>>>` 提示符——輸入一行、立刻看到結果。適合**探索**：這個函式怎麼用？這段語法合法嗎？夾一口嚐嚐就知道。
- **`.py` 檔：寫下來的食譜。** 要留存、要重複執行、要交給別人的程式，寫成檔案，用 `python 檔名.py` 執行。

REPL 有一個「食譜模式沒有」的貼心行為，要先知道免得困惑：
在 REPL 打 `1 + 1`，它**自動印出** `2`；但在 `.py` 檔裡光寫 `1 + 1` **什麼都不會顯示**——檔案模式要自己 `print()`。
...
白話翻譯是：「**只有當這個檔案是被『直接執行』的時候，才做下面的事**」。
為什麼需要它？因為一個 `.py` 檔有兩種被使用的方式——被直接執行（當主角）、或被別人 import（當工具箱）。
這行就是在問：「我現在是主角，還是被借用？」——是主角才跑測試/示範程式碼，被 import 時保持安靜。
...
**REPL = Read–Eval–Print–Loop**——直譯器做四件事不斷循環：
...
關鍵在第 3 步：REPL 會**自動印出每個運算式的值**（透過該值的 `repr()`）。

這就是導讀說的「小碟子 vs 食譜」最大的差異：

- REPL 裡打 `1 + 1` → 直接看到 `2`（自動 Print）。
- `.py` 檔裡寫 `1 + 1` → 沒有任何輸出，要自己 `print(1 + 1)`。
