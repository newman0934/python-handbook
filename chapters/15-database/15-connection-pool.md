...
[DB-API 那章](11-db-api.md)說過：資料庫連線是「昂貴的電話線」——每次架線要 TCP 握手、認證，幾十毫秒就沒了。
每個請求都新架一條線再拆掉？效能直接被架線費吃光。

**連線池 = 共享單車站**：

- 站裡**預先架好 N 條線**（建好的連線）。
- 要查詢時**借一條**（checkout）——有空車直接騎，沒車就排隊等。
- 用完**還車**（checkin）——**線不拆**，回站裡等下一位。

架線費只付一次，之後全是重用。SQLAlchemy 的 Engine 內建這個站，你其實一直在用。

真正要動腦的是**站裡放幾台車（池大小）**：

- **太少**：尖峰時大家排隊等車——延遲飆高、逾時。
- **太多**：資料庫端吃不消——每條連線在 DB 那頭也佔資源，PostgreSQL 預設上限才 100 條。
  而且要算**總帳**：你的服務開 8 個實例 × 每實例 20 條 = 160 條——超標了！

一條換算心法：**所有實例的池加總,必須小於資料庫的連線上限**（還要留餘裕給別人）。

無伺服器/大量實例的場景池怎麼辦（外部 pooler:PgBouncer/RDS Proxy）、借了不還的洩漏怎麼防（with 歸還）——章內展開。
