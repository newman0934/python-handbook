...
分析師拿到的資料常常「形狀不對」,兩種毛病、兩帖藥:

**毛病一:欄位散在多張表**——藥是 **`merge`(合併)**。
和 [SQL JOIN](03-sql-joins.md) 一模一樣:按鍵對名單,`how=` 決定對不上的怎麼辦
(inner 消失/left 留白 NaN)。SQL 那兩個事故——INNER 偷吃列、一對多膨脹——
pandas 照樣會發生,而且 pandas 給了體檢工具:
`merge(..., validate="one_to_many")` 直接宣告預期關係,不符就報錯;
`indicator=True` 讓每列標明「來自左/右/兩邊」。**用起來,別裸奔**。

**毛病二:長寬不對**——藥是**重塑(reshape)**。同一份資料有兩種擺法:

- **長格式＝流水帳**:每筆觀測一列(`城市, 月份, 營收`)——
  **程式最愛**(groupby、畫圖、建模都吃這種),也是 tidy data 原則的形狀。
- **寬格式＝報表**:城市當列、月份攤開當欄——**人眼最愛**,老闆要看的樣子。

互轉就兩個函式:`pivot_table`(長→寬,做報表)、`melt`(寬→長,把欄名壓回值)。
口訣:**分析時轉長,呈現時轉寬**。

這章加上 `concat`(疊資料:同結構的多月份檔案直向堆起來),
湊齊「拼資料」的全部工具——下一章 [EDA](08-eda.md) 就要用它們組出乾淨的分析底表。
