...
傳統寫程式,是**你把規則寫死**:if 金額 > 10000 就標記可疑。
但有些規則**寫不出來**——請你用 if-else 寫「這張圖是不是貓」試試?
貓有各種顏色、角度、品種,你窮舉不完。
...
```text
傳統程式:  規則 + 資料 → 答案     (人寫規則)
機器學習:  資料 + 答案 → 規則     (機器自己歸納出規則)
```

給機器看一萬張標好「是貓/不是貓」的圖,它**自己歸納**出辨識規則(模型),
之後對新圖做預測。這就是 [Part 17 教小孩認水果](../17-data-science/08-machine-learning-intro.md)
的正式版——只是這個 Part 要把每個環節講到能實戰、能面試。

三種學習類型,用「有沒有標準答案」分:

- **監督式(supervised)**:資料**帶答案**(標籤)。學「輸入→輸出」。
  再細分**分類**(答案是類別:貓/狗)與**回歸**(答案是數字:房價)。**最常見**。
- **非監督式(unsupervised)**:資料**沒答案**,自己找結構(把相似的分群、降維)。
- **強化學習(reinforcement)**:在環境裡試錯,靠獎懲學習(下棋、機器人)。

最重要的一句話先立起來,整個 Part 都繞著它:
**ML 的目的不是背熟訓練資料,是「泛化」到沒見過的新資料**——
會背考古題不算學會,考場上答對新題才算。
