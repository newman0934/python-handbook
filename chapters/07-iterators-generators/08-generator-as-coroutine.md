...
- **產出（output）**：`yield value` 把 value 送給外部（`next()`/`for` 取得）。
- **接收（input）**：`x = yield` 讓生成器**暫停並等待**，外部用 `send(v)` 送值進來，`v` 就成為 `yield` 運算式的值（賦給 `x`）。

當你用「接收」這一面，生成器就從「產出序列的 iterator」變成「能接收外部輸入、逐步推進的協程」。三個控制方法：
