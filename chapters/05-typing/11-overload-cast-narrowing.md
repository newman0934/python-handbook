...
- **型別窄化（narrowing）**：mypy 根據控制流（`if`、`isinstance`、`assert`…）在特定分支「縮小」變數的型別。這是**自動**的，是 Union/Optional 好用的基礎。
- **`@overload`**：對**同一個函式**宣告多組「輸入型別 → 輸出型別」的對應，讓型別檢查器依實際引數推斷正確回傳型別。
- **`cast(T, value)`**：**告訴** mypy「把這個值當成 T」——不做任何執行期轉換或檢查，純粹覆寫型別檢查器的判斷。逃生艙，慎用。
