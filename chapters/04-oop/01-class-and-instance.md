...
- **class（類別）**：定義「這種物件長什麼樣、能做什麼」的藍圖——有哪些屬性、哪些方法。
- **instance（實例）**：依藍圖建立出來的具體物件，各自有獨立的狀態。
...
呼應「一切皆物件」（見 [一切皆物件](../10-cpython-internals/01-everything-is-object.md)）：**class 本身也是物件**（`type(Dog)` 是 `type`），可以被賦值、傳遞、當參數——這是後面 metaclass 的伏筆。
