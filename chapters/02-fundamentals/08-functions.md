...
函式的基本概念兩邊一致,但有幾個「前端習慣」到 Python 要調整:

| | Python | JavaScript |
|---|--------|-----------|
| 預設參數 | `def f(x=10):` | `function f(x = 10) {}` |
| 具名引數 | `f(timeout=5)`（語言支援） | 慣例包一個 options 物件 |
| 函式多載（overload） | **沒有**（用預設值 / `*args` / `@singledispatch`） | 也沒有（靠參數判斷） |
| 回傳多值 | `return a, b`（回 tuple） | `return [a, b]` / `return { a, b }` |

一句話:最有感的差異——Python 有真正的**具名引數**（`f(timeout=5)`）,不用像前端那樣包 options 物件;
而 `return a, b` 直接回一個 tuple,呼叫端能 `x, y = f()` 解構,很順。
