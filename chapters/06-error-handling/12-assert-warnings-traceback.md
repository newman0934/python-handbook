...
例外家族之外，還有三個相關小工具。用三種「提示牌」分清楚：

**1. `assert`——工地內部的自檢標語。**
「此處梁柱應該垂直」——寫給**自己人**看的內部檢查：

```python
assert total >= 0, "總額不該為負"   # 照理說一定成立；不成立代表有 bug
```

關鍵定位：assert 驗的是「**不該發生**」的內部假設（防自己的 bug），**不是**驗證使用者輸入。
為什麼？因為它是可以拆掉的——`python -O` 執行時所有 assert **直接被跳過**。拿它擋壞輸入，等於用「驗收後會拆的鷹架」當「正式圍欄」。輸入驗證請老實 `raise ValueError`。

**2. `warnings`——「前方施工注意」告示。**
非致命、但該知道的事（這個 API 要棄用了、這個用法可疑）——**提醒但不擋路**，程式繼續跑。
你用舊版套件時看到的 DeprecationWarning 就是它。

**3. `traceback`——行車記錄器。**
平常例外爆炸時印的那串回溯,就是 traceback。這個模組讓你**程式化地**調閱它:格式化成字串寫進 log、上報到錯誤追蹤系統——事後調查全靠它。

一句定位:**assert 防自己、warnings 提醒別人、traceback 留存證據。**
...
- **`assert`**：驗證「**照理說一定成立**」的內部假設——工地自檢標語。條件為假時拋 `AssertionError`。
  用於**開發期抓 bug**；**不是**驗證外部輸入（`python -O` 會把 assert 全部移除——鷹架驗收後會拆）。
- **`warnings`**：發出「**非致命、但該注意**」的訊息（棄用、可疑用法、未來會變）——施工告示。程式繼續執行，只是提醒。
- **`traceback`**：把例外的回溯資訊**程式化地**取得、格式化、記錄——行車記錄器。用於 log、錯誤回報、除錯工具。
