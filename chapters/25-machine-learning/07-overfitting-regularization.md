...
泛化最大的敵人有個名字:**過擬合(overfitting)**——
模型不是「學會規律」,而是「**背熟了訓練資料**」,連裡面的雜訊、巧合都背下來。
症狀很好認:**練習題滿分,考試慘敗**(訓練分數高、測試分數低)。

它的反面是**欠擬合(underfitting)**:模型太笨,連練習題都做不好(兩邊都差)。
兩者背後是同一個權衡——**偏差-變異(bias-variance)**:

- 模型**太簡單**→抓不住規律(高偏差,欠擬合)。
- 模型**太複雜**→對訓練資料太敏感、把雜訊當規律(高變異,過擬合)。
- 目標:**剛剛好的複雜度**——夠聰明學到規律,又不至於死背雜訊。

用一個比喻:準備考試,只背一條「多讀書就會考好」是欠擬合(太粗);
把整本參考書一字不漏背下來(含出版社的錯字)是過擬合——
你要的是**理解概念**,那個中間地帶。

對付過擬合有兩大武器,這章詳解:

- **正則化(regularization)**:給「複雜」課稅——在誤差裡加一項懲罰,
  逼模型別把權重搞得太大太極端(L1 會把不重要的特徵權重壓成 0,順便做特徵選擇;
  L2 讓權重平滑變小)。翻成白話:**逼模型「說重點」,別鑽牛角尖**。
- **更多資料 + 交叉驗證**:資料越多越難背完;交叉驗證幫你**及早發現**過擬合。

診斷方法很機械:畫**學習曲線**,看訓練與驗證分數的差距——
差距大就是過擬合的鐵證。這是實務調模型的日常動作。
