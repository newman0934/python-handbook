...
單機時代,資料庫自增主鍵(1、2、3……)就是唯一 ID。
但幾十台伺服器、每秒百萬筆時,「排隊跟同一個櫃檯拿號碼牌」就是瓶頸;
各拿各的又會重號。**分散式 ID 生成**要的是:全域唯一、大致遞增、不用互相打電話協調。

**Snowflake** 的解法漂亮得很:把一個 64 bit 整數**切成三段刻進去**——

```text
| 41 bits 時間戳(毫秒) | 10 bits 機器 ID | 12 bits 同毫秒序號 |
```

翻成白話:號碼牌上直接印著「**幾點幾分幾秒毫秒 + 哪個櫃檯發的 + 該毫秒第幾張**」。
三個資訊天然不撞:不同毫秒→時間戳不同;同毫秒不同機器→機器 ID 不同;
同毫秒同機器→序號遞增(一毫秒可發 4096 張,一秒 400 萬張)。
**零協調、本機生成、微秒級速度**,還附贈「ID 大致按時間排序」
(對資料庫 B+tree 插入友善,呼應[索引章](../15-database/05-index-internals.md))。

面試常追問的兩個坑,先備好答案:
**時鐘回撥**(NTP 把機器時間往回調→可能重號;解法:拒發並等待、或記錄上次時間戳)、
**機器 ID 怎麼分配**(靜態設定、或向 ZooKeeper/etcd 註冊)。

替代方案的取捨也要會比:UUIDv4(零協調但無序又長,B+tree 插入差)、
資料庫號段模式(批發一段號碼回來慢慢發)、UUIDv7(時間有序的新標準,漸成主流)。
這章用 Python 實作一個可跑的 Snowflake,含時鐘回撥防護。
