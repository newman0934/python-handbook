...
「在我機器上明明能跑！」——因為你只帶了**菜刀**（程式碼）去別人的廚房,
但那道菜還依賴你家的瓦斯爐火力、特定品牌的醬油、鍋子的尺寸（Python 版本、系統函式庫、依賴）。
換個廚房,菜就走味。

Docker 的解法很暴力：**把整個廚房打包成一個貨櫃**。菜刀、鍋具、瓦斯、醬油全裝進去,
運到哪裡,打開就是同一個廚房、做出同一道菜。

三個核心詞,用物件導向秒懂：

- **image（映像檔）＝類別**：唯讀的「應用＋完整環境」範本,由 `Dockerfile`（建廚房的施工圖）建成。
- **container（容器）＝物件**：image 跑起來的實例,同一個 image 能開出多個容器。
- **layer（層）＝千層蛋糕**：`Dockerfile` 每條指令烤一層,**沒變的層直接重用**（快取）——
  所以「先 COPY 依賴清單裝套件、再 COPY 程式碼」的順序至關重要:改程式碼不會重裝依賴。

「這跟虛擬機（VM）差在哪？」——VM 是**每戶自蓋整棟房**（連地基/kernel 都自己一份）,肥重、開機慢;
容器是**合租共用地基**（共享宿主機 kernel）,只隔離各自的房間（行程與檔案系統）——輕量、秒啟。

這章從 `Dockerfile` 寫法、層快取、到跑起一個 FastAPI 服務,一步步動手。
