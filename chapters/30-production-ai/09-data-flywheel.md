...
到這裡,你的 LLM 服務已經[能部署](02-serving-llm-apps.md)、[可靠](03-reliability.md)、
[看得見](04-observability.md)、[守得住](06-guardrails.md)。
最後一塊,也是讓產品**越用越強**的引擎:**資料飛輪(data flywheel)**。

飛輪的意思是:**用得越多 → 資料越多 → 產品越好 → 更多人用**——
一個會自我加速的正向循環。這是頂尖 AI 產品拉開差距的真正護城河
(不是模型本身,而是這個持續改進的閉環)。
...
```text
1. 部署   → 服務真實使用者(ch02)
2. 收集   → 記錄互動 + 使用者回饋(ch04 可觀測性 + 讚/倒讚等訊號)
3. 挖掘   → 從線上撈出失敗案例(倒讚、重複追問、低信心、拒答)
4. 整理   → 補上正確答案,把它們變成新的評估案例
5. 改進   → 擴充評估集 → 優化 prompt/RAG /(必要時)微調
6. 驗證   → 用 eval 閘門(ch07)+ A/B(ch08)確認真的變好 → 回到 1
```

關鍵洞見:**你的失敗案例,是最珍貴的資產**——
每一個使用者不滿意的回答,都是一個「產品該往哪裡改」的免費指引。
系統性地收集它們、變成評估集、驅動改進,你的產品就會**自己往上爬**。

這章也談 **fine-tune(微調)何時才值得**——
多數時候,把 prompt 和 RAG 做好就夠了,微調是最後手段(貴、慢、要資料量);
以及怎麼從飛輪累積的資料,判斷「現在該微調了嗎」。
這是把 LLM 產品從「上線」推向「持續領先」的最後一哩。
