...
> 若要伺服器渲染 HTML（而非純 API），用 Jinja2 模板——把資料填進 HTML 樣板。靜態檔（CSS/JS/圖片）則直接提供。但要注意 XSS：模板自動跳脫是防護，別關掉它。
...
**模板引擎** 把「HTML 樣板 + 資料」渲染成完整 HTML：

- **樣板（template）**：含**佔位符**與**控制邏輯**的 HTML（`{{ variable }}`、`{% for %}`）。
- **上下文（context）**：填入的資料（dict）。
- **渲染（render）**：把資料填進樣板 → 完整 HTML。

**Jinja2** 是 Python 最流行的模板引擎（Flask 內建、FastAPI 可配）——語法簡潔、功能強、且**預設自動跳脫 HTML**（防 XSS，見下）。
