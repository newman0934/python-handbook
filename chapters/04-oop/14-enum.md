...
`Enum` 定義一組**具名的、唯一的常數成員**。每個成員有：

- **name**：成員名稱（`Color.RED.name` → `'RED'`）。
- **value**：對應的值（`Color.RED.value` → `1`）。

Enum 成員是**單例**——`Color.RED is Color.RED` 永遠成立，所以能安全用 `is` 比較。Enum 底層由 metaclass 實作（見 [metaclass](13-metaclass.md)），這讓它有「不可實例化為新成員、成員唯一、可迭代」等特殊行為。
