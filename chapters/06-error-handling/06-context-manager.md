...
你天天在**用** `with`(`open`、鎖、連線、`pytest.raises`),這章講**何時該自己寫一個**:
當你有一對**成對出現的 setup / teardown**,而且想保證「不管中途有沒有出錯,收尾一定執行」時。經典:

- **資源管理**:自己包資料庫連線/交易(進入開交易、離開時 commit 或出錯 rollback)、開關檔案/socket。
- **暫時性狀態的切換與還原**:暫時切工作目錄、暫時改設定/環境變數、計時(進入記時間、離開印耗時)。
- **取得 / 釋放**:鎖、號誌、進度條 spinner 的開與關。

訊號:當你發現自己在寫 **`try ... finally: 收尾`,而且同樣的收尾在多處重複**,
就把它包成 context manager。多數情況用下一章的 `@contextmanager` 寫最省,
要管理多個資源或複雜狀態時才寫完整的 `__enter__`/`__exit__` 類別。
