...
一般資料庫（PostgreSQL、MySQL）是 **client-server 架構**：一個獨立的 DB 伺服器行程，你的程式透過網路連上去。SQLite 是 **嵌入式（embedded）**：**資料庫引擎就是一個函式庫，直接跑在你的行程內，讀寫一個本地檔案**——沒有伺服器、沒有網路。
...
SQLite 的關鍵限制是**寫入併發**：同一時間只允許一個寫入者（寫時鎖整個資料庫）。讀可以併發，但高併發寫入場景（多使用者同時寫）該用 client-server 資料庫。
