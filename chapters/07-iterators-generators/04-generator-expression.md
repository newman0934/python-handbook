...
[Part 2 的推導式](../02-fundamentals/13-comprehensions.md)你已經會了。現在只改**一對括號**，性質完全改變：

```python
[x * x for x in range(10**9)]   # 方括號：立刻印好「整本書」——十億個數，記憶體爆炸
(x * x for x in range(10**9))   # 圓括號：請來一位「說書人」——講到哪算到哪，幾乎零記憶體
```

方括號是**成品**（真的建出整個 list）；圓括號是**[生成器](03-generator.md)**（惰性，一次產一個）——不用寫 `def` 和 `yield`，一行就得到說書人。

最常見、也最漂亮的用法：**餵給「吃一遍就好」的函式**：

```python
sum(x * x for x in nums)        # 當函式唯一引數時,連圓括號都可省
any(u.is_admin for u in users)
max(len(w) for w in words)
```

這些函式本來就只需要「逐一看過」——給它成品 list 是浪費（建了整個 list 只為看一遍就丟）。

選擇心法一句話：

> **結果要重複用、要索引、要 len → 方括號（成品）；只走一遍就丟 → 圓括號（說書人）。**

別忘了說書人的通病:**只能講一輪**——迭代完就空了,要再用得重新建一個。
...
生成器表達式的語法和推導式幾乎一樣，只差外層括號——但性質完全不同：

- **`[x for x in it]`** → list 推導式，**立刻**建出整個 list（印好整本書，全部佔記憶體）。
- **`(x for x in it)`** → 生成器表達式，回傳**生成器**（說書人），惰性產值（一次一個）。

它是[生成器](03-generator.md)的「表達式版」——不必寫 `def`/`yield`，一行得到一個惰性生成器。

本質完全是生成器：**一次性、省記憶體、可表示大序列**——以及只能遍歷一輪。
