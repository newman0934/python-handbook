...
RAG 有**一大堆旋鈕**:[chunk 多大](02-chunking-strategies.md)、重疊多少、
撈幾段(top_k)、[混合檢索的權重](03-hybrid-retrieval-rerank.md)、
用哪個模型……調了一個,你**怎麼知道是變好還是變壞?**
憑感覺看幾個問答,和 [Part 28 講的 LLM 評估](../28-llm-genai/09-evaluation.md)一樣不可靠。
這章教你科學地量 RAG。

關鍵洞見:RAG 會出錯,但**錯在哪兩層要分開量**,否則你會亂調:

**第一層——檢索對不對(撈到相關資料了嗎)?**

- **Recall@k**:答案需要的片段,**有沒有被撈進**前 k 個裡?
  這是 **RAG 的地基**——該撈的沒撈到,模型再強也是巧婦難為無米之炊,後面全白搭。
- **Precision@k**:撈回來的有多少是真相關(別塞一堆雜訊進 context)。

**第二層——生成對不對(有了資料,答得好嗎)?**

- **忠實度(faithfulness)**:答案是否**忠於撈回的資料**,有沒有[幻覺](../28-llm-genai/01-llm-fundamentals.md)、
  自己加料?——RAG 最在意的指標。
- **答案相關性**:有沒有答到使用者真正問的問題。

**為什麼要分層?** 因為「答錯了」有兩種病因,治法完全相反:
撈錯資料(改 chunking/檢索)vs 撈對了卻答歪(改 prompt/模型)。
分層量測才能**對症下藥**。這章教你用 RAGAS 之類的工具建 RAG 評估集、
自動打分,把調參從「玄學」變成「數據驅動」——這是做出可靠 RAG 的必經之路。
