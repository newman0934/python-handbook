...
mypy 工程化的核心是**漸進式導入（gradual adoption）**——呼應漸進式型別（見 [為什麼型別註記](../05-typing/01-why-type-hints.md)）：

- **新專案**：直接 `strict = true`，從一開始就寫好型別。
- **既有專案**：不可能一次補完——**逐步收緊**：先讓 mypy 能跑（忽略舊模組），再逐個模組開啟嚴格檢查，最終全專案 strict。

關鍵是「**別讓完美擋住開始**」——先讓 mypy 跑起來、擋住新的型別錯誤，再慢慢清理舊的。
