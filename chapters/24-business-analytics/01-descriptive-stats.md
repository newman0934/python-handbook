...
[EDA](../23-data-analysis/08-eda.md) 時你敲了 `describe()`,吐出一排數字——
這章教你**讀懂它們在說什麼**。描述一份資料,問三個問題就夠:

**1. 典型值在哪(集中趨勢)?**

- **平均數**＝把所有錢平分給每個人——**一個郭台銘走進小吃店,全店平均變億萬富翁**,
  受極端值影響大。
- **中位數**＝排隊站中間那個人——**穩健**,不理會兩端的怪咖。
- 口訣:**資料一偏(薪水、房價、消費),看中位數別看平均**——平均會說謊。

**2. 散得多開(離散程度)?**

同樣平均 60 分,一班全考 60、另一班半數 30 半數 90——天差地遠。
**標準差**就是量「大家離平均多遠」的尺:小＝整齊、大＝參差。

**3. 長什麼形狀(分布)?**

- **偏態(skew)**:尾巴拖左還是拖右——收入資料典型右偏(少數高收入拖長右尾)。
- **常態分布**＝鐘形曲線:很多統計方法的預設前提([假設檢定](03-hypothesis-testing.md)會用到)。

還有一個救命概念——**離群值(outlier)**:是真實的大戶,還是打錯的資料?
這章教你用 IQR、標準差法揪出它們,並決定「留、修、還是剔」。
統計不是背公式,是**替一堆數字選對的摘要方式**——選錯了,後面全歪。
