...
看懂了 [dict 的號碼機](04-dict.md)，set 一句話就懂：

> **set 就是「只有 key、沒有 value」的 dict**——只把名字放進置物櫃，不掛東西。

於是它繼承了雜湊表的全部好處：

- **「在不在」問得飛快**：`x in s` 平均 O(1)（號碼機直接定位）。同樣的問題問 list 要逐個翻（O(n)）——**大量成員檢查時，把 list 換成 set 常常是百倍加速**。
- **自動去重**：同一個名字放兩次，櫃子裡還是只有一份。`set(些什麼)` 是一行去重的慣用法。
- **元素必須不可變**：跟 dict 的 key 同理——會變的東西會在櫃子裡迷路。

還附贈數學課的集合運算，做「名單比對」超好用：
...
兩個提醒：**set 是無序的**（別依賴遍歷順序——它連 dict 3.7 的保序都沒有）；
需要「不可變的 set」（例如要放進另一個 set、當 dict key）用 **frozenset**。

## 🔗 前端對照

Python `set` 就是 JavaScript 的 `Set`——無序、不重複的集合。但 Python 的集合運算更完整:

| 操作 | Python | JavaScript |
|------|--------|-----------|
| 建立 | `{1, 2, 3}` | `new Set([1, 2, 3])` |
| 加入 | `s.add(v)` | `s.add(v)` |
| 是否包含 | `v in s` | `s.has(v)` |
| 交集 | `a & b` | 手動：`[...a].filter(x => b.has(x))` |
| 聯集 | `a \| b` | `new Set([...a, ...b])` |
| 差集 | `a - b` | 手動 |

一句話:概念一樣,但 **Python 內建 `&` `\|` `-` 集合運算子**,寫交集 / 聯集 / 差集非常直覺;
JS 的 `Set` 傳統上要自己用展開或迴圈組（很新的 JS 才有 `intersection` 等方法）。
...
set 底層和 dict 一樣是**雜湊表**，只是「只存 key、不存 value」——只放名字、不掛東西的置物櫃。因此：
...
- **無序**：set **不保證任何順序**（不像 dict 3.7 起保留插入順序）——別依賴 set 的遍歷順序。
