...
**PEP 659：Specializing Adaptive Interpreter（專用化適應性直譯器，3.11）** 的核心想法：

- 一般的 bytecode 指令是**通用的**——例如 `BINARY_OP +` 要能處理「int+int、str+str、list+list、自訂 `__add__`」等所有情況，每次都要檢查型別、分派——這是動態型別的開銷。
- 但實際執行時，**同一個指令位置往往反覆處理同一種型別**（一個 `+` 在迴圈裡可能一直是 int+int）。
- **適應性直譯器**觀察執行：當它發現某指令總是處理同型別，就把該指令**替換成專用的快速版本**（如 `BINARY_OP_ADD_INT`——省去型別檢查、直接做整數加法）。若之後型別變了，再退回通用版本（deoptimize）。

這叫**專用化（specialization）**——在執行期「適應」實際的型別模式，把熱點指令換成快速路徑。這不是 JIT（沒編譯成機器碼），而是「更聰明的 bytecode 直譯」。
