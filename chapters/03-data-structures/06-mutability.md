...
前面各章一直出現「可變 / 不可變」。這章把這條**貫穿整個 Python 的主線**一次收攏。

先把定義說人話：

- **可變（mutable）**：這個物件**本身**可以被動手術——list 可以 append、dict 可以加 key。手術後還是同一個物件（同一個身分證字號 `id`）。
- **不可變（immutable）**：物件出生就定型——str、int、tuple。所有「修改」其實都是**做一個新物件**，然後把[便利貼](../02-fundamentals/01-dynamic-typing.md)改貼過去。

一個重點常被搞錯：**可變性是「物件」的性質，不是「變數」的性質**。
沒有「可變的變數」這回事——變數永遠只是便利貼；會不會變的，是便利貼底下的那個東西。

為什麼這是主線？因為它默默決定了四件大事（每件都是真實 bug 的來源）：

1. **能不能當 dict key / 進 set**——不可變才可以（[hashable](07-hashable.md)）。
2. **函式會不會改到你的資料**——傳進去的是可變物件，函式裡動手術，外面就變了。
3. **`+=` 的行為分裂**——對 list 是原地手術（別名都看得到），對 tuple/str 是做新物件換綁。
4. **可變預設參數的地雷**——[Part 2](../02-fundamentals/09-parameters-args-kwargs.md) 預告過的頭號陷阱。

判斷心法一句：**問「改的是同一個物件，還是做了新物件換貼便利貼？」**——`id()` 可以當測謊機。
...
**可變性（mutability）是「物件」的屬性，不是「變數」的屬性**（呼應[名稱綁定](../02-fundamentals/01-dynamic-typing.md)——變數只是便利貼）：

- **不可變（immutable）**：建立後內容不能改。任何「修改」都是**建立新物件**（換貼便利貼）。
- **可變（mutable）**：可以原地（in-place）改變內容——動手術，物件的 `id` 不變。
...
> 問「這個操作是**改了同一個物件**（id 不變），還是**做了新物件並換綁**（id 變了）？」
