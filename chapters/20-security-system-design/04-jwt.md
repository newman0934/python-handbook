...
遊樂園的**防偽手環**:入園時售票處給你戴上,上面印著你的編號、票種、有效期,
並蓋了**只有園方蓋得出來的防偽章**。之後每個設施的工作人員**看一眼手環就放行**——
不用打電話回售票處查你是誰。

這就是 JWT:`header.payload.signature` 三段,
payload 印著你的資訊（user_id、role、過期時間 exp）,
signature 是伺服器用密鑰蓋的防偽章。**驗章不用查資料庫**——無狀態、好擴展,
這是它在分散式系統紅起來的原因。

但手環比喻同時暴露三個致命誤解,面試與實戰都在這裡翻車:

1. **手環內容人人可讀**:payload 只是 Base64URL **編碼**,不是加密——
   任何人 decode 就能看。**別把敏感資料放進 JWT**。簽章保證的是「沒被竄改」,不是「看不到」。
2. **到期前收不回來**:手環發出去就在客人手上,你沒法遠端銷毀——
   JWT 的**撤銷（revocation）很難**。解法:存活期設短（15 分鐘)＋
   用 refresh token 換新,要立即封鎖就得引入黑名單(又變有狀態,取捨)。
3. **驗章要挑演算法**:`alg: none` 攻擊、HS256/RS256 混淆攻擊——
   驗證時**必須指定允許的演算法白名單**,別讓 token 自己說了算。

這章用 PyJWT 實作簽發/驗證/刷新整條流程,並逐一重現這三個攻擊與防法。
