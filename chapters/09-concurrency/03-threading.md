...
**執行緒（thread）** 是同一個行程內的多條執行流，**共享同一份記憶體空間**（全域變數、物件都共用）。這帶來：

- **優點**：建立輕量、切換快、共享資料方便（不必複製或序列化）。
- **缺點**：共享記憶體 → **競態條件（race condition）**——多執行緒同時讀寫同一資料會出錯（見下）。

回顧 [GIL](02-gil.md)：CPython 的執行緒**無法並行 CPU 運算**（GIL），但**能加速 I/O**（等待時釋放 GIL）。所以 `threading` 的正確用途是 **I/O 密集**任務。
