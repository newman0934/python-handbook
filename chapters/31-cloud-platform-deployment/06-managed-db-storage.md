...
前幾章的容器和函式都是**無狀態**的——它們可以隨時被砍掉重開
(這正是能自動擴縮的前提)。但這帶出一個問題:**那資料放哪?**
使用者、任務、上傳的檔案——**狀態必須放在會被砍掉的容器「之外」**,
放進雲的**託管服務**裡。這章講雲上狀態的三個去處。

三類儲存,各有各的定位,別放錯:

- **託管關聯式資料庫(AWS RDS / GCP Cloud SQL)＝正式檔案室**:
  結構化、要交易(ACID)、要 SQL 關聯查詢的**核心業務資料**(users、tasks)。
  雲幫你顧備份、修補、備援——你不用再自己維運一台 [PostgreSQL](../15-database/README.md)。
  特性:常駐、相對貴、要**連線池**(呼應 [Part 15 連線池](../15-database/15-connection-pool.md))。
- **物件儲存(AWS S3 / GCP Cloud Storage)＝無限大的倉庫**:
  存**檔案**——圖片、上傳附件、備份、靜態資源。便宜到誇張、幾乎無限、高耐久。
  規則:**別把檔案存進資料庫或容器磁碟,存 S3**,DB 只存一個指向它的 URL。
- **託管快取/NoSQL(ElastiCache/Redis、DynamoDB)＝櫃檯便條與號碼牌**:
  [Redis](../15-database/18-redis.md) 做快取/session、DynamoDB 做超高吞吐的鍵值存取。

一個上雲的關鍵心態轉變貫穿全章:**運算(compute)與狀態(state)徹底分離**——
容器/函式是「用完即丟」的臨時工,資料則安穩地待在託管服務裡。
唯有如此,你的服務才能自由地擴縮、重啟、換版而不丟資料。
這章帶你把 task-api 的資料庫接上 Cloud SQL/RDS、把檔案接上物件儲存,
並處理雲上資料庫特有的課題:連線池、備份、與私有網路連線。
