...
> Python 沒有 `private`/`protected` 關鍵字——它用「約定」（單底線）與「name mangling」（雙底線）達成封裝。理解這是「君子協定」而非強制，是 Python 封裝哲學的核心。
...
Python 的封裝靠**命名約定**傳達「這東西是不是給外部用的」，但**大多不強制**：
...
核心精神：**單底線是「請不要碰」的禮貌提示，雙底線是「避免命名衝突」的機制**——兩者都不是 Java 那種強制存取控制。
