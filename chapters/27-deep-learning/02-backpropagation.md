...
[上一章](01-neural-network-basics.md)的網路,參數是**隨機**的——預測純屬亂猜。
那它到底**怎麼「學會」**?這章就是深度學習的心臟:
**梯度下降 + 反向傳播**。搞懂這一章,你就懂了所有神經網路訓練的原理。

**梯度下降 = 蒙眼下山。**
你站在山上(高損失)想走到谷底(低損失),但眼睛蒙著。
唯一能做的:用腳感覺**哪個方向最陡**,往下坡踏一小步,重複——
最終走到谷底。「最陡的方向」在數學上就是**梯度**,
「一小步」的大小就是**學習率(learning rate)**:
太大會一步跨過山谷來回彈跳,太小則慢得走不到——這是最關鍵的旋鈕。

問題來了:網路有幾百萬個參數,**每個參數該往哪調、調多少**(每個的梯度)怎麼算?
逐一去試會算到天荒地老。答案是**反向傳播(backpropagation)**——

它其實是你**微積分學過的連鎖律(chain rule)**的大規模應用:
先**前向**算出預測與損失,再**從輸出往回**一層層套連鎖律,
一次高效算出**所有參數的梯度**。像追究一個結果的責任:
從最終誤差往回追,算出「每一層、每個參數該為這個誤差負多少責」,
據此各自微調。

這章用最小的網路**手把手算一遍反向傳播的每個數字**——
繁瑣,但這是唯一能讓你「真懂」而非「聽過」深度學習的方式。
下一章把它組成完整訓練迴圈,之後框架([ch04](04-frameworks.md))會幫你自動做這一切,
但親手算過一次,你才知道框架在背後做什麼。
