...
- **JSON ＝純文字信件**:拆開只有字、數字、清單——內容再惡意,它**沒有手腳**,
  不會自己動。對不可信資料是安全的。
- **pickle ＝會自我組裝的魔法包裹**:拆封時包裹**自己動手組裝自己**——
  而「組裝步驟」寫在包裹裡。攻擊者寄來一個「組裝步驟＝執行 `rm -rf /`」的包裹,
  你 `pickle.loads()` 的那一刻就執行了。**這不是 bug,是 pickle 的設計**:
  它為了能還原任意物件,允許包裹指定「呼叫哪個函式來重建我」（`__reduce__`）。

所以鐵律只有一條:**pickle 只能拆自己人打包的包裹**——
不可信來源(使用者上傳、網路傳來、共用的 Redis/佇列)的資料,永遠不 `pickle.loads`。

同一個坑的變體,一起認:

- **YAML**:`yaml.load()` 同樣能建任意物件——**永遠用 `yaml.safe_load()`**。
- **看不見的 pickle**:很多工具背後偷偷用它——舊版 ML 模型檔(`joblib`/`torch.load`)、
  Celery 若設成 pickle serializer、Django 舊版 session。
  載入來路不明的模型檔＝執行來路不明的程式。

原則:**跨信任邊界只用資料格式(JSON/MessagePack),
pickle 留給「自己存、自己讀」的行程內場景**。這章用可控的無害範例
重現一次 pickle RCE,讓你親眼看到為什麼防不了、只能不用。
