...
> **值（物件）存在於記憶體中；變數只是「綁定到某個物件」的名稱。**

當你寫 `x = 5`：

1. Python 先在記憶體建立一個 `int` 物件 `5`。
2. 再讓名稱 `x` **指向（綁定到）** 這個物件。

`x` 本身不「裝著」5，它只是一個貼在 `5` 這個物件上的標籤。所以更精確的說法不是「x 的值是 5」，而是「x 這個名字目前綁定到物件 5」。

這帶來一個關鍵推論：**同一個物件可以有多個名稱指向它**。`a = b = [1, 2]` 讓 `a` 和 `b` 是同一個 list 的兩個標籤——不是兩份拷貝。
