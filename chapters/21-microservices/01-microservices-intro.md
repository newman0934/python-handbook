...
代價是：**跨服務的資料一致性變難**。單體裡「下訂單 + 扣庫存 + 扣款」可以是一個資料庫[交易](../15-database/06-transactions.md)（ACID）；拆成三個服務後，它們各有各的 DB，無法用單一交易——只能用**最終一致性**與 [Saga](../22-distributed-systems/07-saga.md) 模式協調。這正是微服務複雜度的核心來源之一。
