...
機器學習聽起來玄,監督式學習的本質是**教小孩認水果**：
給他看一堆**標好名字**的照片（「這是蘋果」「這是橘子」）,他自己歸納出規則;
之後拿**沒看過的**照片考他,答對率就是模型的能力。

對應到 scikit-learn 的固定流程,每個字都有位置：

1. **特徵矩陣 `X`**＝照片們（每列一個樣本、每欄一個特徵）;**目標 `y`**＝標好的名字。
2. **train/test split**＝把照片分成「教材」和「考卷」。**千萬別用教材考試**——
   用訓練過的資料評分,分數再高也只證明他會背,不證明他會認（這就是**過擬合 overfitting**:
   死背考古題的學生,模擬考滿分、上考場崩盤）。
3. **`fit(X_train, y_train)`**＝上課;**`predict(X_test)`**＝考試;
   **`score` / `accuracy`**＝改考卷。

scikit-learn 最美的設計是**所有模型同一套介面**：決策樹、邏輯迴歸、隨機森林,
全都是 `fit` → `predict`。換模型＝換一行建構子,其餘照舊——所以入門先學流程,再學模型。

這章帶你走完整趟：切資料、訓練、評估、避開兩大新手雷
（用測試集調參、資料洩漏 data leakage）,並示範 `Pipeline` 把前處理與模型串成一條。
深度學習(PyTorch)是後面 Part 的事,但流程觀念完全共用。
