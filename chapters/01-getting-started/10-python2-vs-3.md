...
你不會再寫 Python 2 了——那為什麼還要讀這章？兩個實際理由：

1. **網路上滿地都是 Python 2 時代的過期教學**（`print "hello"` 不加括號、奇怪的字串處理）。看得懂「這是舊時代的東西」，才不會照抄踩雷。
2. 這是一堂**語言如何演進**的課：Python 3 是史上最痛的語言升級,痛在哪、學到什麼,至今影響 Python 的每個決策。

痛的根源可以用一個比喻講清楚。

Python 2 把「**文字**」和「**位元組**」混在同一個 `str` 裡——像一個抽屜同時塞「信件」和「包裹」，拿的時候常常搞混，於是 `UnicodeDecodeError` 滿天飛（尤其對中文使用者是家常便飯的惡夢）。

Python 3 做了一次不留退路的大掃除：**信件歸信件（`str` = Unicode 文字）、包裹歸包裹（`bytes` = 位元組），兩個抽屜，絕不互通**。
代價：幾乎所有碰到字串、檔案、網路的舊程式都得改——這就是那場持續十年的痛苦遷移。
收穫：Python 3 處理多語言文字（包括中文）正確又清爽。

順帶認識一個機制：`from __future__ import ...`——「提前試用下個版本的功能」的官方後門,Python 用它讓大家平滑過渡,你在現代程式碼裡仍會看到它的身影。
...
Python 3（2008 年發布）是一次**刻意的、不向後相容**的大改版。
Guido 決定「一次修好累積多年的設計錯誤」——代價是 Python 2 的程式碼不能直接在 Python 3 上跑。

最致命的一項改動是**字串模型**——導讀說的「信件與包裹分抽屜」。這不只是語法差異，而是資料模型的根本改變：

- **Python 2**：`str` 是**位元組（bytes）**，另有 `unicode` 型別處理文字。兩者混用時發生惡名昭彰的隱式編碼/解碼——`UnicodeDecodeError` 滿天飛。
- **Python 3**：`str` 是 **Unicode 文字**，位元組是獨立的 `bytes` 型別，兩者**不會隱式互轉**。文字與位元組被徹底分開。

這個改變讓 Python 3 處理多語言文字變得正確而清爽；但也意味著幾乎每個碰到字串/檔案/網路的程式都需要修改——這就是遷移浩大的核心原因。
