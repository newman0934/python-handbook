...
上一章的事故——兩位店員同時改帳、錢憑空消失——這章給解法。兩種思路：

**思路一：一支筆制度（Lock）。**

帳本旁放**一支筆**：要改帳，先拿筆；筆在別人手上，就等。
「拿筆 → 改帳 → 還筆」——同一時刻只有一人能改，競態消失：
...
筆的變體，各一句話：**RLock**（同一人可重複拿的筆）、**Semaphore**（一次發 N 支筆——限流）、**Event**（不是筆,是一面「開工」旗——一人升旗,全員開跑）。

**思路二：傳紙條制度（Queue）——更高明。**

與其大家搶一本帳,不如**根本別共寫**：
A 店員把單子寫成紙條丟進**籃子**（`queue.put`）,B 從籃子取紙條處理（`queue.get`）——籃子本身是執行緒安全的,**沒有共享可變狀態,就沒有競態**。

這就是「生產者-消費者」模式,也是一句重要哲學：

> **Don't share memory to communicate; communicate to share memory.**
> ——能用「傳遞」解決的,不要用「共享+鎖」解決。

鎖用得越多,越容易出下一種事故：**死鎖**（兩人各拿一支筆、互等對方的筆,全店僵住）——章內教你避開。
...
| 工具 | 用途 | 一句話（餐廳版） |
|------|------|------------------|
| **`Lock`** | 互斥：保護臨界區（一次一個執行緒） | 帳本旁的那支筆——拿到才能寫 |
| **`RLock`** | 可重入鎖：同一執行緒可重複取得 | 同一人可重複拿的筆 |
| **`Event`** | 執行緒間傳訊號（等待/通知） | 「開工」旗——一人 set、其他 wait |
| **`Condition`** | 複雜的等待條件 | 等特定條件成立再動 |
| **`Semaphore`** | 限制同時存取的數量 | 一次發 N 支筆——限流 |
| **`Queue`** | 執行緒安全的資料交換 | 傳紙條的籃子——生產者放、消費者取 |

核心觀念：**共享可變狀態需要保護**。

- `Lock` 是最基本的保護（管好那支筆）。
- `Queue` 更高明——透過「**傳遞而非共享**」從根本消除問題。
