...
資料分析師不是「做圖機器」,是**偵探兼醫生**。
老闆丟來一句「營收為什麼掉了?」——這不是可以直接查的問題,是**症狀**。
好的分析師像醫生問診,先把模糊症狀變成可查證的具體問題:
「Q3 華北區 A 產品營收比 Q2 掉 15%——是哪些客戶、哪些通路貢獻了下降?」

整套工作流就是問診 SOP,這章逐步展開:

1. **定義問題**:指標是什麼?跟誰比(基準)?範圍多大?——**問題定義錯,後面全白做**。
2. **取得資料**:用 [SQL](02-sql-aggregation.md) 從資料庫撈——分析師的第一語言。
3. **清理**:真實資料永遠是髒的([Part 17 ch05](../17-data-science/05-data-cleaning.md) 的功夫)。
4. **探索(EDA)**:先[體檢](08-eda.md)再下診斷,別急著開藥。
5. **分析與驗證**:拆解、對比、找出驅動因素——並且**質疑自己的結論**
   (相關不等於因果;辛普森悖論這章會講)。
6. **溝通**:洞察要能**改變決策**才有價值——
   「華北下降主因是兩家大客戶流失,建議⋯⋯」而不是丟一疊圖表。

一句話定調整個 Part:**工具(SQL/pandas)是手段,
「從商業問題到可行動建議」的思路才是本體**。
