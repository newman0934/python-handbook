...
[ch01 光譜](01-cloud-overview.md)的最右端是 **FaaS(函式即服務)**——
**AWS Lambda**、**Google Cloud Functions**。抽象到極致:
你連容器都不用管,**只丟一個函式**上去,雲負責在「有事件時」把它叫起來跑。

它的執行模型是**事件觸發的無狀態函式**:

```text
事件發生(HTTP 請求 / 檔案上傳 / 訊息 / 排程 cron)
   → 雲啟動一個函式實例 → 跑你的程式碼 → 結束、銷毀
```

兩個招牌特性,一好一壞,是理解 FaaS 的關鍵:

- **縮到零 + 按次計費(好)**:沒事件時**完全不跑、不收費**;
  來一百個請求就**瞬間開一百個實例**平行處理。
  對**流量稀疏或尖峰極端**的場景(偶爾觸發的排程、webhook、不定期的批次)超划算。
- **冷啟動(壞)**:實例被銷毀後,下一個請求得**從頭啟動**(載入 runtime、你的程式)——
  這幾百毫秒到數秒的延遲叫**冷啟動**。對延遲敏感的即時 API 是痛點
  (有預熱、預留實例等緩解手段)。

還有幾個先天限制要記住:**執行有時間上限**(不適合跑很久的任務)、
**無狀態**(狀態得放[外部 DB/儲存](06-managed-db-storage.md))、
本機難重現、易被特定雲**綁定(vendor lock-in)**。

什麼時候選 FaaS、什麼時候選 [Cloud Run](03-containers-ecs-cloudrun.md)?
這章給你判準:**事件驅動、稀疏、短小的工作**用 FaaS;
**持續有流量的 web 服務**用容器平台。並帶你用 Python 寫一個 Lambda 函式,
親身體會這種「只寫一個函式就上線」的極簡開發模式。
