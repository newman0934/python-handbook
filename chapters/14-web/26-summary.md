...
> 把這 25 章串成一張圖——最大的驚喜是：**你在 [Part 5](../05-typing/README.md) 學的型別註記，在這裡突然變成了「執行期的資料驗證」。** 而 ch19–25 更把你從「會做出 API」推到「做得**可靠**、能上生產」：統一錯誤契約、條件請求、任務佇列與排程、應用生命週期、串流與 SSE。

## 🗺️ 知識地圖（這 25 章怎麼串起來）

Web 開發看似很多東西，但骨架只有一條：
**請求進來 → 驗證 → 處理 → 回應**。其餘各章都掛在這條線上；
而 **ch19–23 是「進生產」的加固層**：錯誤要有契約、讀寫要有版本、重活要搬離請求路徑。
...
flowchart TD
    ...
    O --> P["進生產加固（ch19–23）"]
    P --> P1["ch19 RFC 9457 錯誤契約<br/>ch20 ETag / 條件請求 / webhook"]
    P --> P2["ch21 為何需要任務佇列<br/>ch22 Celery · ch23 排程"]
...
| 統一錯誤回應格式 | **RFC 9457 problem+json**：固定欄位 + 擴充欄位；`type` 當錯誤種類主鍵 | [ch19](19-error-contract-rfc9457.md) |
| 沒變的資料別重傳 | **ETag + `If-None-Match`** → 304；寫入用 `If-Match` → 412（樂觀鎖） | [ch20](20-etag-conditional-webhook.md) |
| 收 / 送 webhook | **HMAC 簽章**驗來源，`compare_digest` 防時序攻擊，接收端要冪等 | [ch20](20-etag-conditional-webhook.md) |
| 慢工作要「不丟、可重試、可擴充」 | 升級到**獨立任務佇列**（別用 in-process BackgroundTasks） | [ch21](21-task-queue-why.md) |
| 實作分散式任務佇列 | **Celery** + broker：`.delay()` 派工、worker 執行、退避重試、冪等 | [ch22](22-celery.md) |
| 定時 / 排程任務 | cron 語意 + **Celery beat / APScheduler**；排程器要單例 | [ch23](23-scheduling.md) |
| DB 池 / Redis / 模型「開機建、關機收」 | **lifespan**（`yield` 前 startup、後 shutdown，掛 `app.state`） | [ch24](24-lifespan-startup.md) |
| 逐字串流 LLM / 下載大檔 / 即時推播 | **StreamingResponse** + 產生器；單向推播用 **SSE**（比 WebSocket 輕） | [ch25](25-streaming-sse.md) |
...
- [ ] RFC 9457 的 `type`/`title`/`detail` 各是什麼？為什麼判斷要用 `type` 不用 `detail`？（[ch19](19-error-contract-rfc9457.md)）
- [ ] ETag 怎麼省頻寬？`If-None-Match` 和 `If-Match` 方向差在哪？（[ch20](20-etag-conditional-webhook.md)）
- [ ] webhook 怎麼驗來源？為什麼驗簽不能用 `==`？（[ch20](20-etag-conditional-webhook.md)）
- [ ] `BackgroundTasks` 和 Celery 差在哪？什麼時候該升級？（[ch21](21-task-queue-why.md)）
- [ ] 任務佇列為什麼要求任務冪等？至少一次投遞是什麼？（[ch21](21-task-queue-why.md)、[ch22](22-celery.md)）
- [ ] `send_email(id)` 和 `send_email.delay(id)` 差在哪？（[ch22](22-celery.md)）
- [ ] 排程任務在多實例部署會怎樣？怎麼解？（[ch23](23-scheduling.md)）
- [ ] 連線池、Redis、模型應該在哪裡建立？為什麼不能每請求建？（[ch24](24-lifespan-startup.md)）
- [ ] lifespan 的本質是什麼？和棄用的 `on_event` 差在哪？（[ch24](24-lifespan-startup.md)）
- [ ] 怎麼讓 FastAPI 逐字串流 LLM 輸出？SSE 和 WebSocket 怎麼選？（[ch25](25-streaming-sse.md)）
- [ ] 下載大檔怎麼不吃爆記憶體？（[ch25](25-streaming-sse.md)）
...
| **API 錯誤怎麼設計？** | 「統一錯誤契約，用 **RFC 9457 / `application/problem+json`**：固定欄位（`type`/`title`/`status`/`detail`/`instance`）+ 擴充欄位放機器可讀資料；用 `type`（URI）當錯誤種類主鍵，別拿 `detail` 字串做判斷；全域例外處理器集中轉換。」 | [ch19](19-error-contract-rfc9457.md) |
| **BackgroundTasks vs Celery？** | 「`BackgroundTasks` 是 **in-process**——重啟／崩潰會丟任務、沒重試、不能獨立擴充。重要、要可靠的背景工作用**獨立任務佇列（Celery + broker）**：producer `.delay()` 派工、worker 執行、退避重試、死信。投遞是**至少一次**，故任務要**冪等**。」 | [ch21](21-task-queue-why.md)、[ch22](22-celery.md) |
| **定時任務怎麼做？** | 「cron 語意描述時間，用 **Celery beat**（已用 Celery）或 **APScheduler**（輕量）。**排程只派工、worker 執行**；**排程器要單例或加分散式鎖**，否則多實例會重複跑；被排任務仍要冪等。」 | [ch23](23-scheduling.md) |
| **連線池應該在哪建？** | 「**lifespan 的 startup 段**建一次、掛 `app.state`，所有請求共用；shutdown 段關閉。lifespan 是個 **async context manager**（`yield` 前 startup、後 shutdown，`try/finally` 保證清理），取代已棄用的 `@app.on_event`。每請求建池等於失去連線池意義、還會耗盡 DB 連線。」 | [ch24](24-lifespan-startup.md) |
| **怎麼串流 LLM 輸出？SSE vs WebSocket？** | 「用 **`StreamingResponse`** 包 async 產生器、`media_type="text/event-stream"`，逐個 `yield` SSE 事件（`data: ...\n\n`），前端 `EventSource` 收，使用者立刻看到字。**SSE 單向、純 HTTP、內建重連**，單向推播（通知 / LLM / 進度）用它；**要雙向才用 WebSocket**。大檔下載也用 StreamingResponse 邊讀邊送，記憶體不爆。」 | [ch25](25-streaming-sse.md) |
