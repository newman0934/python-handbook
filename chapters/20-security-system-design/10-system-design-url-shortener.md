...
從這章起連四章是**系統設計面試**實戰。先講心法:
系統設計題沒有標準答案,考的是**思考過程**——
面試官想看你會不會像工程師一樣,把模糊需求變成可行架構。流程固定四步:

1. **先問,別急著畫**:功能範圍?流量多大?讀寫比?——
   直接開畫的人第一分鐘就出局。
2. **估規模**:每月 1 億條新網址 ≈ 寫入 40/秒;讀寫比 100:1 → 讀 4000/秒。
   數字不用準,**展示量化思維**才是重點。
3. **設計核心流程**:短碼怎麼生成?——base62 編碼(0-9a-zA-Z 共 62 字元),
   7 碼就有 62⁷ ≈ 3.5 兆組合,夠用幾百年。
4. **針對瓶頸加料**:讀寫比 100:1 告訴你**快取是主角**——
   熱門短碼放 Redis([櫃檯便條](../15-database/18-redis.md)),
   資料庫只接 cache miss;重導用 301 還是 302?(301 瀏覽器會快取,
   省流量但統計不到點擊——又是取捨。)

短網址是經典開場題,因為五臟俱全:編碼、資料庫選型、快取、
唯一性衝突、擴展——每一站都是你在前面 Part 學過的東西,
這章示範怎麼把它們**串成一個面試敘事**。
