...
客戶名單一張表、訂單一張表——要回答「哪個城市的客戶最會買」,
就得**按學號把兩份名單對起來**:`JOIN ... ON customers.id = orders.customer_id`。

JOIN 的類型,差別只在一件事:**對不上的人怎麼辦**——

- **INNER JOIN ＝聯誼配對**:兩邊都到場才算一對;**沒下單的客戶直接消失**。
- **LEFT JOIN ＝全班點名**:左表(客戶)全部保留,有訂單的附上訂單,
  沒訂單的欄位**留白(NULL)**——分析最常用:「所有客戶,不管有沒有買」。
- **FULL OUTER**:兩邊都全留;**RIGHT** 實務少用(左右對調寫成 LEFT 就好)。

分析師的兩個經典事故,都出在「消失的列」:

1. **用 INNER 算「客戶平均消費」**——沒下單的客戶被踢出分母,平均被高估。
   要「所有 X」就用 LEFT,再把 NULL 當 0 處理。
2. **一對多的膨脹**:一個客戶有 5 筆訂單,JOIN 後客戶資料被**複製 5 列**——
   這時再 `COUNT(customers.id)` 就數出 5 個同一人。
   口訣:**JOIN 完先確認粒度**(現在一列代表什麼?),要去重就 `COUNT(DISTINCT ...)`。

這章配上 [ch02 的聚合](02-sql-aggregation.md):JOIN 完再 GROUP BY,
就是「每城市總營收」這類跨表分析的標準式。
