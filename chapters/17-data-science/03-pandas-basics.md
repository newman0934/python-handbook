...
numpy 給了你「陣列」,但真實資料長什麼樣？**表格**——有欄名（姓名、城市、金額）、
每欄型別不同、有的格子還缺值。這就是 pandas 的地盤：**Excel 表格的程式版**。

兩個核心角色：

- **`Series`＝一欄**：一條 numpy 陣列＋每格的**標籤（index）**＋欄名。
- **`DataFrame`＝整張表**：多個 Series 並排，**共用同一組列標籤**。

pandas 和 numpy 最大的觀念差異是那組**標籤**。numpy 按「位置」辦事（第 0 格、第 1 格）,
pandas 按「名字」辦事——兩個 Series 相加時,它**按 index 對名字**,不是按順序：
像發考卷是按學號發,不是按座位順序發。名字對不上的格子,結果是缺值（NaN）而不是錯位。
這個「自動對齊」是 pandas 最貼心也最容易嚇到新手的行為。

再來就是日常四件事,這章逐一示範：

1. **讀資料**：`pd.read_csv(...)` 一行進表。
2. **看資料**：`head()`／`info()`／`describe()` 三板斧。
3. **選資料**：`df["col"]` 選欄；**`loc`（按標籤）vs `iloc`（按位置）**——分不清這兩個是
   新手九成錯誤的來源,先記口訣：**l**oc 看 **l**abel。
4. **過濾**：布林遮罩 `df[df["age"] > 30]`——跟 numpy 一脈相承。
