...
前面幾章一直提到「檢查員」。這章正式介紹他：**mypy**。

先想清楚一件事：**標籤本身不會抓錯**。
貼滿標籤但沒人核對，等於沒貼——所以沒有檢查器的型別註記，只是比較漂亮的註解。

mypy 的工作方式像一位**校對編輯**：

- 他**不會把你的文章「演出來」**（不執行程式）。
- 他只是**逐行閱讀**，追蹤每個值的型別怎麼流動，一發現矛盾就圈起來：
  「這裡說好是 int，你怎麼塞了 str？」「這個可能是 None，你沒檢查就用了。」

這叫**靜態分析**——在程式跑起來之前，錯誤已經被抓到。

還有一個常見誤會要破除：「我沒貼標籤的地方，mypy 是不是就瞎了？」
不會。mypy 有**推斷**能力——看到 `x = 5` 它自然知道 x 是 int。
標籤主要補在它推不出來的地方（尤其是**函式的參數和回傳**——這是它視力的邊界）。

這章教你：怎麼跑 mypy、怎麼設定嚴格度、以及老專案怎麼**漸進採用**（不必一天貼完全部標籤）。
順帶認識同行：pyright（VS Code 的 Pylance 就是它）——原理相同，選一個用就好。
...
mypy 做的是**靜態分析（static analysis）**——那位「不執行、只閱讀」的校對編輯。
它讀原始碼與註記，模擬型別如何流動，找出矛盾：
...
它還有**型別推斷（inference）**：即使沒有註記，也能從賦值推出很多型別（`x = 5` → x 是 int）。
註記是補在它推不出來的地方——尤其**函式邊界**（參數與回傳）。

同行還有：**pyright**（微軟出品，VS Code 的 Pylance 基於它，快且嚴格）、**pyre**（Meta）。
原理相同；mypy 最通用，本書以它為準。
