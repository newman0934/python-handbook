...
回想 [bytecode 章](../10-cpython-internals/README.md)的比喻:CPython 是**口譯員**,
拿著你的講稿逐句翻譯給 CPU 聽。口譯有三重慢:每句都要**現場想**（直譯迴圈開銷）、
每個詞都要**猜詞性**（動態型別:執行到 `a + b` 才查 a、b 是什麼型別）、
每個數字都裝在**盒子**裡要拆裝（PyObject 裝箱拆箱）。

平常這無所謂——瓶頸在 I/O 或演算法。但當熱點是**數值密集的緊迴圈**
（跑一億次的加乘）,口譯開銷就是主角。終極解法:**別口譯了,事先把講稿整篇翻好**——
編譯成機器碼直接念。

前提只有一個:**編譯器得知道型別**。你告訴它「n 一定是 int、arr 一定是 double 陣列」,
它就能生成不查型別、不拆箱、直奔 CPU 指令的原生碼——常見 10~100 倍加速,
甚至能釋放 GIL、用上多核與 SIMD。

兩條路,取捨不同:

- **Numba**:加一個 `@njit` 裝飾器,首次呼叫時 JIT 編譯。**幾乎零改寫**,
  專吃 numpy 數值迴圈;但支援的 Python 子集有限。
- **Cython**:寫 `.pyx`、標型別、事先編譯成擴充模組。**改寫成本高**,
  但控制力最強,適合要發佈的函式庫（pandas、scikit-learn 底層就是它）。

這章各給一個可跑的對照範例,並講清楚**什麼時候不值得**用它們
（I/O-bound、已能向量化、熱點不在迴圈——都輪不到編譯出場）。
