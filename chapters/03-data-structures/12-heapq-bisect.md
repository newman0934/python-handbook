...
**heapq — 二元最小堆積（min-heap）**：一種用 list 表示的完全二元樹，永遠滿足「父節點 ≤ 子節點」。因此**最小值永遠在 `heap[0]`**（O(1) 查看），而插入（push）與彈出最小（pop）都是 **O(log n)**。Python 的 `heapq` 是 **min-heap**（最小值先出）。

**bisect — 二分搜尋**：在**已排序**的序列上，用二分法找「x 應該插入的位置」，O(log n)。可用來維持有序、計數、做範圍查詢。

關鍵前提：`bisect` 要求序列**已排序**；`heapq` 則自己維持堆積性質。
