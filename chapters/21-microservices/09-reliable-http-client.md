...
用生活比喻:呼叫別人就像打電話叫外送。你不會**握著話筒無限等**(這就是逾時,要設上限);
也不會**掛掉立刻重撥、每秒一通**(要退避,別狂扣對方);而如果**連打十通都沒人接**,
你就知道這家店今天沒開——這時**乾脆別再打了**,這正是 [ch07](07-rate-limit-circuit-breaker.md) 的熔斷器:
像**保險絲跳閘**一樣直接跳開、快速失敗,過陣子再試一通看它恢復沒。把這套常識寫進程式,就是可靠的 HTTP 客戶端。
