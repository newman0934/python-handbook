...
> 只要幾行、不預裝一堆你用不到的東西，就想跑起一個 Web 服務——Flask 就是為此而生。這章講這個「微」框架到底微在哪，以及什麼時候該選它、而不是 FastAPI。

## 💡 白話導讀（建議先讀）

Flask 是一台**輕便攤車**——「微框架」的「微」不是能力小,是**核心刻意只帶必需品**：

```python
from flask import Flask
app = Flask(__name__)              # 推出攤車

@app.route("/hello/<name>")        # 掛菜單:這個網址由這個函式服務
def hello(name):
    return {"message": f"Hi {name}"}   # 回 dict 自動變 JSON

app.run()                          # 開張!
```

五行,一個能跑的 Web 應用——攤車的魅力就是這個起步速度。

「微」的哲學:**核心只有爐子（路由 + 請求處理）**,其他配備——資料庫、表單、登入——**你自己選件加裝**（Flask-SQLAlchemy、Flask-Login⋯⋯）。自由,但要自己組。

定位坦白說：

- Flask 是 **WSGI（同步）** 框架——[上一章](01-wsgi-asgi.md)的舊規格,海量並發不是它的主場。
- 本書後續主力是 **FastAPI**(下一章)——async、型別驅動、自動文件。
- 但 Flask 值得會:**存量專案巨多**、學習曲線最平、而且它的概念(路由、視圖函式、請求上下文)是所有 Python Web 框架的通用語。

把這章當「Web 框架的共同概念課」讀,下一章換 FastAPI 就只是換語法。
...
- **應用物件**：`app = Flask(__name__)`——推出攤車，代表你的 Web 應用。
- **路由（route）**：`@app.route` 裝飾器把「URL 路徑」對應到「處理函式（view function）」——掛菜單。
- **請求/回應**：處理函式回傳的字串/dict 自動變成 HTTP 回應。

「微」的哲學：

> **核心只做路由 + 請求處理**，其他（DB、表單、認證）由你選套件組合——輕便攤車，自由但要自己組裝。
