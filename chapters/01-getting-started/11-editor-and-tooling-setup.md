...
> 打字當下發現 < 存檔時發現 < 跑測試發現 < 上線後被用戶發現

這章要配的一套工具，目的就一個：**把發現錯誤的時間，盡量往左推**。

四個角色，像一間出版社的分工：

| 角色 | 出版社比喻 | 做什麼 | 用誰 |
|------|-----------|--------|------|
| **Linter** | 挑錯編輯 | 抓可疑寫法、沒用到的變數、潛在 bug | **ruff** |
| **Formatter** | 排版師 | 自動統一縮排、空格、換行（PEP 8 不用背） | **ruff format** |
| **Type checker** | 事實查核 | 對照型別標籤抓矛盾（[Part 5](../05-typing/README.md) 的檢查員） | **mypy** |
| **Test runner** | 試印 | 跑測試確認行為正確 | **pytest** |

好消息：前兩個角色現在由 **ruff** 一人分飾（Rust 寫的,快到沒感覺）,配置大幅簡化。

編輯器（VS Code / PyCharm）的角色是**把這四位請進你的打字現場**——你一邊打字,紅波浪線即時標錯、滑鼠悬停看型別、存檔自動排版。
背後的技術叫 LSP（語言伺服器協定）,知道名字即可。

這章給你一份「照做就好」的完整配置——花三十分鐘配一次,之後每天省下的除錯時間都是淨賺。
...
現代趨勢是 **ruff** 一個工具同時扮演 linter + formatter（Rust 寫成，比舊工具快數十倍），大幅簡化配置。

編輯器（VS Code、PyCharm 等）的角色是**把這些工具整合進打字現場**：
透過 **LSP（Language Server Protocol）** 這個標準協定，編輯器與語言伺服器（如 Pylance / pyright）溝通——即時補全、跳轉定義、錯誤標示，都是它在背後跑。
