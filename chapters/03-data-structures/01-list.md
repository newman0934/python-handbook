...
list 是 Python 最常用的容器。理解它的關鍵，是知道它底層長什麼樣：

**一排連續的置物格，每格放一張「提貨單」。**

- 格子是**連續**的、有編號（索引）——所以「拿第 100 格的東西」是**瞬間**的：直接算位置走過去（O(1)）。
- 格子裡放的不是物品本身，是**指向物品的提貨單**（指標）——所以一個 list 能混裝任何型別（`[1, "a", [2]]`），每張提貨單長得都一樣。

這個構造直接解釋了快慢：

- **尾端加東西快**（`append`）：後面本來就留了幾個空格（預留空間），直接放。
- **頭部/中間插入、刪除慢**：第 0 格要插東西？**後面每一格都得往後挪一位**——十萬格就挪十萬次（O(n)）。

所以有一條實用判斷：**常常要從頭部進出的佇列，別用 list**（用 [deque](08-collections-module.md)，兩端都快）。

一句規格總結 list：**有序、可變、可混裝、尾端快頭部慢**——這章把每一項展開。

## 🔗 前端對照

Python `list` 就是 JavaScript 的 `Array`——有序、可變、可混型別。日常操作幾乎一一對應:

| 操作 | Python | JavaScript |
|------|--------|-----------|
| 加到尾端 | `xs.append(v)` | `xs.push(v)` |
| 移除尾端 | `xs.pop()` | `xs.pop()` |
| 長度 | `len(xs)` | `xs.length` |
| 取子段 | `xs[1:3]`（slice） | `xs.slice(1, 3)` |
| 是否包含 | `v in xs` | `xs.includes(v)` |
| 轉換 / 過濾 | comprehension | `.map()` / `.filter()` |

一句話:資料模型相同。差別多在**方法名**（`append` vs `push`、`len(xs)` vs `xs.length`）,
以及 Python 的 slice `xs[1:3]` 是語言語法、比 `.slice()` 更順手。
...
CPython 的 list 底層是一個**連續的陣列，存的是「指向物件的指標」**——那排放提貨單的置物格。這帶來幾個本質：

- **有序**：元素依插入順序排列，有明確索引。
- **可變（mutable）**：能原地新增、刪除、修改（見[可變 vs 不可變](06-mutability.md)）。
- **異質**：因為存的是指標（提貨單），元素可以是任何型別、混合存放（`[1, "a", [2]]`）。
- **動態**：容量自動成長。CPython 採 **over-allocation（預留額外空間）**——尾端本來就留了空格，所以連續 `append` 平均是 O(1)（攤銷）；偶爾空格用完才觸發擴容、複製整個底層陣列。

「連續陣列」也決定了快慢：

- **按索引存取 O(1)**——直接算位址。
- **頭部/中間插入刪除 O(n)**——後面元素全得搬移。
