...
Python 沒有「內建於語言的加法」——`+` 只是呼叫物件的 `__add__` 方法的簡寫：
...
'ab'
>>> [1] + [2]          # list 的 __add__ 是延伸
[1, 2]
```

同一個 `+`，對 int 是加法、對 str 是連接、對 list 是串接——因為它們各自實作了不同的 `__add__`。這就是為什麼「一切皆物件」如此根本：連運算子行為都由物件自己定義（見 [一切皆物件](../10-cpython-internals/01-everything-is-object.md)）。
