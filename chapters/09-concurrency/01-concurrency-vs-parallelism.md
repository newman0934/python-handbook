...
整個 Part 9 用一間**餐廳**講完。先分清兩個常被混用的詞。

**並發（concurrency）＝一位老闆輪流做很多事。**
一人餐廳：接單、下麵、收銀——他在三件事之間**切換**，每件事都有進展，但**一雙手，同一瞬間只做一件**。

**並行（parallelism）＝多雙手真的同時動。**
請了三位師傅，三碗麵**同一瞬間**在煮。

關鍵領悟：

- 並發是**安排工作的方式**（把事情拆成可交錯的小段）——**一個人（單核心）也做得到**。
- 並行是**執行的事實**（多件事同時發生）——**必須有多雙手（多核心）**。

為什麼 Python 特別需要分清這兩者？先劇透下一章：
CPython 有一把鎖（GIL），讓多執行緒**能並發、卻難以並行**——就像餐廳可以請很多店員，但某個關鍵工具只有一件，同一時刻只有一人能用。

所以「加執行緒為什麼沒變快」這個 Python 最經典的困惑，答案就從這組區分開始。

還有一組要對齊的詞：**I/O 密集**（大部分時間在**等**——等網路、等磁碟,像等外送食材）vs **CPU 密集**（大部分時間在**算**——像手工揉麵）。
**等待可以並發解決,計算必須並行解決**——這句話是整個 Part 的選型總綱。
...
- **並發**：**結構上**同時「處理」多個任務——執行時間重疊，但不一定「真的同時跑」。一位老闆輪流照看多件事（切換照顧，一雙手）。
- **並行**：**實際上**同時「執行」多個任務——多個計算真的在同一瞬間進行。多位師傅各煮一鍋（多雙手同時動）。

Rob Pike 的名言：「**並發是關於處理很多事情，並行是關於同時做很多事情。**」

- 並發是一種**程式結構**（把任務拆成可交錯的單位）——**不需要**多核心（單核也能交錯）。
- 並行是一種**執行方式**（多核心真的同時跑）——**需要**多核心。
