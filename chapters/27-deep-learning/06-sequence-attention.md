...
[CNN](05-cnn.md) 為影像而生,但**文字、語音、時間序列**是另一種東西——**序列**:
有先後順序、長度不定、而且**遠處的詞會互相影響**
(「我在法國長大……所以我說流利的**法文**」——要填對最後一個詞,得記住開頭的「法國」)。
這章講序列建模怎麼從笨辦法一路演進到**改變世界的 Transformer**。

**第一代:RNN(遞迴神經網路)＝一邊讀一邊記筆記。**
逐字讀,維護一個「記憶(隱藏狀態)」隨讀隨更新。
問題有二:讀得越遠,開頭的記憶**越模糊**([梯度消失](07-training-techniques.md));
而且**必須一個字一個字讀**,沒法平行——慢。
(LSTM/GRU 改善了記憶,但「不能平行」的硬傷還在。)

**第二代:注意力(attention)＝不靠記憶,直接回頭查全文。**
核心是 **Query-Key-Value**,直覺像**查字典**:
處理「法文」這個詞時,它**主動回頭掃描整句話**,
給相關的詞(「法國」)高權重、無關的詞低權重——
**任意兩個詞直接建立關聯,不管隔多遠**,一舉解決遠距依賴。
而且所有位置能**平行**計算——快。

**第三代:Transformer ＝完全用注意力堆起來的網路**
(論文名就叫〈Attention Is All You Need〉)。
它是今天所有大型語言模型([Part 28 的 GPT](../28-llm-genai/README.md))的骨架——
**讀懂這章的 attention,你就握住了理解 ChatGPT 的鑰匙**。
這章從 RNN 的困境講到 self-attention 的機制,為下個 Part 的 LLM 鋪好路。
