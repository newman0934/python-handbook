...
Python 的核心設計：**任何物件都能放在布林情境（如 `if`、`while`、`and`/`or`）中判斷真假**，不限於 `True`/`False`。這叫 **truthiness**。

判斷規則：物件預設為「真（truthy）」，除非它屬於下列「假（falsy）」值之一。Python 會呼叫物件的 `__bool__()`（沒有的話退而求其次呼叫 `__len__()`）來決定。
