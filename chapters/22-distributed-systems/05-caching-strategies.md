...
> **為什麼是 Redis,不是行程內的 `lru_cache`?** 因為這裡的便條要**所有服務實例共享**。
> [Part 18 的 `lru_cache`](../18-performance/04-caching.md) 是**行程內**的迷你版——奈秒級、零成本,
> 但每個 Pod 各貼各的便條,更新時你只撕得掉自己那台的。要「一撕全撕」,便條就得放在**共享的外部櫃檯**(Redis)。
> 兩者的完整取捨與選型,見 [Part 18「本地 vs 分散式快取」](../18-performance/04-caching.md#延伸本地-vs-分散式快取該用哪個)。
