...
前面所有模型都是**監督式**——有標準答案(標籤)可對照。
但很多真實任務**根本沒有答案**:「把客戶分成幾群好做行銷」——
分成哪幾群?沒人知道,得讓資料自己說。這就是**非監督式學習**,
主角是**分群(clustering)**:沒有老師,自己把相似的東西湊成堆。

入門演算法 **k-means**,像**辦同學會排桌**:

1. 先隨便擺 k 張桌子(k 個群中心)。
2. 每個人**走到最近的桌子**坐下(指派)。
3. 每桌算出「大家的重心」,**把桌子移到重心**(更新)。
4. 重複 2–3,直到大家不再換桌(收斂)。

簡單有效,但有幾個**必須先知道的脾氣**,這章會逐一拆:

- **k 要自己指定**:分幾群是你給的,不是它學的。怎麼挑?
  **手肘法(elbow)** 和輪廓係數(silhouette)幫你找合理的 k。
- **初始位置會影響結果**:隨便擺可能擺出爛結果——`k-means++` 是聰明的擺法(預設)。
- **假設群是圓的、大小相近**:遇到細長形、密度不均的群會失靈——
  這時換 **DBSCAN**(按密度分群,還能挑出雜訊點)。

分群的難處還在於**沒有標準答案可驗證**——好壞得靠內部指標(輪廓係數)+ 業務判讀。
這章也講分群結果怎麼解讀、怎麼落地成「客戶分群」這類商業應用。
