...
你對資料庫說「我要這些資料」(宣告式)——那**誰決定怎麼拿**?答案:**查詢優化器**,資料庫裡的導航系統。

跟導航一模一樣:你給目的地,它評估各條路線的成本(用索引?全表掃?哪種 JOIN?誰先誰後),挑**估計最便宜**的走。同一句 SQL,可能有幾千種走法,快慢差百倍。

三個「反直覺但正確」的行為,懂了導航邏輯就通:

**1. 「加了索引它卻不用」——往往是對的。**
要撈的資料佔全表一半時,「照索引一筆筆跳著讀」(大量隨機 I/O)比「從頭順順讀完」(順序 I/O)更慢——導航選大路不抄小巷,是算過的。
口訣:**索引適合撈一小撮,撈一大片直接全掃**。

**2. JOIN 有三種演算法,選錯差一萬倍。**
nested loop(小表配索引)、hash join(兩張大表)、merge join(都排好序)——優化器按統計挑。**大表 JOIN 沒索引**=退化成 O(N×M),經典災難。

**3. 它靠「統計資料」估算——統計過期就會選錯路。**
像導航用舊地圖。大量灌資料後查詢突然變慢?先 `ANALYZE` 更新統計。

診斷工具就一個:**EXPLAIN**——把導航選的路線攤開給你看。會讀它,慢查詢就不再是玄學。
