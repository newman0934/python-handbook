...
`SomeClass(args)` 這個「呼叫類別」的動作，背後是兩步：

1. **`__new__(cls, *args)`**：**建立並回傳**一個新的（尚未初始化的）實例。它是 static method（第一參數是 `cls`），負責「配置記憶體、造出物件」。
2. **`__init__(self, *args)`**：拿到 `__new__` 回傳的實例（即 `self`），**設定它的屬性**。不回傳值。

只有當 `__new__` 回傳的是 `cls` 的實例時，`__init__` 才會被自動呼叫。流程：

```text
Point(1, 2)
  → Point.__new__(Point, 1, 2)   # 造出空的 Point 實例
  → Point.__init__(實例, 1, 2)   # 初始化它
  → 回傳實例
```
