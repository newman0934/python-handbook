...
HTTP 是**請求-回應（request-response）** 協定——客戶端發**請求**、伺服器回**回應**，一來一回：
...
HTTP 是**無狀態（stateless）** 的——每個請求獨立，伺服器不記得上一個請求（狀態靠 cookie/token 維持，見 [CORS/cookie](14-cors-cookie-session.md)）。
