...
- **`raise SomeError("msg")`**：拋出一個新例外。
- **`raise`**（單獨、在 except 內）：**重新拋出**當前正在處理的例外（保留原始 traceback）。
- **`raise NewError from original`**：拋出新例外並串接原因（見 [例外鏈](05-exception-chaining.md)）。

「該拋例外還是回傳錯誤值」是設計問題：**例外適合「異常、不該正常繼續」的情況**；正常的「找不到」有時回 `None` 更合適（見取捨）。
