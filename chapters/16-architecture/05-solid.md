...
SOLID 是五條「把類別設計得好改」的守則，名字嚇人，道理都是生活常識：

- **S（單一職責）**——**一間房間一個用途**。廚房兼廁所，裝修任何一邊都動到另一邊。
  一個類別兼「算業務＋存 DB＋發信」，任何需求變都得改它、都可能弄壞別的功能。
- **O（開放封閉）**——**加插座不敲承重牆**。加新功能應該是「插新的上去」（新增類別/函式），
  而不是回頭改一大段舊程式（每改一次舊碼，就賭一次不弄壞它）。
- **L（里氏替換）**——**替換零件要合規格**。號稱能替換原廠的副廠零件，裝上去卻漏水，
  比沒有還糟。子類別接在父類別的位置上，行為承諾不能偷偷變（經典反例：`Square` 繼承 `Rectangle`）。
- **I（介面隔離）**——**遙控器別塞 100 顆按鈕**。強迫使用者依賴一個胖介面，
  結果每個實作者都要寫一堆 `raise NotImplementedError`。拆成幾支小遙控器，各取所需。
- **D（依賴反轉）**——**牆上裝插座，不焊死電線**。高層模組依賴「插座」（抽象介面），
  任何合規格的裝置都能插上——這正是前面 [Clean Architecture](02-clean-architecture.md) 與
  [DI](03-dependency-injection.md) 的理論根據。

它們不是教條，而是同一個目標的五個角度：**讓改動的影響範圍越小越好**。
下面逐條展開，每條配「違反 vs 遵守」的 Python 對照。
