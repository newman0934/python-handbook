...
程式裡用到一個名字，Python 去哪找它？答案是**由近到遠找四個地方，找到就停**：

1. **自己房間**（Local）——當前函式裡。
2. **客廳**（Enclosing）——包住你的外層函式。
3. **家門口**（Global）——這個 `.py` 檔的頂層。
4. **巷口便利商店**（Built-in）——`print`、`len` 這些永遠買得到的內建品。

口訣 **LEGB**，由內而外。四處都沒有？`NameError`。

真正的坑不在「讀」，在「**寫**」。規則是：

> **在函式裡對一個名字賦值，Python 一律當作「你要在自己房間放一個新的」**——哪怕外面已經有同名的東西。

所以「想改外面的變數」會意外變成「在房間裡新開一個同名的」，外面那個毫髮無傷。
更陰的是有名的 `UnboundLocalError`：函式裡先用再賦值，Python 因為看到賦值就認定它是房間貨,結果「用」的當下房間裡還沒有——爆炸。

要真的改外面的,得先聲明：

- `global x`——「我說的是家門口那個 x」。
- `nonlocal x`——「我說的是客廳那個 x」（[閉包](12-closures.md)的好朋友）。

（但先說：需要大量 global 通常是設計有問題——能傳參數就傳參數。）
...
當程式**讀取**一個名稱，Python 依序在四個作用域裡尋找，找到就停——由近到遠的四個地方：

```text
L  Local      — 當前函式內部（自己房間）
E  Enclosing  — 外層（包住它的）函式（客廳）
G  Global     — 模組層級（.py 檔頂層）（家門口）
B  Built-in   — 內建名稱（print、len、range⋯⋯）（巷口便利商店）
```

**L → E → G → B**，第一個找到的作用域勝出；四層都沒有就 `NameError`。
