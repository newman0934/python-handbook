...
> 把這 12 章串成一張圖——所有架構模式，都在解同一個問題：**怎麼讓「三年後的你」還改得動這份程式碼。**
...
| 新專案要不要一開始就拆微服務 | **不要——先模組化單體**：一個部署 + 強模組邊界，痛了再沿接縫拆 | [ch12](12-modular-monolith.md) |
...
- [ ] 模組化單體和大泥球單體差在哪？它怎麼為「未來拆微服務」預留接縫？（[ch12](12-modular-monolith.md)）
- [ ] 什麼時候「不該」上這些架構？（陷阱題）
