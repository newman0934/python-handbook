...
[線性回歸](04-linear-regression.md)預測**數字**(房價),
但更多商業問題是**是非題**:這封信是垃圾信嗎?這筆交易是詐騙嗎?這客戶會流失嗎?
這叫**分類(classification)**,入門主角是**邏輯回歸(logistic regression)**——
名字有「回歸」,幹的卻是分類的活,很多人第一次都被搞糊塗。

它的巧思是「**在線性回歸外面包一層**」:

```text
1. 先算線性組合 z(和線性回歸一樣,z 可以是任意大小的數)
2. 用 sigmoid 把 z 壓成 0~1 之間的「機率」
3. 機率 ≥ 0.5 判為「是」,否則「否」
```

**sigmoid** 是一條 S 形曲線,專門把「任意實數」擠進「0 到 1」——
於是輸出可以解讀成**機率**(「這封信有 87% 像垃圾信」),
比硬邦邦的 0/1 有用得多。

一個關鍵觀念會反覆用到——**決策閾值(threshold)不是非 0.5 不可**:
抓詐騙寧可錯殺(閾值調低、多攔一些)、推薦商品寧可精準(閾值調高)——
**閾值是你根據業務成本轉的旋鈕**,直接牽動下一章的 precision/recall 取捨。

這章也把多分類(一對多、softmax)、線性 vs 非線性邊界講清楚,
並示範 scikit-learn 幾行訓練一個分類器——**流程和回歸一模一樣**(`fit`/`predict`),
這正是 sklearn 一致介面的威力。
