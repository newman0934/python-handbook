...
你花三天撈數、檢定、畫圖,興沖沖丟出 20 張圖給主管——
他看了三秒問:「所以呢?**我該做什麼?**」——你的分析就白做了。
**資料的價值不在算出來,在讓人聽懂、並據以行動**。這章教說故事的技術。

第一原則反直覺,但一旦學會終生受用——**金字塔原則:結論先行**。
學校教你「鋪陳→推導→結論」(論文式);
商業溝通要**倒過來**:**先甩結論,再給支持**。

```text
【結論】「建議砍掉 A 方案、加碼 B——預估多賺 15%」  ← 第一句就講
   ├─ 支持 1:A 方案的漏斗在付款步驟流失 60%
   ├─ 支持 2:B 方案 A/B 測試顯著勝出
   └─ 支持 3:B 的獲客成本低 30%
```

為什麼?**決策者時間有限、注意力在開頭最集中**。
先給結論,他立刻知道「這關乎什麼、要不要繼續聽」——藏到最後只會讓人失去耐心。

第二個工具是 **SCR 框架**開場:
**Situation(現況,大家都同意的事實)→ Complication(衝突,出問題了)→
Resolution(解方,你的建議)**——三句話把聽眾拉進你的故事。

還有一個把洞察變行動的黏著劑,每個發現都要走完:
**Finding(發現什麼)→ So-what(所以這代表什麼)→ Action(該做什麼)**。
只講 Finding 是報數字;走到 Action 才是分析師的價值。
這章用一個完整案例,把一堆圖表重組成一個讓人點頭的敘事。
