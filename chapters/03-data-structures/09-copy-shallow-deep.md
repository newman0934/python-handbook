...
「我明明複製了一份，怎麼改副本，原本也跟著變？」——這章就是這個經典靈異事件的破案報告。

用「相簿」想。你有一本相簿（外層 list），裡面每頁插著照片（內層物件）。「複製」有三種深度：

1. **`b = a`（賦值）**：根本沒有複製。只是同一本相簿**多貼了一張標籤**（[便利貼](../02-fundamentals/01-dynamic-typing.md)）。當然改哪個都一樣——就是同一本。

2. **淺複製（`a.copy()`、`a[:]`）**：買了一本**新相簿殼**，但每一頁插的是**同一批照片**（內層仍共用）。
   - 對新相簿「換頁、加頁、撕頁」→ 原本不受影響（殼是分開的）✓
   - 但**在照片上畫畫**（修改內層可變物件）→ 兩本一起變 ✗——因為照片根本是同一張！

3. **深複製（`copy.deepcopy(a)`）**：新相簿殼＋**每張照片都重新沖洗一份**——層層全獨立，怎麼改都互不相干。

判斷要用哪種，一個問題就夠：

> **資料有沒有巢狀的可變內層？**（list 裡有 list？dict 的 value 是 list？）
> 沒有 → 淺複製就夠。有、而且會改內層 → 深複製。

「改副本動到原本」的真相,幾乎都是:**你以為複製了全部,其實只複製了殼。**
...
處理「複製」有三種層次，差別在**複製到多深**（相簿模型）：

1. **賦值 `b = a`**：完全不複製——同一本相簿多貼一張標籤（別名）。改任一個都影響彼此。
2. **淺複製（shallow copy）**：建立**新的外層容器**（新相簿殼），但裡面的元素仍是**原元素的參照**（同一批照片，內層共用）。
3. **深複製（deep copy）**：遞迴複製**所有層次**（照片也重新沖洗）——副本與原本完全獨立。

適用判斷：

- 對「一層」的資料（無巢狀可變內層），淺複製就夠獨立。
- 對**巢狀**資料（list of list、dict of list），淺複製的內層仍共用——改內層會互相影響，這時才需要深複製。
