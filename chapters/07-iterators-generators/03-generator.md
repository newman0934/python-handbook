...
**當你要「產生一連串值」、而且不需要它們同時全部待在記憶體裡時,用 generator 取代回傳 list。** 訊號:

- **讀大檔 / 大資料集**:逐行處理幾 GB 的 log——generator 一次一行,記憶體從幾 GB 降到幾 KB。
- **串成處理管線**:`資料源 → 過濾 → 轉換 → 取前 N`,每一步都是 generator,惰性接起來、**只算真正要的**。
- **無限或長度未知的序列**:讀串流、分頁 API——邊拉邊產出,不必等全部到齊。
- **只走一遍就夠**的資料:不需要「可重複遍歷、可索引」,generator 更省。

反過來,**需要多次遍歷、要 `[i]` 索引、要 `len()`、要切片 → 用 list**(generator 只能走一次)。
