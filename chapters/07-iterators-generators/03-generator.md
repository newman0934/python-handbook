...
普通函式呼叫後「一路跑到底、回傳一個值、狀態消失」。**生成器函式**（函式體含 `yield`）完全不同：

- 呼叫它**不執行函式體**，而是回傳一個**生成器物件**（一個 iterator）。
- 每次 `next()`（或 `for`）時，函式體**執行到下一個 `yield`**，吐出 `yield` 的值，然後**暫停**——保留所有區域變數與執行位置。
- 下次 `next()` 從**暫停處繼續**，直到下一個 `yield` 或函式結束（結束時 raise `StopIteration`）。

關鍵字是**暫停與恢復**：生成器記住「執行到哪、變數是什麼」，能走走停停。這讓它天生是 iterator（自動有 `__iter__`/`__next__`），且惰性產值。
