...
先破除一個心態：測試不是「寫完程式後補交的作業」。

它是**走鋼索下面的安全網**。

沒有網：每次改動都是賭命——「這行改下去,會不會弄壞別的地方?」不知道,只能祈禱。
於是出現最典型的症狀：**沒人敢動老程式碼**——不是不想改,是不敢。

有網：改完跑一遍測試,綠燈=沒摔,紅燈=網接住了你,還告訴你摔在哪。**敢改,是測試買給你的最大資產**（防回歸）。

測試還附贈幾個常被低估的紅利：

- **活的說明書**:想知道函式怎麼用?看它的測試——而且測試不會過期（過期會紅燈,註解不會）。
- **設計的照妖鏡**:一個函式很難測?通常代表它設計不良（做太多事、耦合太緊）——寫測試逼你寫出好架構。
- **除錯加速器**:紅燈直接指出「哪個功能壞了」,不用手動點來點去。

這個 Part 的路線:老框架 unittest（看懂）→ **pytest**（主力）→ fixture/參數化/mock（三大武器）→ 覆蓋率、TDD、屬性測試（進階心法）。
...
1. **防回歸（regression）**：改動後跑測試，確認沒弄壞既有功能——最大的價值。**沒測試就不敢改程式**（沒網不敢動作）。
2. **活文件**：測試展示「這個函式該怎麼用、預期什麼行為」——比註解可靠（測試會被執行，過期就紅燈）。
3. **設計回饋**：難測試的程式通常是設計不良（緊耦合、職責不清）——寫測試逼你寫出鬆耦合、可測試的程式。
4. **快速定位**：測試失敗直接指出「哪個功能壞了」。
