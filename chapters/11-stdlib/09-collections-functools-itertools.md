...
這三個模組你其實**都見過了**——這章是把三個工具抽屜並排放好、畫一張總地圖：

| 抽屜 | 裝什麼 | 在哪詳講過 |
|------|--------|-----------|
| **`collections`** | 專用**容器**（Counter、defaultdict、deque⋯⋯） | [Part 3](../03-data-structures/08-collections-module.md) |
| **`functools`** | 處理**函式**的工具（wraps、lru_cache、partial⋯⋯） | [Part 8](../08-functional-decorators/05-functools.md) |
| **`itertools`** | **惰性迭代**積木（chain、islice、product⋯⋯） | [Part 7](../07-iterators-generators/06-itertools.md) |

記憶法一句話：**容器找 collections、函式找 functools、迭代找 itertools**——名字就是分類。

三者的共同精神,也是標準庫哲學:「**電池內建（batteries included）**」——常見的輪子別自己造,先翻抽屜。

這章的用法:當**速查地圖**——依「我要處理的是容器/函式/序列?」定位抽屜,再跳去對應深講的章節。適合快讀。
...
| 模組 | 提供什麼 | 一句話 |
|------|----------|--------|
| **`collections`** | 專用容器型別 | 補內建 dict/list 的不足（容器抽屜） |
| **`functools`** | 操作/增強函式的工具 | 處理函式的函式（函式抽屜） |
| **`itertools`** | 惰性迭代工具 | 高效組合/處理序列（迭代抽屜） |

共同點：**都是「電池內建」的高品質工具——用它們取代自己造輪子**。名字即分類：容器/函式/迭代。
