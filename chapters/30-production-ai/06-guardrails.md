...
[上一章](05-prompt-injection-security.md)談攻擊者主動的注入;這章談更廣的**護欄(guardrails)**——
在 LLM 的**入口和出口各裝一道濾網**,確保「進去的」和「出來的」都安全、合規、合格。
想像 LLM 是一台強大但沒常識的機器,護欄就是它前後的**安全閘門**。

**輸入護欄(進 LLM 前的濾網):**

- **PII 遮蔽**:把使用者輸入裡的個資(email、電話、身分證號)先**遮蔽或替換**
  (`john@x.com` → `[EMAIL]`)——避免把個資送給第三方 API、避免寫進 log。
  需要時最後再用對映表還原。
- **注入偵測、惡意內容過濾、長度限制**:把明顯有問題的輸入擋在門外。

**輸出護欄(LLM 回應送出前的濾網):**

- **格式/schema 驗證**:確保回的 JSON 合法、欄位齊全(呼應[結構化輸出](../28-llm-genai/04-structured-output-tools.md))。
- **內容安全**:過濾有害、不當、洩漏機密的回應。
- **幻覺/忠實度檢查**:在 RAG 場景,驗證答案有沒有忠於撈回的資料
  (呼應 [RAG 評估的 faithfulness](../29-ai-applications/04-rag-evaluation.md))。
- **競品/越界話題攔截**:客服機器人不該幫你寫詩、不該談競品。

關鍵設計原則:護欄要能**攔截(block)、改寫(rewrite)、或放行**,
而且**本身要快又便宜**(常用規則、小模型、正則,而非每次都叫大模型)——
否則護欄自己變成延遲和成本的瓶頸。這章實作一套可插拔的輸入/輸出護欄管線,
是 LLM 產品**合規上線**(尤其金融、醫療、對外客服)的必備。
